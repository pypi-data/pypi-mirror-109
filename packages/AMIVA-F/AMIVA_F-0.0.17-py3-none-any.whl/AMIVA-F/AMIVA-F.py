...
class Entry_formular:
    def __init__(self, master):
        ...
        self.label1 = tk.Label(self.frame, text="Please enter your mutation of interest into the entry field below. \n"
                                                "Accepted format: e.g H1929P \n"
                                                "for Histidine being substituted by Proline in the mutation at position"
                                                " 1929 according to UniProt Q14315")
    ...
    def hydrophobicity_atomnumb(self):
        ...
        "unelegant solution to retrieve IDS e.g H1928P, K82R ... from user after preparation because WEKAs outfile " \
    ...
    # print(cls)    #looks good as well. shows weights for each node
        ...
        for keys, values in prediction_disease_dict.items():
            if keys == "yes":
                ...
                self.wekaentryid.insert(0, intro_form.entry1.get())
        ...
        with open(dirtarget, "w", encoding="utf-8") as mutfh:
            for userinput in entryids[0:-1]:  #exclude SAP
                if userinput.get() == "":
                    lstmerge.append("?")
                    continue
                lstmerge.append(str(userinput.get()))
            lstmerge.append(self.var.get())
            lstmerge.append(self.var1.get())
            lstmerge.append(self.var2.get())
            lstmerge.append(self.var3.get())
            if self.SAPentry.get() == "":
                lstmerge.append("?")
            else:
                lstmerge.append(self.SAPentry.get())
            ...
            if len(intro_form.phosphohits) == 0:
                mutfh.write("\nNo posttranslational modifications found within 8Å surrounding\n\n")
            else:
                mutfh.write("\nFound the following posttranslational modification positions within 8Å surrounding:\n")
                for entries in intro_form.phosphohits:
                    mutfh.write(str(entries) + "\n")
