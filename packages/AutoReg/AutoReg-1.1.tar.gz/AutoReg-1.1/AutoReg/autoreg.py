class AutoReg:
    ...
    def find_char_regex(self,ValueString):
        if ValueString.isalpha()==True:
            if self.Case_Sensitive==True and self.Char_Sensitive==True:
                if ValueString.isupper()==True:
                    return '[A-Z]'
                else:
                    return '[a-z]'
                ...
                return '\D+'
        elif ValueString.isnumeric()==True:
            ...
            if self.Space_Sensitive==True:
                ...
                return '\s+'
        elif ValueString in self.regex_special_char:
            if self.Specialchar_Sensitive==True:
                return '\\'+ValueString
            else:
                return '\W+'

    def getRegex(self,ValueString,Case_Sensitive=True,Char_Sensitive=True,Digit_Sensitive=True,Space_Sensitive=True,Specialchar_Sensitive=True):
        try:
            ...
            if ValueString!='' and isinstance(ValueString,str)==True:     
                regex=[self.find_char_regex(i) for i in ValueString]
                final_str_ls=[]
                inter_ls=[regex[0]]
                regex.append('A1S@')
