...
def counting_sort(input_list: Sequence[T],
    ...
    indices: List[int] = list(0 for _ in range(slots))
...
def interpolate_line(point: Point2D, line: Line3D) -> float:
    ...
    line_vec: numpy.ndarray = numpy.array(  # type: ignore
        (line[1][0]-line[0][0], line[1][1]-line[0][1]))
    point_vec: numpy.ndarray = numpy.array(  # type: ignore
...
def half_plane_distance(line: Line2D, point: Point2D,
    ...
    vec: numpy.ndarray = numpy.array(  # type: ignore
        (line[1][0]-line[0][0], line[1][1]-line[0][1]))
    # Get the right-hand normal vector of the line
    norm_vec: numpy.ndarray = numpy.array((vec[1], -vec[0]))  # type: ignore
    ...
    check_vec: numpy.ndarray = numpy.array(  # type: ignore
        (point[0]-line[0][0], point[1]-line[0][1]))
...
def distance_to_polygon(point: Point2D, polygon: Polygon2D) -> float:
    ...
    for index, vtx_1 in enumerate(polygon):
        ...
        edge_vec: numpy.ndarray = numpy.array(  # type: ignore
...
def line_segments_intersection(line_1: Line2D, line_2: Line2D,
    ...
    vec_1: numpy.ndarray = numpy.array(  # type: ignore
        (pt_b[0]-pt_a[0], pt_b[1]-pt_a[1]))
    vec_2: numpy.ndarray = numpy.array(  # type: ignore
        (pt_d[0]-pt_c[0], pt_d[1]-pt_c[1]))
    vec_3: numpy.ndarray = numpy.array(  # type: ignore
...
def point_on_line(line: Line2D, point: Point2D,
    ...
    if not ((line[0][0] <= point[0] <= line[1][0]
        ...
        return False
    line_vec = numpy.array(  # type: ignore
        (line[1][0]-line[0][0], line[1][1]-line[0][1]))
    point_vec = numpy.array(  # type: ignore
        (point[0]-line[0][0], point[1]-line[0][1]))
