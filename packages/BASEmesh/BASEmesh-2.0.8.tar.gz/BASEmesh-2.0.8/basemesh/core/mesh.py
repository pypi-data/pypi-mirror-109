...
class MeshElement(SupportsSpatial):
    ...
    def area(self) -> float:
        ...
        pt_a: np.ndarray = np.array(self.points[0])  # type: ignore
        pt_b: np.ndarray = np.array(self.points[1])  # type: ignore
        pt_c: np.ndarray = np.array(self.points[2])  # type: ignore
        # Get vectors
        vec_ab = pt_b - pt_a
        vec_ac = pt_c - pt_a
...
class Mesh(ElevationSource):
    ...
    def elevation_at(self, point: Union[Point2D, Point3D]) -> float:
        ...
        try:
            ...
            # the point to check
            triangle_2d: Triangle2D = tuple(  # type: ignore
