...
class FAQWrapper:
    ...
    def solveForQuery(
        self, queryVector: np.ndarray, K: int, topSimilar: int = 5
    ) -> FAQOutput:
        ...
        return FAQOutput(
            faqId=self.id,
            faqName=self.FAQ.name,
            answer=bestAnswer,
            question=bestMatchQuestion,
            score=bestScore,
            similarQuestions=self._getClosestQuestions(
                rankedIndices, topSimilar, bestMatchQuestion.text
            ),
            maxScore=maxScore,
        )
...
class Bani:
    def __init__(
        self, FAQs: List[FAQ], modelPath: str = None, assignVectors: bool = True
    ):
        if modelPath == None:
    ...
    def _registerFAQs(self, FAQs: List[FAQ]):
        ...
        for faq in FAQs:
            if faq.hasVectorsAssigned() == True and self.assignVectors == False:
                ...
                raise VectorNotAssignedException(
                    "assignVectors is False , but the vectors are not stored in faq named {}".format(
                        faq.name
                    )
                )
    ...
    def findClosest(
        self, query: str, K: int = 3, topSimilar: int = 5
    ) -> List[FAQOutput]:
        """
        ...
        for faq in self.FAQs:
            competeList.append(
                faq.solveForQuery(queryVector=queryVector, K=K, topSimilar=topSimilar)
            )
    ...
    def findClosestFromFAQ(
        self, faqId: int, query: str, K: int = 3, topSimilar: int = 5
    ) -> FAQOutput:
        assert (
            faqId in self.idToFAQ
        ), "The id {} not in faqId only {} are available".format(
            faqId, list(self.idToFAQ.keys())
        )
    ...
    def test(self, faqId: int, testData: List[Tuple[str, str]], K: int = 3) -> float:
        ...
        if nonMatched:
            warnings.warn(
                "{} questions in the test set did not match any orignal questions".format(
                    len(nonMatched)
                )
            )
    ...
    def train(
        ...
        **kwargs
        ...
        for faq in self.FAQs:
            lossInstance = LossHandler(
                lossName=lossName, model=self.model, FAQ=faq.FAQ, batchSize=batchSize
            )
        ...
        self.model.fit(
            train_objectives=trainingObjectives,
            epochs=epochs,
        ...
        for faq in self.FAQs:
            print(
                "Assigning vectors from the trained model to FAQ {}".format(
                    faq.FAQ.name
                )
            )
