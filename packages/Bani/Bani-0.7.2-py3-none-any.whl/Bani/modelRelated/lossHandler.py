...
from .utils import (
    convertForBatchHardTripletLoss,
    convertForContrastiveLoss,
    convertForTripletLoss,
...
class LossHandler:
    def __init__(self, lossName: str, FAQ, model, batchSize: int):
        ...
        assert (
            lossName in supportedLosses
        ), "Error {} loss not supported , supported losses are {}".format(
            lossName, supportedLosses
        )

        if lossName == supportedLosses[0]:
            assert (
                batchSize > 8 and batchSize % 8 == 0
            ), "needed for the pk sampler (p ,k > 2), must give large batches for batch hard"
            ...
            self.trainDataLoader = DataLoader(
                self.dataset, batch_size=batchSize, sampler=sampler
            )

        elif lossName == supportedLosses[1]:
            assert (
                batchSize > 4 and batchSize % 4 == 0
            ), "needed for good contrastive loss"
            ...
            self.trainDataLoader = DataLoader(
                self.dataset, shuffle=True, batch_size=batchSize
            )
