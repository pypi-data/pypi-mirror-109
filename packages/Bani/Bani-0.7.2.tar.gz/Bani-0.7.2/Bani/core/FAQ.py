...
class FAQUnit:
    def __init__(self, id: int, question: Question, orignal: Question, answer: Answer):
        ...
        """
        assert (
            question.label == orignal.label
        ), "label for orignal and current question must be same"
        self.id = id
...
class FAQ:
    def __init__(
        self, name: str, questions: List[str] = None, answers: List[str] = None
    ):
        self.name = name
        ...
        if not (questions is None or answers is None):
            assert len(questions) == len(answers)
            self.questions, self.answers = processRaw(
    ...
    def getAnswerWithLabel(self, label: int) -> Answer:
        ...
        if label not in self.l2A:
            raise LabelDoesNotExists(
                "The answer with label {} does not exists".format(label)
            )
    ...
    def getQuestionWithLabel(self, label: int) -> Question:
        ...
        if label not in self.l2Q:
            raise LabelDoesNotExists(
                "The question with label {} does not exists".format(label)
            )
    ...
    def _paraphrase(self, generator: GenerateManager):
        ...
        if len(generatedQuestions) != len(self.questions):
            raise WrongGeneratorOutputException(
                "number of orignal questions is {} but generator returned {} Lists !!!".format(
                    len(self.questions), len(generatedQuestions)
                )
            )
        ...
        for orignalQuestion, paraphrases in zip(self.questions, generatedQuestions):
            ...
            tempFAQ.append(
                FAQUnit(idCount, orignalQuestion, orignalQuestion, answer=answer)
            )
            idCount += 1
    ...
    def _assignVectors(self, model):
        ...
        if self.isUsable() == False:
            raise AttemptedUsingNonUsableFAQ(
                "Must generate paraphrases before assigning vectors"
            )
    ...
    def buildFAQ(self, generator: GenerateManager, model=None):
        ...
        if generator is None:
            generator = GenerateManager(
                producers=[IdentityProducer()], names=["IdentityProducer"], nums=[1]
            )
    ...
    def isEmpty(self):
        if (
            ...
            or len(self.questions) == 0
            or len(self.answers) == 0
        ):
            return True
