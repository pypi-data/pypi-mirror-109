...
class QuestionGenerator:
    def __init__(self, name: str, producer, num: int):
        ...
        if "exact_batch_generate" not in dir(producer) and "batch_generate" not in dir(
            producer
        ):
            raise AttributeError(
                "Must have either batch_generate or exact batch generate in the producer object !!"
            )
...
class GenerateManager:
    ...
    def __init__(
        self, producers: List[Any], names: List[str] = None, nums: List[int] = None
    ):
        """
        ...
        assert len(names) == len(
            set(names)
        ), "The names of generators must be unique !!!"

        if nums is None:
            nums = [10 for i in range(len(producers))]

        assert len(producers) == len(names) == len(nums)
        self.generators: List[QuestionGenerator] = [
            QuestionGenerator(name=names[i], producer=producers[i], num=nums[i])
            for i in range(len(producers))
        ]
    ...
    def removeProducer(self, name):
        ...
        if name not in preNames:
            raise ValueError(
                "A producer with name {} does not exists exists".format(name)
            )
    ...
    def _sequential(self, questions):
        ...
        for generator in self.generators:
            try:
                ...
                warnings.warn(
                    "{} pipeline failed !!!! {}".format(generator.name, exception)
                )
