...
def minLabelChecker(lossName: str, FAQ: FAQ, minSameLabels: int):
    ...
    for L, count in counter.items():
        if count < minSameLabels:
            raise TrainDataInvalid(
                "Attempted to use {} loss but label {} has only {} exmaples !!! At least {} required".format(
                    lossName, L, count, minSameLabels
                )
            )
...
def convertForSoftmaxLoss(FAQ: FAQ):
    ...
    labelToQuestions: Dict[
        int, List[str]
    ] = dict()  # Contains mapping from the loss labels to questions
    faq = FAQ.FAQ
    index = 0
    for unit in faq:
        label = unit.label
