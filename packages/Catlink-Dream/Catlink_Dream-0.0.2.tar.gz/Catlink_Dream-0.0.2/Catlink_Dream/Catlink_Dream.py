...
class 文件句读(object):
    ...
    def 下一实词或虚词(自身):
        while True:
            ...
            意为右括号序列 = ['】', '也~', '者~', '耳~', '焉~', '哉~']
            for 左起 in 意为左括号序列:
                自身.行内容 = re.sub(左起, ' 【 ', 自身.行内容)
            for 右落 in 意为右括号序列:
                自身.行内容 = re.sub(右落, ' 】 ', 自身.行内容)
...
def 按实词解(实词或虚词):
    ...
    elif 实词或虚词[0] == '"': return 实词或虚词[1:-1].decode('unicode_escape')
...
def 转絮梦表达式(表达式):
    ...
    #这一行留给字符串类型
    elif isinstance(表达式, list): return '【' + ' '.join(map(转絮梦表达式, 表达式)) + '】'
    else: return str(表达式)
...
def 伊始忆义():
    ...
    忆.update({
        ...
        '输出':       lambda 表达式,控制台输出=sys.stdout: 控制台输出.write(表达式 if isinstance(表达式, str) else 转絮梦表达式(表达式))
    })
    return 忆
底层忆义 = 伊始忆义()
...
#class 编译器:
#    def __init__(自身, 文件 = input("希望运行的文件的绝对路径：")):
#        自身.文件 = 文件
#    def 运行(自身):
#        编译运行(自身.文件)
