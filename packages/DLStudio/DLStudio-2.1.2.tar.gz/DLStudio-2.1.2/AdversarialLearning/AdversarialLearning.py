...
__date__      = '2021-June-13'   
__url__       = 'https://engineering.purdue.edu/kak/distDLS/DLStudio-2.1.2.html'
...
WGAN:
    ...
    "gradient target" of +1 for mean of the output of the Critic when it sees
    the images produced by the Generator and the "gradient target" of -1 for
    the output of the Critic when it sees the training data directly.  As to
    why we use the gradient targets of +1 and -1, it was shown by the
    original authors of WGAN that the optimal Critic has unit gradient norm
    almost everywhere under P and Q.  That is, the magnitude of the partial
    derivative of the output of the optimal Critic with respect to its input
...
WGAN-GP:
    ...
    the authors Gulrajani, Ahmed, Arjovsky, Dumouli, and Courville of the
    paper "Improved Training of Wasserstein GANs" (which followed the
    original WGAN publication) that implementing a 1-Lipschitz constraint
    with weight clipping biases the Critic towards learning extremely simple
    probability distribution functions.  The authors showed how the
    performance of a WGAN could be improved by putting to use the theoretical
    property that the optimal WGAN critic has unit gradient norm almost
    everywhere under P and Q. [See Proposition 1, Corollary 1 of the paper
   ...
   Critic Loss  =   E   | C(G(z)) |     -    E | C(x) |    +    λ.| ||∇_x̂ C(x̂) ||^2  − 1   |
    ...
    (meaning the Critic itself) with respect to its input.  Since both Critic
    ...
    by taking a weighted such of those drawn from the training data and those
    produced by the Generator:

               x̂  ←  εx  +  (1 − ε) x̃

    We feed it into the Critic and, based on the input-output values, estimate
    the gradient needed in the GP term in Eq. (9).
...
class AdversarialLearning(object):
    ...
    class DataModeling(nn.Module):             
        ...
        ############################################################################################################
        ##  The training routines follow, first for a GAN constructed using either the DG1 and or the DG2 networks, 
        ##  and then for a WGAN constructed using the CG1 networks.
        ############################################################################################################
        ...
        def run_wgan_code(self, dlstudio, advers, critic, generator, results_dir):
            ...
            clipping_thresh = self.advers.clipping_threshold
            # For each epoch
            for epoch in range(dlstudio.epochs):        
                ...
                while i < len(dataloader):
                    ...
                    if i % (ncritic * 20) == 0:   
                        current_time = time.perf_counter()                                                            
                        elapsed_time = current_time - start_time                                                      
                        print("[epoch=%d/%d   iter=%4d   el_time=%5d secs]     loss_critic=%7.4f   loss_gen=%7.4f   Wasserstein_dist=%7.4f" %  (epoch,dlstudio.epochs,i,elapsed_time,loss_critic.data[0], loss_gen.data[0], wasser_dist.data[0]))
        ...
        def run_wgan_with_gp_code(self, dlstudio, advers, critic, generator, results_dir):
            """

            This function is meant to be run with Critic-Generator pair CG2.  Regarding how to set the 
            parameters of this method, see the following script
            ...
            for epoch in range(dlstudio.epochs):        
                ...
                while i < len(dataloader):
                    ...
                    if i % (ncritic * 20) == 0:   
                        current_time = time.perf_counter()                                                            
                        elapsed_time = current_time - start_time                                                      
                        print("[epoch=%d/%d   iter=%4d   el_time=%5d secs]     loss_critic=%7.4f   loss_gen=%7.4f   Wasserstein_dist=%7.4f" %  (epoch+1,dlstudio.epochs,i,elapsed_time,loss_critic.data[0], loss_gen.data[0], wasser_dist.data[0]))
                    Gen_losses.append(loss_gen.data[0])      
                    Cri_losses.append(loss_critic.data[0])   
                    #  Get G's output on fixed_noise for the GIF animation:
                    if (iters % 500 == 0) or ((epoch == dlstudio.epochs-1) and (i == len(self.train_dataloader)-1)): 
