...
class FetchSLYEngine(FetchEngine):
    ...
    def esg_metrics_indicator(self, codes=None, name=None, begin_date=None, 
        ...
        clause_list = and_(table.__dict__['effective'] == 1,
                           table.__dict__['effective'].isnot(None))
        if codes is None:
        ...
        if name is None:
            ...
            clause_list.append(exp)
        if begin_date is not None and end_date is not None:
            clause_list.append(table.__dict__[time_name] >= begin_date)
            clause_list.append(table.__dict__[time_name] <= end_date)
