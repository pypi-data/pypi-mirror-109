...
class Inbreeding():
    ...
    def applyInbreeding(self, selectedAnimals:pd.DataFrame, allAnimals:pd.DataFrame) -> pd.Series:
        print(selectedAnimals, allAnimals, sep="\n")
        r = selectedAnimals.groupby(selectedAnimals["Ear tag"]).apply(
    ...
    def __inbreedCoeff(self, animal:pd.Series, allAnimals:pd.DataFrame) -> pd.Series:
        ...
        if sireIndex <= 0 or damIndex <= 0:
            ...
            try:
                ...
                averageDam = 0
            print(animal, np.min(abs(averageDam), abs(averageSire)), sep="\t")
            return pd.Series(
