...
class Inbreeding():
    ...
    def __cffa(self, sire:np.int64, dam:np.int64, allAnimals:pd.DataFrame) -> np.float64:
        ...
        """
        try:
            sireIndex = allAnimals.index[allAnimals["Ear tag"] == sire][0] + 1
        except KeyError:
            sireIndex = 0
        try:
            damIndex = allAnimals.index[allAnimals["Ear tag"] == dam][0] + 1
        except KeyError:
            damIndex = 0
        if sireIndex != 0:
            try:
                sireAverage = allAnimals.loc[
                    allAnimals["Year of Birth"] == allAnimals.loc[sireIndex - 1,"Year of Birth"],
                    "Inbreeding"
                ].mean()
            except KeyError:
                sireAverage = 0
        ...
        if damIndex != 0:
            try:
                damAverage = allAnimals.loc[
                    allAnimals["Year of Birth"] == allAnimals.loc[damIndex - 1,"Year of Birth"],
                    "Inbreeding"
                ].mean()
            except KeyError:
                damAverage = 0
