...
class LSSM:
    ...
    def getListOfParams(self):
        ...
        for field in dir(self): 
            ...
            if not field.startswith('__') and isinstance(val, (np.ndarray, list, tuple)):
                params[field] = val
        return params
    ...
    def randomize(self, randomizationSettings = None):
        ...
        while not isOk:
            ...
            try:
                ...
                print('Error in updating secondary parameters:\n{}\nRegerating random noises...'.format(e))
                pass
    ...
    def generateRealization(self, N, random_x0=False, x0=None, w0=None):
        if random_x0 and self.state_dim > 0:
            x0 = np.atleast_2d(np.random.multivariate_normal(mean=np.zeros(self.state_dim), cov=self.XCov)).T
