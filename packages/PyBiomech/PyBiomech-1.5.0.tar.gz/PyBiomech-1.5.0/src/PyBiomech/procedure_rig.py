...
def performAnalysis(
    ...
    results['paths'].update(ligaPaths)
    ...
    for L2PProjName in L2PProjsList:
        ...
        for i in range(N):
            ...
            pointsInRef = kine.changeMarkersReferenceFrame(points, RTi[i,...])
            ...
            pointsProj = kine.changeMarkersReferenceFrame(pointsInRef, RT[i,...])
            pointsProj = np.array([pointsProj[m][0,:] for m in pointsProj])
    
            lineProjPaths[name][i] = pointsProj
        
        
    results['paths'].update(lineProjPaths)
    ...
    interLineAnglesList = params['calcInterLineAnglesList']['selected']
