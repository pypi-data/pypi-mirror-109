...
class RKClient:
    ...
    def __init__(self, receiver_url: str, emitter_id: Optional[UUID] = None, timeout_sec: int = 5):
    ...
    def send_pem(self, pem: PEM) -> Tuple[str, bool]:
        ...
        def _send_pem():
            return requests.post(url=self.receiver_url, data=payload, timeout=self.timeout_sec)
    ...
    def ping(self) -> Tuple[str, bool]:
        ...
        """
        def _ping():
            return requests.get(url=self.receiver_url + "/ping", timeout=self.timeout_sec)
    ...
    def get_info(self) -> Tuple[str, bool]:
        ...
        """
        def _get_info():
            return requests.get(url=self.receiver_url + "/info", timeout=self.timeout_sec)
    ...
    def get_tag(self, namespace: UUID, tag_name: str) -> Tuple[str, bool]:
        ...
        def _get_tag():
            return requests.get(url=self.receiver_url + f"/tag/{namespace.hex}/{tag_base64}", timeout=self.timeout_sec)
    ...
    def set_tag(self, namespace: UUID, tag_name: str, pem: PEM) -> Tuple[str, bool]:
        ...
        def _set_tag():
            return requests.post(url=self.receiver_url + f"/tag/{namespace.hex}/{tag_base64}/{pem.ID.hex}", timeout=self.timeout_sec)
