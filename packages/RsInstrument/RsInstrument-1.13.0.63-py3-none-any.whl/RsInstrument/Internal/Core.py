...
class Core(object):
	...
	def reconnect(self, force_close: bool = False) -> bool:
		...
		if not self.allow_reconnect:
			raise Exception('Reused sessions do not support reconnection')
		active = self.io.is_connection_active()
		...
		if active is True:
			return False
		if not active:
			self.io = Instrument(self.resource_name, self.simulating, self._instrumentSettings, None)
			self._apply_settings_to_instrument(self.sett_dr)
			self._apply_settings_to_instrument(self.sett_user)
			self.io.set_simulating_cmds()
			self.io.check_status()
