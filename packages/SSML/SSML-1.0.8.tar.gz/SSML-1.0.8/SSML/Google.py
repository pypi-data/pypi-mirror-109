...
class Google:
    ...
    def breakk(self, time=None, strength=None):
        ...
        Args:
            time (str): Sets the length of the break by seconds or milliseconds (time must be in milliseconds).
        ...
        """
        if time:
            self._speech += f"<break time='{time}ms'/>"
        elif strength:
    ...
    def audio(self, src, alt, description=None, clip_begin=0, clip_end="infinity", speed="100%", repeat_count="1", repeat_dur="infinity", sound_level="+0dB"):
        ...
        self._speech += f"<audio src='{src}' clipBegin='{clip_begin}s' "
        if clip_end != "infinity":
            self._speech += f"clipEnd='{clip_end}s' "
        else:
            self._speech += f"clipEnd='{clip_end}' "
        self._speech += f"speed='{speed}' repeatCount='{repeat_count}' repeatDur='{repeat_dur}' soundLevel='{sound_level}'>"
        if description:
