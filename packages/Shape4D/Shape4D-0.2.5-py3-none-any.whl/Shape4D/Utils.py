...
def UnitVector(v): 
    L = Length(v)
    if ScalarZero(L):
       raise ValueError('UnitVector() : v/length(v) : length(v)->0 !!!')
    return v/L   
...
# To check if  P is inside a segment(P1,P2), True/False
...
# (2) None, no intersection;
# (3) 'coplane', plus a warning message on console: there are two points of intersection
...
def LineXPolygon(P0,L,Polygon):
    ...
    if Point == 'coplane':                # If the line falls in the plane, it goes into 2D space
        PL=P0+L
...
# (1) P1
# (2) None
# (3)'coplane', the line is on the plane
