...
class DataTreatment:
    """
        ...
        law_calculation(self, law_model, cumlutative=True)
    ...
    def law_calculation(self, law_model, cumlutative=True):
        """
        Calulate 100 values x bounded (bewtween 1 and 99 percentiles respectively) and calculate law data corresponding to the parameters founds 
        """
        ...
        data["default"] = law[0].cdf(**temp) if cumlutative else law[0].pdf(**temp)
        ...
        if self.models["Method"] == u"mle":
                ...
                data["upper_bound"] = law[0].cdf(**temp) if cumlutative else law[0].pdf(**temp)
                
                temp["c"] = self.estimation_result["Weibull"]["b_%.2f"%((1. + self.confidence)/2)]
                temp["scale"] = self.estimation_result["Weibull"]["h_%.2f"%((1. + self.confidence)/2)]
                data["lower_bound"] = law[0].cdf(**temp) if cumlutative else law[0].pdf(**temp)
    ...
    def model_visualization(self):
        ...
        for law_model in results:
            ...
            #Calulate 100 values x bounded (bewtween 1 and 99 percentiles respectively) and calculate data matching with the parameter found 
            x, data = self.law_calculation(law_model)
            
            ax.plot(x, data["default"], 'b-', lw=2, alpha=0.6, label=u"%s cdf estimation %s" % (law_model, self.param_title_graph(law_model))) 
            ...
            if self.models["Method"] == u"mle":
                #Add up and bottom confidence dot curve include in mle method
                ax.plot(x, data["upper_bound"], 'm--', lw=2, alpha=0.6, label=u"cdf %.2f β = %.1f, η = %.2f" % (((1. - self.confidence)/2)*100, self.estimation_result["Weibull"]["b_%.2f"%((1. - self.confidence)/2)], self.estimation_result["Weibull"]["h_%.2f"%((1. - self.confidence)/2)])) 
                ax.plot(x, data["lower_bound"], 'c--', lw=2, alpha=0.6, label=u"cdf %.2f β = %.1f, η = %.2f\n" % ( ((1. + self.confidence)/2)*100, self.estimation_result["Weibull"]["b_%.2f"%((1. + self.confidence)/2)], self.estimation_result["Weibull"]["h_%.2f"%((1. + self.confidence)/2)]))  
            ...
            if law_model == "Weibull":
                #Add the Allain Plait representation if it's a weibull law 
                i += 1
                self.ax[i].plot(np.log(x), np.log(-np.log(1 - data["default"])), 'r-', lw=2, alpha=0.6, label="Allan plait weibull representation")
