...
def AutoUpdate():
    if 200 == requests.head("https://pypi.org/project/YouTubeMusicAPI/1.9/").status_code:
        if "Windows" == platform.system():
            os.system("pip install YouTubeMusicAPI==1.9")
...
def getsonginfo(song_name):
    ...
    Data = {"Name" : f"{query['title']}", "ID" : f"{query['id']}", "Total Listeners" : f"{str(query['views'].replace(' views', ''))}", "Duration" : f"{query['duration']}", "Artist" : f"{query['channel']}", "URL" : f"https://music.youtube.com/watch?v={query['id']}", "Album Art" : f"https://i.ytimg.com/vi/{query['id']}/hq720.jpg"}
    return Data
