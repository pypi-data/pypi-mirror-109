...
def trips():
    ...
    trips = pd.DataFrame(data={'tour_id': [1, 1, 2, 2, 2, 2, 2, 3, 3, 4],
                               ...
                               'is_work': [True, True, False, False, False, False, False, False, False, True],
                               'is_school': [False, False, False, False, False, False, False, True, True, False],
                               'is_eatout': [False, False, True, True, True, True, True, False, False, False],
...
@pytest.fixture(scope='module')
def model_spec():
    index = ["@(df['stop_time_duration'] * df['is_work'].astype(int)).astype(int)",
...
def test_build_patterns(trips):
    ...
    output_columns = [tdc.TOUR_ID, tdc.PATTERN_ID, tdc.TRIP_NUM,
                      tdc.STOP_TIME_DURATION, tdc.TOUR_ID, tdc.OUTBOUND]
...
def test_generate_alternative(trips):
    ...
    pd.testing.assert_series_equal(trips.groupby(trips.index)['trip_duration'].max(),
