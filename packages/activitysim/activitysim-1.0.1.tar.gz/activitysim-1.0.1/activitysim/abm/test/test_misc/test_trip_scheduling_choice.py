...
def tours():
    tours = pd.DataFrame(data={'duration': [2, 44, 32, 12, 11, 16],
...
@pytest.fixture(scope='module')
def model_spec():
    index = ["@(df['main_leg_duration']>df['duration']).astype(int)",
    ...
    values = [-999, -6.5884, -5.0326, -2.0526, -1.0313, -0.46489, 0.060382, -0.7508, 0.53247]
...
def test_generate_schedule_alternatives(tours):
    ...
    output_columns = [tsc.SCHEDULE_ID, tsc.MAIN_LEG_DURATION,
                      tsc.OB_DURATION, tsc.IB_DURATION]

    assert set(output_columns).issubset(windows.columns)


def test_no_stops_patterns(tours):
    no_stops = tours[(tours['num_outbound_stops'] == 0) & (tours['num_inbound_stops'] == 0)].copy()
    windows = tsc.no_stops_patterns(no_stops)

    assert windows.shape[0] == 1
    assert windows.shape[1] == 3

    output_columns = [tsc.MAIN_LEG_DURATION,
                      tsc.OB_DURATION, tsc.IB_DURATION]

    assert set(output_columns).issubset(windows.columns)

    pd.testing.assert_series_equal(windows[tsc.MAIN_LEG_DURATION], no_stops['duration'],
...
def test_one_way_stop_patterns(tours):
    one_way_stops = tours[((tours['num_outbound_stops'] > 0).astype(int) +
                           (tours['num_inbound_stops'] > 0).astype(int)) == 1].copy()
...
def test_two_way_stop_patterns(tours):
    two_way_stops = tours[((tours['num_outbound_stops'] > 0).astype(int) +
                           (tours['num_inbound_stops'] > 0).astype(int)) == 2].copy()
...
def test_run_trip_scheduling_choice(model_spec, tours, skims, locals_dict):
    ...
    out_tours = tsc.run_trip_scheduling_choice(model_spec, tours, skims, locals_dict,
                                               2, None, "PyTest Trip Scheduling")

    assert len(tours) == len(out_tours)
    pd.testing.assert_index_equal(tours.sort_index().index, out_tours.sort_index().index)
    ...
    assert len(out_tours[out_tours[output_columns].sum(axis=1) == out_tours[tsc.TOUR_DURATION_COLUMN]]) == len(tours)
