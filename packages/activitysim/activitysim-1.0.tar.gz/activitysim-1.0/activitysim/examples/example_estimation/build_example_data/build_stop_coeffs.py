...
for what in ['work', 'school', 'univ', 'social', 'shopping', 'eatout', 'escort', 'atwork', 'othmaint', 'othdiscr']:
    ...
    df['coefficient_name'] = \
        np.where(dupes, 'coef_' + df.Description + '_' + df['alt'], 'coef_' + df.Description)
    df['coefficient_name'] = df['coefficient_name'].str.lower()
    df['coefficient_name'] = df['coefficient_name'].str.replace('[^a-zZ-Z0-9]+', '_', regex=True)
    del df['alt']
    ...
    for index, row in df.iterrows():
        ...
        spec.loc[spec.Description == row['Description'], alt_cols] = [m.get(a, a) for a in alts]
