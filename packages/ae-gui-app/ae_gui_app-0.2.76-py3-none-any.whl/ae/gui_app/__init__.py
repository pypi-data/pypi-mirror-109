...
class MainAppBase(ConsoleApp, ABC):
    ...
    def font_min_max_size_from_win(self, win_width: float, win_height: float) -> Tuple[float, float]:
        ...
        """
        min_len = min(win_width, win_height) * self.dpi_factor()
    ...
    def setup_app_states(self, app_states: AppStateType):
        ...
        if not MIN_FONT_SIZE <= font_size <= MAX_FONT_SIZE:   # adopt device scaling on very first app start
            if font_size == -1.0:
                min_f, max_f = self.font_min_max_size_from_win(win_width, win_height)
