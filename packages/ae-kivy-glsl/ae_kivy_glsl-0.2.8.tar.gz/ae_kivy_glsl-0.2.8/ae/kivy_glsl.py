...
this ae namespace portion provides the mixin class :class:`ShadersMixin` that can be combined with any Kivy widget for
to display GLSL-/shader-based graphics, gradients and animations.
...
to disable the automatic creation of a timer event pass a zero value to this argument.
...
{
  vec2 pix_pos = (frag_modelview_mat * gl_FragCoord).xy - win_pos;
  float len = length(pix_pos - (center_pos - win_pos));
...
class ShadersMixin:
    ...
    def add_shader(self, add_to: str = '',
        ...
        :param start_time:      base/start time. passing the default value zero is syncing the `time` glsl parameter
                                of this shader with :meth:`kivy.clock.Clock.get_boottime()`.
                                pass None to initialize this argument to the current Clock boot time; this
    ...
    def _pos_changed(self, *_args):
        ...
        for ren in self.running_shaders:
    ...
    def refresh_shader(self, shader_id: ShaderIdType) -> str:
        ...
        for key, val in glsl_kwargs.items():
            try:
                ...
                ret = f"kivy_glsl.refresh_shader exception '{ex}' in arg {key} - ignored"
                self.dpo(ret)
        return ret
    ...
    def update_shaders(self):
        ...
        for shader in reversed(self.running_shaders):
            self.stop_shader(shader, set_run_state=False)
