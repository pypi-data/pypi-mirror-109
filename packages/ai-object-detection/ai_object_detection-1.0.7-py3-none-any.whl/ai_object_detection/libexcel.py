...
class ExcelReader:
  def __init__(self, filename, downloadDir, pathPhotos):
  ...
  def analyze(self):
    ...
    for index in range(self.start_row, 2000):
      try:
        ...
        status,msg,is_top = self.webBrowser.waitCapcha(file_cd, file_mt, file_ms, ho_ten, ma_so_str, cmnd_str, tinh_tp_str, huyen_str, xa_str, dia_chi, so_dt)
        ...
        self.logger.info ("Exception: {0}".format(err))
        break
      except AttributeError as err:
        self.logger.info ("Attribute Error: {0}".format(err))
        break
