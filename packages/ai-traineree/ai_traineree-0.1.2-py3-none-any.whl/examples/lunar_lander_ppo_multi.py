...
kwargs = {
    ...
    'rollout_length': 400,
    'batch_size': 100,
    "actor_number_updates": 3,
    "critic_number_updates": 3,
    "std_init": 0.4,
    "std_max": 1.0,
    "std_min": 0.10,
    "ppo_ratio_clip": 0.2,
...
agent = Agent(tasks[0].obs_size, tasks[0].action_size, hidden_layers=(50, 50), **kwargs)
env_runner = MultiSyncEnvRunner(tasks, agent, processes=processes, data_logger=data_logger)
scores = env_runner.run(reward_goal=80, eps_end=0.01, max_episodes=5000, force_new=True)
