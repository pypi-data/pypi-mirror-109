...
def pcd_std_est(pcd):    
    ...
    # A. Create library
    C50_library = ctypes.CDLL(os.path.join(path,"libpcd_std_est_50c") + exten)

    # B. Specify function signatures
    C50_fxn = C50_library.pcd_std_est_50c
    C50_fxn.argtypes = (ctypes.POINTER(ctypes.c_float), ctypes.POINTER(ctypes.c_int),
                                   ctypes.c_int, ctypes.POINTER(ctypes.c_float), ctypes.POINTER(ctypes.c_float))

    # C. Invoke function
    num_point = len(pcd)
    num_k = 50
    idx = np.zeros((num_point, num_k), dtype=np.int64)
    kdt = KDTree(pcd, leaf_size=100, metric='euclidean')
    dist, idx = kdt.query(pcd, k=num_k)
    idx = np.int32(idx)

    if not idx.flags['F_CONTIGUOUS']:
        idx = np.asfortranarray(idx)

    p_idx = idx.ctypes.data_as(ctypes.POINTER(ctypes.c_int))
    ...
    C50_fxn(p_pcd, p_idx, num_point, p_sig_pcd, p_dens_pcd)

    #============= recompute with bigger K, if sigma_pcd is large =============
    if (sigma_pcd[0] * np.sqrt(dens_pcd[0]) > 1.5 and sigma_pcd[0] * np.sqrt(dens_pcd[0]) < 3.5):

        # A. Create library
        C200_library = ctypes.CDLL("libpcd_std_est_200c"+exten)

        # B. Specify function signatures
        C200_fxn = C200_library.pcd_std_est_200c
        C200_fxn.argtypes = (ctypes.POINTER(ctypes.c_float), ctypes.POINTER(ctypes.c_int),
                                ctypes.c_int, ctypes.POINTER(ctypes.c_float), ctypes.POINTER(ctypes.c_float))
        ...
        C200_fxn(p_pcd, p_idx, num_point, p_sig_pcd, p_dens_pcd)

    # ============= recompute with bigger K, if sigma_pcd is large =============
    if (sigma_pcd[0] * np.sqrt(dens_pcd[0]) >= 3.5 and sigma_pcd[0] * np.sqrt(dens_pcd[0]) < 4.5):

        # A. Create library
        C300_library = ctypes.CDLL("libpcd_std_est_300c"+exten)

        # B. Specify function signatures
        C300_fxn = C300_library.pcd_std_est_300c
        C300_fxn.argtypes = (ctypes.POINTER(ctypes.c_float), ctypes.POINTER(ctypes.c_int),
                                ctypes.c_int, ctypes.POINTER(ctypes.c_float), ctypes.POINTER(ctypes.c_float))
        ...
        C300_fxn(p_pcd, p_idx, num_point, p_sig_pcd, p_dens_pcd)

    # ============= recompute with bigger K, if sigma_pcd is large =============
    if (sigma_pcd[0] * np.sqrt(dens_pcd[0]) >= 4.5):

        # A. Create library
        C500_library = ctypes.CDLL("libpcd_std_est_500c"+exten)

        # B. Specify function signatures
        C500_fxn = C500_library.pcd_std_est_500c
        C500_fxn.argtypes = (ctypes.POINTER(ctypes.c_float), ctypes.POINTER(ctypes.c_int),
                                             ctypes.c_int, ctypes.POINTER(ctypes.c_float), ctypes.POINTER(ctypes.c_float))
        ...
        C500_fxn(p_pcd, p_idx, num_point, p_sig_pcd, p_dens_pcd)
