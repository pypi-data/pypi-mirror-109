...
# Default Ray version to return when a user either asks for `anyscale.connect required_ray_version`
...
class ClientBuilder:
    ...
    def connect(self) -> None:  # noqa
        ...
        if _allow_multiple_clients:
            if not self._cluster_name:
                ...
                self._log.info(
                    "`ANYSCALE_ALLOW_MULTIPLE_CLIENTS` is set, multiple clients will be allowed to connect to one session."
                )
    ...
    def _exec_self_in_local_docker(self) -> None:
        ...
        cur_file = os.path.abspath(sys.argv[0])
        docker_image = _get_base_image("ray-ml", DEFAULT_RAY_VERSION, "cpu")
        command = [
            "docker",
            "run",
            "--env",
...
if __name__ == "__main__":
    if len(sys.argv) > 1 and sys.argv[1] == "required_ray_version":
        print(f"ray=={DEFAULT_RAY_VERSION}")
    else:
