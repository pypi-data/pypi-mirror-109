...
class Event(ApexObject):
    ...
    def dates(self):
        ...
        for session in self.sessions:
            ...
            elif session.start_date.month == session.end_date.month:
    ...
    def get_comments(self) -> list:
        results = ApexRequest.search_all(f'templates/{self.id}/comments/'
                                         if self.is_template else f'events/{self.id}/comments/')
        return [Comment(item) for item in results]
    ...
    def is_bookable(self, portal_id: str = None) -> bool:
        ...
        if portal_id and self.portal_participants and str(portal_id) in self.portal_participants.keys():
            ...
            bookable = booked_participants < max_participants
        return bookable
