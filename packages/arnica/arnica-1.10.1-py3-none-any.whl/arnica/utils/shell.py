    ...
    *Parent class for computationnal shells*
    ...
    def rad_theta_components(self, vect_y, vect_z):
        ...
        returns the radial and azimuthal components of a vect y,z
         ...
         u_rad(n) = ( grid%w(3,n)*grid%x(2,n) + grid%w(4,n)*grid%x(3,n) ) / (grid%w(1,n) * radius(n))
      u_tan(n) = (-grid%w(3,n)*grid%x(3,n) + grid%w(4,n)*grid%x(2,n) ) / (grid%w(1,n) * radius(n)
           ...
           w4 w
             ...
             u_rad(n) = ( v*y + w*z ) /  r)
           u_tan(n) = (-v*z + x*y ) / (grid%w(1,n) * radius(n)
        """
