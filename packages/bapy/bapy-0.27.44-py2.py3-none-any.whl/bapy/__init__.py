...
STDOUT_DEFAULT = True
SUDO_DEFAULT = True
VENV_CLEAR_DEFAULT = False
VENV_DEPS_DEFAULT = True
    ...
    def __new__(cls, C):
        ...
        for item in inspect.classify_class_attrs(C):
            ...
            if kind is Kind.DATA:
                ...
                if i.coro:
                    kw['coro'].add(name)
            kw['attribute'][name] = Attribute(defining=item.defining_class, i=i, kind=kind, object=item.object,
                                              qualname=qualname)
        ...
        for name, f in fields.items():
            if name not in kw['attribute']:
                ...
                elif f.default is not DATA_MISSING:
                    kind = Kind.INITVAR
                    data = f.default
                else:
                    kind = Kind.UNKNOWN
                ...
                if f.init and name not in init_and_state:
                    kw['init'][name] = kw['data'][name]
                kw['attribute'][name] = Attribute(defining=defining, i=Is(kw['data'][name]), kind=kind, object=data,
                                                  qualname=name)
        # noinspection PyUnresolvedReferences
        for name, df in kw['signature'][C].items():
            if name not in kw['init']:
                ...
                kw['init'][name] = kw['data'][name]
                kw['attribute'][name] = Attribute(defining=C, i=Is(kw['data'][name]), kind=Kind.INITVAR,
                                                  object=kw['data'][name], qualname=name)
...
class Frame:
    ...
    def __init__(self, frame):
        ...
        if self.include:
            ...
            rv = None
            for i in FrameId:
                # TODO: ¿Por qué tenía esto para depurar?.
            ...
            # TODO: Tenia self.path.include !! No entiendo el int() - int() es que esa es la real.
...
class Is:
    ...
    """
    __slots__ = ('_cls', '_fields', '_func', '_iter', '_mro',
                 '_type', '_varstype', 'data',)
    ...
    def __call__(self):
        ...
        try:
            ...
            self._cls = value = self.data if self.type else self.data.__class__
            return value
    ...
    def type(self):
        try:
            ...
            self._type = value = isinstance(self.data, type)
            return value
...
class Obj(ObjBase):
    ...
    def hash(self, access=Access.PROTECTED, prop=True):
        ...
        Returns:
            Hash Dict.
        """
        ic(self.vars(access=access, ignore=lambda x, y: x in self.c.mroitems[N.IGNORE_HASH], prop=prop))
...
class Path(pathlib.Path, pathlib.PurePosixPath):
    ...
    def find_up(self, file=PIs.FILE, name=PSuffix.ENV, uppermost=False):
        ...
        >>> Path(email.mime.application.__file__).find_up(name=INIT_PY).path  # doctest: +ELLIPSIS
        Path('.../lib/python.../email/mime/__init__.py')
        >>> Path(email.mime.application.__file__).find_up(name=INIT_PY, uppermost=True).path  # doctest: +ELLIPSIS
    ...
    def path(self, uppermost=True):
        ...
        >>> import email.mime.application
        >>> from bapy import Path
        >>> Path(email.mime.application.__file__).path(uppermost=False)  # doctest: +ELLIPSIS
        Path('.../lib/python.../email/mime')
        >>> Path(email.mime.application.__file__).path()  # doctest: +ELLIPSIS
    ...
    def root(self, uppermost=True):
        ...
        >>> import email.mime.application
        >>> from bapy import Path
        >>> Path(email.mime.application.__file__).root(uppermost=False)  # doctest: +ELLIPSIS
        Path('.../lib/python.../email')
        >>> Path(email.mime.application.__file__).root()  # doctest: +ELLIPSIS
        Path('.../lib/python...')
    ...
    def to_modules(self, suffix=True, uppermost=True):
        ...
        >>> import email.mime.application
        >>> Path(email.mime.application.__file__).to_modules(uppermost=False)  # doctest: +ELLIPSIS
        'mime.application'
        >>> Path(email.mime.application.__file__).to_modules()  # doctest: +ELLIPSIS
        ...
        if (path and ((path := Path(path).resolved).exists() or (path := Path.cwd() / path).resolve().exists())) \
            ...
            with Path(path).cd:
                if path := subprocess.run(shlex.split(
                        self.value), capture_output=True, text=True).stdout.removesuffix('\n'):
                    return Path(path) if self is PathGit.PATH else furl.furl(path)
        return rv
