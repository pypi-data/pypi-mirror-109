...
`dip_external_id`)
        ...
        self._parser.add_argument("-a", "--activity_type", help="Destination repository",
                                  choices=['DRS', 'IA', 'BUDA', 'DEEP_ARCHIVE', 'ARCHIVE', 'SYNC',
                                           'ARCHIVE_REMOVED', 'GOOGLE_BOOKS'],
                                  required=True)

        self._parser.add_argument("-w", "--work_name", help="work being distributed", required=True)
        ...
        self._parser.add_argument("dip_source_path", help="Source path - string, (optional)", nargs='?')
        self._parser.add_argument("dip_dest_path", help="Destination path (optional) - string", nargs='?')
    ...
    def validate(self):
        ...
        if pa.end_time is not None and pa.begin_time is not None and pa.end_time < pa.begin_time:
            raise ValueError("end time before begin time")
...
def dip_log_shell() -> None:
    ...
    """
    dlp = DipLogParser("Logs a number of different publication strategies", "log_dip [OPTIONS] [dip_source_path] [dip_dest_path]  ")
