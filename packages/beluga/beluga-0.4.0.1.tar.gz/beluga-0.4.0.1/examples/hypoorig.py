...
rho_2 = 'rho0*exp(a1*h+a2*h**2+a3*h**3+a4*h**4+a5*h**5)'
rho_1 = 'rho0*exp(-h/H)'

rho = '(1.0)*({}) + (1.0-1.0)*({})'.format(rho_1, rho_2)
...
trajectory = sol_set[-1][-1]
u0=np.abs(trajectory.u[:,0])
u1=[trajectory.u[i,1] if trajectory.u[i,0]>0 else pi+trajectory.u[i,1] for i in range(len(trajectory.t))]

plt.figure(figsize=(12,6))
plt.subplot(121)
plt.plot(trajectory.t,trajectory.u[:,0],'r')
plt.plot(trajectory.t,u0,'bo-')
plt.subplot(122)
plt.plot(trajectory.t,trajectory.u[:,1],'r')
plt.plot(trajectory.t,u1,'bo-')
