...
def executeWinScp(winscpExe, keyFile, server, commandAry, showCmd=False):
...
def hold(msg=None, exitValue=None):
    msg = msg or "测试暂停，输入exit可以退出"
    exitValue = exitValue or "exit"
    while True:
        if getpass.getpass(f"{msg}：") == exitValue:
            break
