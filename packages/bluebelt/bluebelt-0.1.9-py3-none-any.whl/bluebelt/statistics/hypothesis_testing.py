...
class TwoSampleT():
    ...
    def __init__(self, frame, columns=None, related=False, confidence=0.95, alpha=0.05, **kwargs):
        ...
        self.h0 = bluebelt.helpers.text.get_h0_equal_means(self.frame.columns)
        self.calculate(**kwargs)
    ...
    def calculate(self, **kwargs):
        ...
        self.ci_mean_a = ci.ci_mean(self.array_a)
        self.ci_mean_b = ci.ci_mean(self.array_b)
...
def _equal_means_plot(plot_obj, **kwargs):
    ...
    for row in range(0, plot_obj.ncols):
        ...
        # 2. CI for the mean #####################################
        ci_mean = ci.ci_mean(array, confidence=plot_obj.confidence)
        ...
        # plot CI values
        ci_digits_str = bluebelt.helpers.text.format_digits([ci_mean[0], ci_mean[1]], digits=digits)
        ...
        # set xticks
        local_xticks = bluebelt.helpers.boxplot.get_boxplot_quantiles(array, whis=1.5)
    ...
    for row in range(0, plot_obj.ncols):
        ...
        ax.xaxis.set_major_formatter(FormatStrFormatter(f'%{bluebelt.helpers.text.format_digits(xticks, digits=digits)}'))
        ax.set_xlim(xlims)

    # add suptitle
    fig.suptitle(t=f'{plot_obj.test} for {bluebelt.helpers.text.get_nice_list(plot_obj.frame.columns)}', x=fig.subplotpars.left, **style.equal_means_plot.suptitle)
...
def _equal_variance_plot(plot_obj, **kwargs):
    ...
    # add suptitle
    fig.suptitle(t=f'{plot_obj.test} for {bluebelt.helpers.text.get_nice_list(plot_obj.frame.columns)}', x=fig.subplotpars.left, **style.equal_variance_plot.suptitle)
...
def _distribution_plot(plot_obj, **kwargs):
    ...
    for row in range(0, nrows):
        for col in range(0, ncols):
            ...
            if id < plot_obj.ncols:
                ...
                if id == 0:
                    ax.set_title(f'{plot_obj.test} for {bluebelt.helpers.text.get_distribution(plot_obj.dist)}', **style.distribution_plot.title)
                ax.set_xticks([])
                ax.set_yticks([])
    ...
    # add suptitle
    fig.suptitle(t=f'Probability plot of {bluebelt.helpers.text.get_nice_list(plot_obj.frame.columns)}', x=fig.subplotpars.left, **style.distribution_plot.suptitle)
