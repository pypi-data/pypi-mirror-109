...
class multiclass_lib:
    ...
    def f_importance(self, key_list,bar_type=None):
        multiclass_f_importance(self.parent, key_list,bar_type)
...
def multiclass_f_importance(C_mod, key_list, bar_type = None):
    ...
    if bar_type == None:
        ...
        for iii in range(le_ke):
            ax = fig.add_subplot(le_ke, 1, iii+1, polar=True)
            l_mod = C_mod.get_model(key_list[iii])
            lbiii = labels[iii]
            fe_i = pd.DataFrame(l_mod.feature_importances_, index=C_mod.get_input().columns, columns=[lbiii])
            fe_i = fe_i.sort_values(lbiii, ascending=False)
            labels_b = fe_i.index
            left = np.array([ii + 1 for ii in range(labels)])
            height = fe_i[lbiii]
        ...
        fe_i = pd.DataFrame(l_mod.feature_importances_, index=C_mod.get_input().columns, columns=[labels[0]])
        for iii in range(1,le_ke):
            l_mod = C_mod.get_model(key_list[iii])
            lbiii = labels[iii]
            fe_i[lbiii] = l_mod.feature_importances_

        fe_i["all_model_sum_akods"] = fe_i.sum(axis=1)
        fe_i = fe_i.sort_values("all_model_akods", ascending=False)
        labels_b = fe_i.index

        if bar_type == "overlaid":
            left = np.array([ii + 1 for ii in range(labels)])
            sum_heig = [0 for jjj in range(labels)]
            for iii in range(le_ke):
                ax.barh(left, fe_i[iii],bottom=sum_heig , color=color_list[iii], align="center", label=labels[iii])
                sum_heig += fe_i
            ax.set_xlabel("importance")
            ax.set_ylabel("labels")
            ...
            left = np.array([ii + 1 for ii in range(labels)])
            for iii in range(le_ke):
                ax.barh(left + b_width*iii, fe_i[iii], width = b_width, color=color_list[iii], align="center", label=labels[iii])
            ax.set_xlabel("importance")
            ax.set_ylabel("labels")
