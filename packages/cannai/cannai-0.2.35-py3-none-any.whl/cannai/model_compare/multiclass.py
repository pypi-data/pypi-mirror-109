...
def multiclass_f_importance(C_mod, key_list, bar_type = None):
    ...
    if bar_type == None:
        ...
        if bar_type == "overlaid":
            ...
            for iii in range(le_ke):
                ax.barh(left, fe_i,bottom=sum_heig , color=color_list[iii], align="center", label=labels_b[iii])
                sum_heig += fe_i
            ...
            left = np.array([ii + 1 for ii in range(le_la)])
            for iii in range(le_ke):
                ax.barh(left + b_width*iii, fe_i[iii], width = b_width, color=color_list[iii], align="center", label=labels_b[iii])
            ax.set_xlabel("importance")
            ax.set_ylabel("labels")
