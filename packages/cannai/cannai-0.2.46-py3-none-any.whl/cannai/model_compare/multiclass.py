...
def multiclass_f_importance(C_mod, key_list, bar_type = None):
    ...
    if bar_type == None:
        ...
        if bar_type == "overlaid":
            ...
            for iii in range(le_ke):
                height = sklearn.preprocessing.minmax_scale(fe_i.iloc[:,iii])
                ax.bar(left, height,bottom=sum_heig , color=color_list[iii], align="center", label=labels_b[iii])
            ...
            for iii in range(le_ke):
                height = sklearn.preprocessing.minmax_scale(fe_i.iloc[:, iii])
                ax.bar(left + b_width*iii, height, width = b_width, color=color_list[iii], align="center", label=labels_b[iii])
            ax.set_ylabel("importance")
            ax.set_xlabel("labels")
