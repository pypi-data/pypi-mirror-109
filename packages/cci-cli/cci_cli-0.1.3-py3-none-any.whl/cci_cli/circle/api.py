...
from typing import List, BinaryIO, Dict, Optional, Iterable
...
class CircleCI:
    ...
    def _get_request(self, url: str, query_params: Optional[dict] = None):
    ...
    def _get_paginated_items(
        self, url: str, query_params: Optional[dict] = None, limit: Optional[int] = None
    ) -> Iterable[dict]:
        """
        Get enough pages to have at least `limit` results.

        The Circle CI API gets a fixed number of results per page, and gives a "next page token" to get more results.
        This returns an iterator that keeps requesting pages until we have at least `limit` results
        and yields them page by page.
        """
        query_params = query_params or dict()  # We'll need to use it for the page token
        limit = limit or 1  # By default we make a single request and stop
        count_items = 0
        while count_items < limit:
            response = self._get_request(url, query_params=dict(query_params))
            page_items = response.get("items", list())
            if not page_items:
                break
            query_params["page-token"] = response.get("next_page_token")
            count_items += len(page_items)
            yield from iter(page_items)
    ...
    def list_pipelines(
        ...
        after_pipeline_id: Optional[str] = None,
        ...
        output = []
        for pipeline in self._get_paginated_items(
            url, query_params=query_params, limit=limit
        ):
    ...
    def get_workflows_with_urls(self, pipeline_id: str) -> List:
        items = self.get_workflows(pipeline_id)
        for item in items:
            assert "url" not in item, "URL not expected in response"
            item["url"] = self.get_workflow_url(item)
        return items
    ...
    def cancel_job(self, project_name: str, job_number: int) -> Dict:
        url = f"{self.base_url}/{self.project_slug}/{project_name}/job/{str(job_number)}/cancel"
        return self._post_request(url=url, params={})
    ...
    def wait_for_workflows_creation(self, pipeline_id: str, timeout: int):
        ...
        while time.time() <= timeout and len(workflows) == 0:
            workflows = self.get_workflows_with_urls(pipeline_id)
            if len(workflows) > 0:
                break
            utils.show_progress()
            time.sleep(2)
