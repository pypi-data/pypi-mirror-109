...
class ApiClient:
    ...
    def make_request(
        ...
        print "read_conductor_credentials(use_api_key)", use_api_key
...
def read_conductor_credentials(use_api_key=False):
    ...
    if (
        ...
        or not CONFIG.get("api_key", {}).get("client_id")
...
def get_bearer_token(refresh=False):
    """
    Return the bearer token from a cached(global) variable.  If there is no cached value, then fetch
    a new bearer token and return it (and cache it).
...
def creds_same_domain(creds):
    ...
    return (
        audience_domain
        and audience_domain.rpartition("/")[-1] == CONFIG.get("api_url").rpartition("/")[-1]
    )
...
def request_instance_types(as_dict=False):
    ...
    # [
    #    {cores: 16, description: "16 core, 64GB Mem", name: "m5.4xlarge", memory: 64.0} {cores: 48,
    #    description: "48 core, 192GB Mem", name: "m5.12xlarge", memory: 192.0}
    # ]
