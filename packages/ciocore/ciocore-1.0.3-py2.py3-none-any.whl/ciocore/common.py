...
def base_dir():
    """
    Return the top level directory for the local Conductor repo. This is derived by traversing up
    directories from this current script.

    Note that due to symkinks, we can't use os.path.realpath on __file__ because __file__ may be a
    symlinked path and would return the directory for the "real" file (as opposed to the directory
    of the symlinked file (__file__))
    """
    module_filepath = __file__
    if module_filepath.startswith(".%s" % os.sep):
        # the module filepath will somtimes be relative (to the current working directory),
        # reconstruct full path if necessary
        module_filepath = module_filepath.replace(".%s" % os.sep, "%s%s" % (os.getcwd(), os.sep))
...
def _cast_env_var(env_var, default=None):
    """
    Read the given value (which was read from an environment variable, and process it onto an
    appropriate value for the config.yml file.
...
class Config:
    def __init__(self):
        ...
        project_url = os.environ.get(
            "CONDUCTOR_PROJECT_URL", "https://atomic-light-001.appspot.com"
        )
    ...
    def get_api_key_from_variable():
        ...
        try:
            ...
            try:
                ...
                message = (
                    "An error occurred reading the API key from the CONDUCTOR_API_KEY variable"
                )
