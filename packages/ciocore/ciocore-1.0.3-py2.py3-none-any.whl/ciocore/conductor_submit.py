...
class Submit(object):
    ...
    def consume_args(self, args):
        """
         "Consume" the give arguments (which have been parsed by argparse). For arguments that have
         not been specified by the user via the command line, default to reasonable values. If
         inherit_config is True, then default unspecified arguments to those values found in the
         config.yml file.

         1. Use the argument provided by the user via the command line.  Note that this may be a
            non-True value, such as False or 0.  So the only way to know whether the argument was
            provided by the user is to check whether is value is not None (which implies that
            argparse arguments should not be configured to give a default value when the user has not
            specified them).

        2. If the user has not provided the argument, and if inherit_config is True, then use the
           value found in the config.

        3. If inherit_config is False or the the value in the config doesn't exist, then default to
           an empty or non-True value.
        ...
        self.software_package_ids = self.resolve_arg(
            args, "software_package_ids", [], combine_config=True
        )
    ...
    def resolve_arg(cls, args, arg_name, default, combine_config=False):
        ...
        The general premise is as follows: If an argument value was provided then use it.  However,
        if the combine_config, bool is True, then we want to combine that value with the value found
        in the config.  This only works with argument types which can be combined, such as lists or
        dictionaries. In terms of which value trumps the other, the argument value will always trump
        the config value. For example, if the config declared a dictionary value, and the argument
        also provided a dictionary value, they would both be combined, however, the argument value
        will replace any keys that clash with keys from the config value.

        If the argument nor the config have a value, then use the provided default value


        combine_config: bool. Indicates whether to combine the argument value with the config value.
                        This will only ever occur if both values are present.

        The order of resolution is:
        1. Check whether the user explicitly specified the argument when calling/ instantiating the
           class.  This is indicated by whether the argument's value is set to None or not. If the
           value is not None, then use it (combining it with the config value if appropriate)

        2. If the specified argument does not have value (i.e. it's None), then use the value found
           in the config.

        3. If the config does not define the expected argument then use the default argument

        """

        # Map a combine operation by type
        combine_op = {dict: dict.update, list: list.extend}
        ...
        if arg_name in CONFIG:
            ...
            # If the arg value is  None, it indicates that the arg was not explicity set. And if the
            # config lists a value,then use it. Note that it's valid that the config may state a
            # non-True value (i.e. 0, False, etc)
            if arg_value is None:
                return config_value

            # IF we're here, then it means that the arg_Value is not None, so simple check whehter
            # we're combining its value with the the config value.  If not, simply return the
            # arg_value
            if not combine_config:
                return arg_value

            # If we're here, then it means that the arg_value is not None, and that we're combining
            # that value with the value found in the config. Get the combine operation (update if
            # dict, or extend if list)
            ...
            assert type(config_value) == type(
                arg_value
            ), "Cannot combine differing data types: %s, %s" % (config_value, arg_value)

            # because we're updating a list or dict from the config object, it will mutate. Make a
            # copy first
    ...
    def validate_args(self):
        ...
        # If a command was provided, make sure that it provies a frames argument. Also make sure
        # that it doesn't also provide a tasks_data arg.
        if self.command:
            logger.warning('"command" argument deprecated. Please use "tasks_data" arg')
            if not self.frames:
                raise BadArgumentError(
                    'Must provide a "frames" argument when using the "command" argument'
                )
            if not self.output_path:
                raise BadArgumentError(
                    'Must provide a "output_path" argument when using the "command" argument'
                )
            if self.tasks_data:
                raise BadArgumentError(
                    'Cannot supply both the "command" and "tasks_data" arguments'
                )
            if self.upload_only:
                raise BadArgumentError(
                    'Cannot supply both the "command" and "upload_only" arguments'
                )
        ...
        # If the tasks_data arg is provided,
            ...
            if not self.output_path:
                raise BadArgumentError(
                    'Must provide a "output_path" argument when using the "tasks_data" argument'
                )

            # make sure upload_only isn't also specified
            if self.upload_only:
                raise BadArgumentError(
                    'Cannot supply both "tasks_data" and "upload_only" arguments'
                )
            ...
            for task_data in self.tasks_data:
                if "command" not in task_data:
                    raise BadArgumentError(
                        'entry in "tasks_data" argument must specify a "command" key. Got %r'
                        % task_data
                    )
        ...
        # if the upload_only arg is provided, ensure that there are provided files for upload
            ...
            if not (self.upload_file or self.upload_paths):
                raise BadArgumentError(
                    '"upload_only" job must provide either "upload_paths" and/or "upload_file"'
                )

        else:
            raise BadArgumentError(
                'Job must provide a "command" or "tasks_data" or indicate that it\'s "upload_only"'
            )
        ...
        if self.instance_type is not None and self.machine_flavor is not None:
            raise BadArgumentError(
                'Cannot supply both the "instance_type" and "machine_type" arguments. Choose one.'
            )

        if self.instance_type is not None and self.cores is not None:
            raise BadArgumentError(
                'Cannot supply both the "instance_type" and "cores" arguments. Choose one.'
            )
        ...
        if self.machine_flavor is not None:
            ...
            if self.machine_flavor not in supported_machine_flavors:
                raise BadArgumentError(
                    "Machine type %r is not one of %s"
                    % (self.machine_flavor, supported_machine_flavors)
                )
        ...
        if self.gpu_config:
            ...
            if self.gpu_config.get("type") not in supported_gpu_types:
                raise BadArgumentError(
                    "GPU type %s is not one of %s"
                    % (self.gpu_config.get("type"), supported_gpu_types)
                )

    def send_job(self, upload_files, upload_size):
        """
        Construct args for two different cases: - upload_only - running an actual command (cmd)
        ...
        if self.upload_only:
            # If there are not files to upload, then simulate a response string and return a "no
            # work to do" message
            if not upload_files:
            ...
            submit_dict.update(
                {"frame_range": "x", "command": "upload only", "instance_type": "n1-standard-1"}
            )
        else:
        ...
        response, response_code = self.api_client.make_request(
            uri_path="jobs/", data=json.dumps(submit_dict), raise_on_error=False, use_api_key=True
        )
    ...
    def main(self):
        ...
        In order to give flexibility to customers (because a customer may consist of a single user
        or a large team of users), there are two options avaiable that can dicate how these job
        submission stages are executed. In a simple single-user case, a user's machine can doe all
        three of the job submission stages.  Simple.  We call this local_upload=True.

        However, when there are multiple users, there may be a desire to funnel the "heavier" job
        submission duties (such as md5 checking and dependency uploading) onto one dedicated machine
        (so as not to bog down the artist's machine). This is called local_upload=False. This
        results in stage 1 being performed on the artist's machine (dependency gathering), while
        handing over stage 2 and 3 to an uploader daemon.  This is achieved by the artist submitting
        a "partial" job, which only lists the dependencies that it requires (omitting the md5
        hashes). In turn, the uploader daemon listens for these partial jobs, and acts upon them, by
        reading each job's listed dependencies (the filepaths that were recorded during the
        "partial" job submission).  The uploader then  md5 checks each dependency file from it's
        local disk, then uploads the file to cloud storage (if necesseary), and finally "completes"
        the partial job submission by providing the full mapping dictionary of each dependency
        filepath and it's corresponing md5 hash. Once the job submission is completed, conductor can
        start acting on the job (executing tasks, etc).
        ...
        # If opting to upload locally (i.e. from this machine) then run the uploader now This will
        # do all of the md5 hashing and uploading files to the conductor (if necesary).
        ...
        # If the NOT uploading locally (i.e. offloading the work to the uploader daemon
            ...
            # update the upload_files dictionary with md5s that should be enforced this will
            # override the None values with actual md5 hashes
        ...
        # Any md5s that are incuded, are expected to be checked against if/when the uploader daemon
        # goes to upload them. If they do not match what is on disk, the uploader will fail the job
    ...
    def get_upload_files(self):
        ...
        """
        # begin a list of "raw" filepaths that will need to be processed (starting with the
        # self.upload_paths)
    ...
    def cast_metadata(cls, metadata, strict=False):
        """
        Ensure that the data types in the given metadata are of the proper type (str or unicode). If
        strict is False, automatically cast (and warn) any values which do not conform.  If strict
        is True, do not cast values, simply raise an exception.
        """
        ...
        error_msg = (
            "Metadata %%s %%s is not of a supported type. Got %%s. Expected %s"
            % " or ".join([type_.__name__ for type_ in cls.metadata_types])
        )
        ...
        for key, value in metadata.iteritems():
            ...
            # This should never happen, but need to make sure that the casting process doesn't cause
            # the original keys to collide with one another
