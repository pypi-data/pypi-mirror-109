...
class Uploader(object):
    ...
    def _create_workers(self, start=True):
        ...
        WORKERS: - DownloadWorker # Downloads files - HistoryWorker  # logs out history of
            downloaded files
        """
        ...
        for _ in range(thread_count):
            ...
            global RUN_STATE
            wrk = UploaderWorker(
                RUN_STATE, self._results_queue, account=account, project=project, location=location
            )
...
class UploaderWorker(multiprocessing.Process):
    ...
    It will then create an FileGenerator() object from the filepath, registering the
    self.event_handler() method as the callback to be fired for each chunk uploded to GCS.
    ...
    def __init__(self, run_state, results_queue, account=None, location=None, project=None):
    ...
    def run(self):
        ...
        while self._run_state.value == Uploader.STATE_RUNNING:
            try:
                ...
                self.log("Preventing process from exiting due to Exception:\n", level=EXCEPTION)
    ...
    def maybe_upload(self):
        ...
        if local_md5 != expected_md5:
            # error
            raise exceptions.UploaderFileModified(
                "different md5 - local: %s expected: %s" % (local_md5, expected_md5)
            )
    ...
    def file_md5(self, filepath):
        ...
        with open(filepath, "rb") as fileobj:
            while self._run_state.value == Uploader.STATE_RUNNING:
                ...
                if self.maybe_touch():
                    self.touch()
                    Backend.touch(self.current_upload, bytes_downloaded=0, location=self.location)
                chunk_count += 1
...
class FileGenerator(object):
    """
    Since requests.put() can take a generator as the data param in order to allow for streaming
    uploads, this class can be used to create a generator from a filepath. Optionally, it can take a
    function/callable class instance as an event handler. The handler will be called when each chunk
    of the file is read.
    """
...
class Backend:
    ...
    def finish(cls, upload, location=None, bytes_downloaded=0):
        ...
        try:
            ...
            if err.response.status_code == 410:
                LOGGER.warning("Cannot finish file %s.  File not active (410)", upload["id"])
            raise err
        except:
            raise
    ...
    def fail(cls, upload, location=None, bytes_downloaded=0):
        ...
        try:
            ...
            if err.response.status_code == 410:
                LOGGER.warning("Cannot fail file %s.  File not active (410)", upload["id"])
            raise err
        except:
            raise
...
def run_uploader(args):
    """
    Start the uploader process. This process will run indefinitely, polling the Conductor cloud app
...
def resolve_args(args):
    """
    Resolve all arguments, reconsiling differences between command line args and config.yml args.
...
def resolve_arg(arg_name, args, config):
    """
    Helper function to resolve the value of an argument. The order of resolution is:

    1. Check whether the user explicitly specified the argument when calling/ instantiating the
       class. If so, then use it, otherwise...
    2. Attempt to read it from the config.yml. Note that the config also queries environment
       variables to populate itself with values. If the value is in the config then use it,
       otherwise...
    3. return None
    ...
    # If the arg is not None, it indicates that the arg was explicity specified by the caller/user,
    # and it's value should be used
