...
class AEconductorRenderTemplate(pm.ui.AETemplate):
    ...
    def update_autosave_cleanup(self, nodeName):
        autosave = pm.PyNode(nodeName).attr("autosave").get()
        self.dimControl(nodeName, "cleanupAutosave", not autosave)
        self.dimControl(nodeName, "autosaveTemplate", not autosave)
        if not autosave:
            return
        ...
        if not autosave_template:
            return

        if autosave_template and autosave_template.lower().strip() == "<scene>":
            self.dimControl(nodeName, "cleanupAutosave", True)
            pm.displayWarning(
                "Disabling 'cleanupAutosave'. You cannot use cleanupAutosave when either the autosave template would overwrite your current scene, or when using the upload daemon."
            )
        else:
            self.dimControl(nodeName, "cleanupAutosave", False)
