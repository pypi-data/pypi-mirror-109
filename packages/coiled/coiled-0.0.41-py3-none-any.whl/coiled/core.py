...
class Cloud(Generic[IsAsynchronous]):
    ...
    async def _create_cluster(
        ...
        *,
        ...
        await self._websocket_stream(ws, log_output, use_spinner=False)
    ...
    async def _start_job(
        ...
        await self._websocket_stream(ws, log_output)
    ...
    async def _set_backend_options(
        ...
        name = self.user
        ws_server = self.server.replace("http", "ws", 1)
        ws_endpoint = f"{ws_server}/ws/api/v1/{account}/cluster-info/"
        try:
            ws = await session.ws_connect(ws_endpoint)
        except aiohttp.WSServerHandshakeError:
            raise ValueError(
        ...
        initial_message = {"type": "initial_load_and_register"}

        await ws.send_json(initial_message)
        error_details = await _backend_options_ws(ws, log_output)
        if error_details:
            raise ValueError(f"Unable to set backend_options: {error_details}")
    ...
    async def _websocket_stream(
        self, ws: aiohttp.ClientWebSocketResponse, log_output, use_spinner=True
    ):
        error_details = None
        with Spinner() if use_spinner else contextlib.suppress():
            async for msg in ws:
                if msg.type == aiohttp.WSMsgType.TEXT:
                    ...
                    print("closing connection", file=log_output)
                    await ws.close()
        ...
        # I have yet to see an actual close message above (WSMsgType.CLOSE). It
        # seems like the useful information about a close event is held in these
        # two properties.
        ws_exception = ws.exception()
        close_code: Optional[int] = ws.close_code
        is_bad_close_code = close_code and 1000 < close_code < 3000

        if error_details:
            # Just print the ws_exception and close code, raise the error_details
            if ws_exception:
                print(f"Websocket exception: {ws_exception}", file=log_output)
            print(f"Websocket close code: {close_code}", file=log_output)
...
def set_backend_options(
    ...
    if use_coiled_defaults is True:
        ...
        if create_vpc is True:
            raise NotImplementedError("VPC for GCP is not yet implemented")
