...
@pytest.mark.xpass(
    strict=True, reason="Failing in CI: https://github.com/coiled/cloud/issues/2858"
)
@pytest.mark.asyncio
...
@pytest.mark.xfail(
    strict=True, reason="AWS VM self-hosted is not yet in the test settings"
)
@pytest.mark.test_group("core-slow-group-12")
def test_set_backend_options_aws_vm_self_hosted(cloud, sample_user, mocker):

    mocker.patch("coiled.utils.boto3")
    account = sample_user.account
    from pprint import pprint as print

    print(settings.ENABLED_CLUSTER_BACKENDS)
...
@pytest.mark.test_group("core-slow-group-12")
def test_set_backend_options_gcp_vm(cloud, sample_user):
    ...
    # set no region and try to create VPC
    with pytest.raises(NotImplementedError) as e_info:
        coiled.set_backend_options(
            backend_type=types.BackendChoices.VM_GCP, create_vpc=True
        )

    assert "VPC for GCP is not yet implemented" in str(e_info)
