...
for i in range (0,len(snr)):
  ...
  for j in range(0,len(noise)):
    demod[2*j] = (noise[j].real<0)*1
    demod[2*j+1] = (noise[j].imag<0)*1
    a = np.array(demod[2*j])
    b = np.array(demod[2*j+1])
    demodd = np.array((a,b)) 
    demodqpsk = np.append(demodqpsk, demodd)
  #np.reshape(demodd, -1, order='F')
...
for i in range (0,len(snr)):
  def dqpsk(noise): 
    for j in range(0,len(noise)):
      ...
      b = np.array(demod[2*j+1])
      demodd = np.array((a,b)) 
      demodqpskk =  np.reshape(demodd, -1, order='F')
    return dqpsk
    def dbpsk(noise):
  demod = (noise.real>0)*1
  return demod
...
demodxx = dqpsk(noise)
print('demodulasi= ', demodxx) 


for i in range (0,len(snr)):
  def BER(demodulasi): 
    ber[i] = sum((demodulasi!=bit)*1)/nbit
    demodulasi = np.zeros(0)
    return ber
biterror = BER(demodxx)
print('ber = ', ber)
