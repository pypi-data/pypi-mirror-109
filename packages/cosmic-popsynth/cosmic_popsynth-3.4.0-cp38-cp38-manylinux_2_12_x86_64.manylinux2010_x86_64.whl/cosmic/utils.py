...
def error_check(BSEDict, filters=None, convergence=None, sampling=None):
    ...
    if sampling is not None:
        ...
        if ("qmin" not in list(sampling.keys())) & ("m2_min" not in list(sampling.keys())) & (sampling["sampling_method"] == 'independent'):
    ...
    if convergence is not None:
        ...
        for key in list(convergence[flag].keys()):
...
def convert_kstar_evol_type(bpp):
    ...
    evolve_type_string_to_int_dict = {
        v: k for k, v in list(evolve_type_int_to_string_dict.items())
    }
...
def parse_inifile(inifile):
    ...
    for section in cp.sections():
        ...
        if section == "cmc":
            if "rand_seed" not in list(dictionary.keys()):
                dictionary["rand_seed"] = {}
                dictionary["rand_seed"]["seed"] = 0
            if "filters" not in list(dictionary.keys()):
                dictionary["filters"] = 0
            if "convergence" not in list(dictionary.keys()):
                dictionary["convergence"] = 0
            if "sampling" not in list(dictionary.keys()):
        ...
        for option in cp.options(section):
            ...
            try:
                ...
                if option not in list(dictionary[section].keys()):
