...
class MainWindow(QtG.QMainWindow):
            ...
            if self.is_folder_3D_tomo is True:
                if file_type == "CBOX" and self.picking_filament is False:
                    ...
                    if file_type == "CBOX" and self.picking_filament is True:
                        ...
                        # i have to convert the data in single box filament (filament (start-end)) for saving as filament on file
                        if self.use_circle_combobox.currentText() != helper.RECT_FILAMENT_START_END:
                            self.box_dictionary_to_save=helper.convert_filament_start_end(filament_dict=self.picked_filament_dictionary, is_tomo_folder=self.is_folder_3D_tomo)
    ...
    def _write_all_type_optimization(self, box_dir, selected_slice):
        ...
        if self.box_dictionary_3D_view:     # if i do not trace it is empty
    ...
    def write_all_type(self):
        ...
        if result:
            if self.is_cbox_untraced and not self.has_filament: # not self.has_filament because in this version we cannot trace in filament case
    ...
    def _apply_3d_visualization_in_tomo_case(self):
        ...
        else:   # it is not 3D case
            self.picking_filament_combox.setCurrentText(DISABLED)
            self.blur_picking_filament(set_enabled=False)  # if we load after picking it is blurred
    ...
    def _import_boxes_3D_tomo_folder(self, box_dir, keep=False):
        ...
        if self.is_cbox_untraced and not self.has_filament: # not available in filament mode
