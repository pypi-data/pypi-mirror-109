        ...
        for col, value in dataset.isnull().sum().items():
            if value > 0:
                if col in nominal_cols + ordinal_cols:
                    ...
                    dataset[col] = dataset[col].fillna(mode_val, inplace=True)
            ...
            if value > 0:
                if col in nominal_cols + ordinal_cols:
                    ...
                    if np.abs(scipy.stats.skew(dataset[col])) > 1:
                        ...
                        dataset[col] = dataset[col].fillna(median_val, inplace=True)
                    else:
                        print("Replaced nulls in " + col + " with mean")
                        mean_val = dataset[col].mean()
                        dataset[col] = dataset[col].fillna(mean_val, inplace=True)
        return dataset
    else:
...
def nominal_scale_corr(nominal_series, continuous_series):
    ...
    continuous_series = continuous_series.fillna(mean_val, inplace=True)
