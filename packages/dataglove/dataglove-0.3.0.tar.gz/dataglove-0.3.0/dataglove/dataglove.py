...
def __isUsingNewHapticSystemFirmware(dataGloveIO):
	samdVersion = [int(x) for x in Forte_GetSAMDAppletVersion(dataGloveIO).split(".")]
	if (samdVersion[0] >= 5 and samdVersion[1] >= 1):
		return True
	return False
...
def Forte_SendOneShotHaptic(dataGloveIO, actuatorID, note, amplitude):
	__connectionException(dataGloveIO)
	if __isUsingNewHapticSystemFirmware(dataGloveIO):
...
def Forte_SendLoopHaptic(dataGloveIO, actuatorID, note, amplitude):
	__connectionException(dataGloveIO)
	if __isUsingNewHapticSystemFirmware(dataGloveIO):
		return __func(__lib.Forte_SendLoopHaptic, None, [c_void_p, c_byte, c_byte, c_float])(dataGloveIO, actuatorID, note, amplitude)
	else:
		return __func(__lib.Forte_SendLoopHaptic_OLD, None, [c_void_p, c_byte, c_byte, c_float])(dataGloveIO, actuatorID, note, amplitude)

def Forte_ToggleOneShot(dataGloveIO, actuatorID, makeOneShot):
	__connectionException(dataGloveIO)
	return __func(__lib.Forte_ToggleOneShot_OLD, None, [c_void_p, c_byte, c_byte])(dataGloveIO, actuatorID, makeOneShot)

def Forte_SelectHapticWave(dataGloveIO, actuatorID, waveform, soundChannel=0):
	__connectionException(dataGloveIO)
	if __isUsingNewHapticSystemFirmware(dataGloveIO):
		return __func(__lib.Forte_SelectHapticWave, None, [c_void_p, c_byte, c_byte, c_byte])(dataGloveIO, actuatorID, soundChannel, waveform)
	else:
		return __func(__lib.Forte_SendLoopHaptic_OLD, None, [c_void_p, c_byte, c_byte, c_float])(dataGloveIO, actuatorID, note, amplitude)

def Forte_SetGrainLocation(dataGloveIO, actuatorID, location, soundChannel=0):
	__connectionException(dataGloveIO)
	if __isUsingNewHapticSystemFirmware(dataGloveIO):
		return __func(__lib.Forte_SetGrainLocation, None, [c_void_p, c_byte, c_byte, c_float])(dataGloveIO, actuatorID, soundChannel, ocation)
	else:
...
def Forte_SetPitchBend(dataGloveIO, actuatorID, pitch, soundChannel=0):
	__connectionException(dataGloveIO)
	if __isUsingNewHapticSystemFirmware(dataGloveIO):
		return __func(__lib.Forte_SetPitchBend, None, [c_void_p, c_byte, c_byte, c_byte])(dataGloveIO, actuatorID, soundChannel, pitch)
	else:
		return __func(__lib.Forte_SetPitchBend_OLD, None, [c_void_p, c_byte, c_byte])(dataGloveIO, actuatorID, pitch)

def Forte_SetGrainSize(dataGloveIO, actuatorID, size, soundChannel=0):
	__connectionException(dataGloveIO)
	if __isUsingNewHapticSystemFirmware(dataGloveIO):
		return __func(__lib.Forte_SetGrainSize, None, [c_void_p, c_byte, c_byte, c_float])(dataGloveIO, actuatorID, soundChannel, size)
	else:
		return __func(__lib.Forte_SetGrainSize_OLD, None, [c_void_p, c_byte, c_float])(dataGloveIO, actuatorID, size)

def Forte_SetGrainFade(dataGloveIO, actuatorID, fade, soundChannel=0):
	__connectionException(dataGloveIO)
	if __isUsingNewHapticSystemFirmware(dataGloveIO):
		return __func(__lib.Forte_SetGrainFade, None, [c_void_p, c_byte, c_byte, c_float])(dataGloveIO, actuatorID, soundChannel, fade)
	else:
