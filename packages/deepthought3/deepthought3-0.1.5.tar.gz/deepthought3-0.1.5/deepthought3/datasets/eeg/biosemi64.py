...
from deepthought3.datasets.eeg.channel_layout import ChannelLayout, XYPlotChannelLayout
...
class Biosemi64Layout(ChannelLayout):
    ...
    def as_montage(self):
        ...
        pos = self.xyz_coords()
        pos = pos[:, [1, 0, 2]]  # swam x and y
        pos *= [1, -1, 1]  # flip y
        ch_pos = {n: p for (n, p) in zip(self.channel_names(), pos)}
        standard_montage_positions = make_standard_montage("biosemi64").get_positions()

        montage = make_dig_montage(
            ch_pos=ch_pos,
            nasion=standard_montage_positions["nasion"],
            lpa=standard_montage_positions["lpa"],
            rpa=standard_montage_positions["rpa"],
            hsp=standard_montage_positions["hsp"],
            hpi=standard_montage_positions["hpi"],
        )

        # return make_standard_montage("biosemi64")
        # return make_dig_montage(ch_pos=ch_pos)
        return montage
