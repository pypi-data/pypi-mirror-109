...
class AbstractGraphQLTest(GraphQLTestCase):
    ...
    def check_query(self, query: Union[str, Dict[str, any]], arguments: Dict[str, any] = None) -> Dict[str, any]:
        ...
        if arguments is None:
            arguments = {}
        executed = self.query(query, variables=arguments)
    ...
    def check_mutation(self, mutation: Union[str, Dict[str, any]], arguments: Dict[str, any] = None) -> Dict[str, any]:
        ...
        if arguments is None:
            arguments = {}
        executed = self.query(mutation, input_data=arguments)
        assert 'data' in executed, "GraphQL output should return 'data', but this call did not."
        assert 'errors' not in executed, f"Got graphQL errors: {executed}"
        assert executed['data'] is not None, f"data paylaod is None. Output={executed}"
        return executed['data']
