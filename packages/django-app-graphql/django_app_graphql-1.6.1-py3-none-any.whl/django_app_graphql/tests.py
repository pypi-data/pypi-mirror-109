...
class AbstractGraphQLTest(GraphQLTestCase):
    ...
    def assert_jsonpath_has_field_value_of_type(self, query: Union[str, Dict[str, any]], json_path: str,
                                           expected_types: Union[type, List[type]], arguments: Dict[str, any] = None):
        """
        ...
        def constraint(actual: any) -> Tuple[bool, str]:
            return type(actual) in expected_types, f"expected type of {actual} (type {type(actual)}) would be one of: {expected_types}"
