...
from doger.pythonjsonlogger import jsonlogger


DEFAULT_FORMAT = (
    "%(color)s%(name)s: %(message)s"
)

CUST_STDOUT_FORMAT = "%(color)s [PID %(process)d] [%(filename)s:%(lineno)d] >>> [%(levelname)s] %(message)s"

CUST_FILE_FORMAT = "[%(asctime)s] --> %(message)s <-- [%(filename)s:%(lineno)d] [%(levelname)s] [%(threadName)s]"


DEFAULT_EXTRAS_FORMAT = (
    "%(color)s %(message)s%(color_stop)s %(extras)s"
)
...
class ColorFormatter(logging.Formatter):
    ...
    LEVEL_COLORS = {
        ...
        logging.INFO: "\033[01;34m",  # BOLD BLUE
