...
def guru(level="DEBUG", name = 'my_library', tofile=False, logfile=None, errorlogfile=None):
    '''
    loguru日志设置
    :param level:
    :param tofile: 是否把日志记录到文件中
    :return:
    '''
    logger.remove()   # 把loguru默认的日志去除
    current = datetime.datetime.now()
    date = current.strftime("%Y-%m-%d")
    LOG = logfile or f"logs/norms/every_{date}.log"
    ERRORLOG = errorlogfile or f"logs/errors/error_{date}.log"
    ...
    config = {
        ...
        "levels": [dict(name=name, no=13, icon="¤", color="")],
        # "extra": {"user": "someone"},
        ...
        config = {
            "handlers": [
                ...
                dict(
                    ...
                    level="DEBUG",
                    mode="w",
                    format="<level>{message}</level> >>> {file}:{line} {level}",
                    enqueue=True,
                    serialize=True,
                ),
                dict(sink=ERRORLOG, level="ERROR", enqueue=True, serialize=True),
            ],
            "levels": [dict(name=name, no=13, icon="¤", color="")],
            # "extra": {"user": "someone"},
            "extra": {"common_to_all": "default"},
