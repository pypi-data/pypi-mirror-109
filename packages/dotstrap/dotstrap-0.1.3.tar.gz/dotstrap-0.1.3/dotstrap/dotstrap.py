...
def clone( repo ):
    ...
    runccore("git", "clone", "--bare", repo, "%s/.dotstrap" % os.environ["HOME"])

    if run( "checkout" ) != 0:
        echo(click.style('could not checkout - did you specify a valid repo? or maybe some files were in the way?', fg="red"))
        return
