...
###--------------------------------------------------------------------------------------------
    ...
    def __get_params_root_path(self, context=None):
        ...
        stage, env = Contexts.parse_context(context)
        # if env == 'default':
    ...
    def __assert_param_file_exists(self, context, key):
        if context:
            if not self.__context_exists(context):
                # raise Exception(MESSAGES['ContextNotFound'].format(context))
                raise Exception(MESSAGES['ParameterNotFound'].format(key))

        if not self.__param_group_exists(context, key):
            # raise Exception("Parameter group '{0}' not found.".format(self.__get_param_group_from_key(key)))
            raise Exception(MESSAGES['ParameterNotFound'].format(key))
...
###--------------------------------------------------------------------------------------------
    ...
    def get(self, context, key):
        ...
        #     raise Exception(MESSAGES['ParameterNotFoundScope'].format(key))
        return value
