...
class ConfigManager:
    ...
    def load(self, working_dir, config_overrides):
        ...
        workingDirConfigPath = os.path.join(self.working_dir, self.config_dir)
        configFilePath = os.path.join(workingDirConfigPath, self.config_file)
        if not os.path.exists(configFilePath):
            Logger.warn(MESSAGES['NoDSOConfigFound'].format(self.working_dir))

        for dir in Path(workingDirConfigPath).resolve().parents:
            configFilePath = os.path.join(dir, self.config_dir, self.config_file)
            if os.path.exists(configFilePath):
                Logger.debug(f"Found DSO configuration: path={dir}")
        ...
        for configFile in reversed(self.config_files):
            ...
            if os.path.exists(configFilePath) and os.path.samefile(configFile, configFilePath):
        ...
        if config_overrides:
            try:
                ...
                for config in configs:
                    ...
                    if not old:
                        Logger.warn(f"DSO configuration '{key}' was ignored to override because it was not found in the merged DSO configuration.")
                    else:
