from .backend import generate_tree, fold_tree, get_example_data, Options, default_options, to_graphviz, fold_and_out, fold, Tree, DTree

def viz(root_obj, compress_level:int = 2, property:str = 'long') -> DTree:
    '''
    :param root_obj: A python object
    :param compress_level: 0(Minimal compression) ~ 3(Full compression). Each number has its preset printing options. see also compress(Options())
    :param property: Either "long" or "short" or "memo". "short" shows only its structure while "long" shows a little of its contents
    :return: Tree: A tree object based on treelib.Tree
    '''
