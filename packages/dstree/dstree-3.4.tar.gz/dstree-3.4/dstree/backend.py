...
class Custom_class():
    '''
    A user defined custom class should have either __getitem__ or __getattribute__ method defined.
    the methods are already defined for most of data objects.
    '''
...
def get_example_data(data:str="woz_data_dir"):
    '''
    :param data: str of either "nested_data", "feature_data", "array_data" or "dataframe_data"
    :return:
    '''
    import os
    import json
    ...
    directory = dict(
        nested_data= "woz_train_en.json",
        feature_data='wos-v1.pkl',
        array_data = 'iris_arr',
        dataframe_data = 'iris_df'
    )

    home = os.path.dirname(__file__)

    if data == "nested_data":
        with open(os.path.join(home, directory[data]), 'r', encoding='utf-8') as f:
            example_data = json.load(f)
    elif data == "feature_data":
        with open(os.path.join(home, directory[data]), mode='rb+') as f:
            example_data = dill.load(f)
    elif data == 'array_data':
        example_data = load_iris()['data']
    elif data == 'dataframe_data':
        example_data = load_iris(as_frame = True)
    else:
        return None
...
class DTree(Tree):
    def __getitem__(self, key)->Dnode:
...
def generate_tree(root_obj)->DTree:
    tree = Tree()
...
if __name__ == "__main__":
    ...
    # root_obj = get_example_data("dataframe_data")
    # root_obj = get_example_data("nested_data")
    # root_obj = get_example_data("array_data")
    root_obj = get_example_data("feature_data")
