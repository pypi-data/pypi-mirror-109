...
class Table(object):
    def __init__(self, epm, tableName):
        ...
        self.db = epm
        self.columns = epm.do("select * from information_schema.columns where TABLE_SCHEMA = %s AND TABLE_NAME = %s;", (epm.db, self.tableName))
        self.pk = epm.do("SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.KEY_COLUMN_USAGE WHERE TABLE_SCHEMA = %s AND TABLE_NAME = %s;", (epm.db, self.tableName))
