...
@njit(nogil=True)
def RK4_OBSERVER_CJH_STYLE(THE_DYNAMICS, OB, hs, CTRL):
    NS = OB.NS
    ...
    fx = THE_DYNAMICS(OB.x, OB, CTRL)
    for i in range(0, NS):
        k1[i] = fx[i] * hs
        xk[i] = OB.x[i] + k1[i]*0.5
    ...
    for i in range(0, NS):
        k4[i] = fx[i] * hs
        OB.x[i] = OB.x[i] + (k1[i] + 2*(k2[i] + k3[i]) + k4[i]) * one_over_six
...
def DSP(ACM, CTRL, reg_id, reg_iq, reg_speed, OB, HUMAN):
    ...
    if True:
        ...
        OB.uab_prev[0] = CTRL.cmd_uab[0] # Updating "OB.uab_prev[0]" here is valid if commanded voltage is used
        OB.uab_prev[1] = CTRL.cmd_uab[1] # Updating "OB.uab_prev[1]" here is valid if commanded voltage is used
        OB.uab_curr[0] = OB.uab_prev[0]
        OB.uab_curr[1] = OB.uab_prev[1]
        if False:
            RK4_OBSERVER_CJH_STYLE(DYNAMICS_2ndOdESO_LOF, OB, CTRL.CL_TS, CTRL)
        else:
            OB.output_error = np.sin(CTRL.theta_d - OB.x[0]) # OE version 1
            RK4_OBSERVER_CJH_STYLE(DYNAMICS_4thOdESO_LOF, OB, CTRL.CL_TS, CTRL)
            # OB.output_error = np.sin(CTRL.theta_d - OB.x[0]) # OE version 2
        while OB.x[0]> np.pi: OB.x[0] -= 2*np.pi
        while OB.x[0]<-np.pi: OB.x[0] += 2*np.pi
...
def ACMSimPyIncremental(
    ...
    for ii in range(len(machine_times)):
        ...
        if jj >= down_sampling_ceiling:
            ...
            watch_data[15][watch_index] = OB.x[0] # theta_d
            watch_data[16][watch_index] = OB.x[1] / (2*np.pi*ACM.npp) * 60 # omega_elec
            watch_data[17][watch_index] = OB.x[2] # TL
            watch_data[18][watch_index] = OB.x[3] # pT
...
if __name__ == '__main__':
    ...
    numba__scope_dict = OD([
        ...
        (r'Position [rad]',               ( 'ACM.x[3]', 'CTRL.theta_d', 'OB.x[0]'            ,) ),
        ...
        (r'K_{\rm Active} [A]',           ( 'ACM.x[5]', 'KActive'             ,) ),
        (r'Load torque [Nm]',             ( 'OB.x[2]'                         ,) ),
    ])
