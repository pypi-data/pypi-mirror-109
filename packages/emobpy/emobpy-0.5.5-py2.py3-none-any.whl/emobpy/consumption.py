...
class BEVspecs:
    ...
    def search_by_parameter(self, parameter='Power kW (kilowatts)', first_x=10, brand_filter=[], model_filter=[],
                            year_filter=[]):
        """
        Searching for vehicles sorted in descending order of given parameter. It returns a Pandas DataFrame.

        Args:
            parameter (str): Vehicle parameter to compare. Defaults to 'Power kW (kilowatts)'
        ...
        df = pd.DataFrame(columns=['brand', 'model', 'year', 'value'])
                ...
                for year_name, year_values in model_values.items():
                    ...
                    for para_name, para_value in year_values.items():
                        if para_name == parameter:
                            df = df.append(
                                {'brand': brand_name, 'model': model_name, 'year': year_name, 'value': para_value},
                                ignore_index=True)
    ...
    def get(self, brand, model, year, parameter):
        ...
        Args:
            ...
            model (str): E.g. 'ID.3'
            year (int): E.g. 2020
            parameter (str): E.g. 'Acceleration from 0 to 100 km/h s (seconds)'
        ...
        if parameter in self.parameters:
            try:
                value = self.data[brand][model][str(int(year))][parameter]
                return value
            except KeyError:
    ...
    def get_fallback_parameter(self, parameter):
        ...
        """
        try:
            return self.data['fallback_parameters'][parameter]
        except KeyError:
            return None
...
class ModelSpecs:
    ...
    def add_calculated_param(self):
        ...
        if (
            ...
            self.parameters["PMR W/kg"] = self._pmr(
        ...
        if (
            ...
            self.parameters["Frontal area m2"] = self._frontal_area(
...
class Consumption:
        ...
        for i, row in self.profile.iterrows():
            if row["state"] == "driving":
                ...
                stv = [
                    ...
                    ["Generator", "Regenerative braking", P_g_out_t / 3600 / 1000],
    ...
    def _fill_rows(self):
        ...
        self.timeseries['consumption'] = (self.timeseries['consumption']*self.timeseries['distance']).fillna(0)/(self.timeseries['distance'].replace(0,1))  # this is an artifact to make zero the consumption when distance is zero
