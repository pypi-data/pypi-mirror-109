...
def nombre2lettres(x):
    ...
    if len(y)<=3:
        ...
        if len(y) == 4  and x[-4] == '1':
            ...
            total = unite(y[-9:-6])+' million '+unite(y[-6:-3])+' mille'+sp+unite(y[-3:])
        elif len(y) <= 9:
...
def conv(x):
    x=str(x)
    if '.' in x:
        euros,centimes = x.split('.')[0],x.split('.')[1]
        if len(centimes)==1:
            centimes=centimes+'0'
        if centimes != '00':
            return nombre2lettres(euros)+' euros et '+nombre2lettres(centimes)+' centimes'
        else:
            return nombre2lettres(euros)+' euros'
    else:
        return nombre2lettres(x)+' euros'
