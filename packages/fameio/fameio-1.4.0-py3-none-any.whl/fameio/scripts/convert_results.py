...
from fameio.scripts.make_config import get_config_or_default
from fameio.source.cli import arg_handling_convert_results
from fameio.source.logs import set_up_logger, log_and_raise_critical
from fameprotobuf import DataStorage_pb2


DEFAULT_CONFIG = {"log_level": "warning", "log_file": None, "agents_to_extract": None, }
...
def create_data_frames(class_name, proto_buffer, agent_fields):
    """Returns list of DataFrames each containing output (time step, value) of one agent (by id) and column (by name)"""
    data_frames = []
...
def extract_series(series, agent_fields):
    ...
    'times': dictionary mapping each output column (by name) to list of time steps where an output occurred
    'agentId': id of agent that created this output
    'values': dictionary mapping each output column (by name) to list of values for each time steps with an output
    """
    values_dict = {}
    time_index_dict = {}
