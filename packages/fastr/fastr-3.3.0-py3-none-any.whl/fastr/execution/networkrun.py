...
from ..globals import set_current_run
...
class NetworkRun(Serializable):
        ...
        sink_data_json = {
            sink_node: {
                str(sample_id): {
                    ...
                    "sample_status": status.name,
                    "errors": [list(str(e) for e in x) for x in failed_annotations],
                } for sample_id, (job, status, failed_annotations) in sink_data.items()
    ...
    def execution_finished(self):
        ...
        result = not any(x[1].name == 'FAILED' for sink_data in self.sink_results.values() for x in sink_data.values())
        self.result = result
    ...
    def register_signals(self):
        ...
        if threading.current_thread() is not threading.main_thread():
            log.warning('Cannot change signals from outside of main thread!')
            return
