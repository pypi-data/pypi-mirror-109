...
from ..core.samples import SampleId, SampleIndex, SampleItem, SampleState
...
class NodeRun(BaseNodeRun):
    ...
    def execute(self):
        ...
        for sample_index, sample_id, job_data, job_depends, failed_annotations, status in self.input_group_combiner:
            log.debug('----- START -----')
            ...
            job = self.create_job(sample_id, sample_index, job_data, job_depends, status)

            if status == SampleState.MISSING:
                job.status = JobState.execution_skipped
                job.write()
    ...
    def set_result(self, job, failed_annotation):
        ...
        for output in self.outputs.values():
            ...
            missing_input_data = any(x.status == SampleState.MISSING for x in job.input_arguments.values())
            ...
            if not failed_annotation and self.blocking:
                ...
                status = SampleState.MISSING if missing_input_data else SampleState.VALID
            ...
            # Errors and no samples
                ...
                status = SampleState.MISSING if missing_input_data else SampleState.FAILED
                output[sample_id, sample_index] = SampleItem(sample_index,
                                                             sample_id,
                                                             OrderedDict({0: tuple(output_values)}),
                                                             {job},
                ...
                sample_item = output[sample_index]

                # Update failed annotations and status
                new_failed_annotation = set(sample_item.failed_annotations)
                new_failed_annotation.update(failed_annotation)
                new_status = SampleState.MISSING if sample_item.status == SampleState.MISSING else SampleState.FAILED

                new_item = sample_item.replace(
                    failed_annotations=new_failed_annotation,
                    status=new_status,
                )
                output[sample_index] = new_item
            else:
    ...
    def create_job(self, sample_id: SampleId, sample_index: SampleIndex, job_data,
                   job_dependencies, status, **kwargs):
        """
