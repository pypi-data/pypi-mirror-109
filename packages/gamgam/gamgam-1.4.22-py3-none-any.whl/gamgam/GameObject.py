...
class Animator(Component):
    ...
    def reflect(self):
        for i in self.animation_count.keys():
            if type(self.animation[i][self.animation_count[0]]) == int:
                if t.time() - self.animation_count[1] == self.animation[i][self.animation_count[0]]:
                    self.animation_count[0] += 1
            elif type(self.animation[i][self.animation_count[0]]) == Image:
                find_GameObject(self.this_object).image = self.animation[i][self.animation_count[0]]
                self.animation_count[0] += 1
