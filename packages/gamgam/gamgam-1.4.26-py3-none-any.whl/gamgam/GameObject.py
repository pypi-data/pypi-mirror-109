...
class Animator(Component):
    ...
    def reflect(self):
        if self.now is not None:
            if self.animation_count[i][0] >= len(self.animation[i]):
                self.animation_count[i][0] = 0
            if type(self.animation[i][self.animation_count[i][0]]) == int:
                if t.time() - self.animation_count[i][1] == self.animation[i][self.animation_count[i][0]]:
                    self.animation_count[i][0] += 1
            elif type(self.animation[i][self.animation_count[i][0]]) == Image:
                find_GameObject(self.this_object).image = self.animation[i][self.animation_count[i][0]]
                self.animation_count[i][0] += 1
