...
class Animator(Component):
    ...
    def reflect(self):
        if self.now is not None:
            ...
            if type(self.animation[self.now][self.animation_count[self.now][0]]) == int:
                if self.animation_count[self.now][1] == 0:
                    self.animation_count[self.now][1] = t.time()
                elif t.time() - self.animation_count[self.now][1] >= self.animation[self.now][self.animation_count[self.now][0]]:
    ...
    def stop(self):
        self.now = None
        for i in animation_count.keys():
            animation_count[i][0] = 0
            animation_count[i][1] = 0
