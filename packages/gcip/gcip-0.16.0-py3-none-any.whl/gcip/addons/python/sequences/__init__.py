...
def full_stack(
    ...
    sequence.add_children(
        ...
        Flake8(),
        Pytest(),
        EvaluateGitTagPepe440Conformity(**evaluate_git_tag_pep440_conformity_args),
        BdistWheel(),
    )

    if mypy_package_dir:
        sequence.add_children(Mypy(mypy_package_dir))

    pages_sphinx = gitlab_pages.Sphinx()
    ...
    twine_upload_dev = TwineUpload(dev_repository_url, dev_user, varname_dev_password)
    ...
    twine_upload_stable = TwineUpload(stable_repository_url, stable_user, varname_stable_password)
