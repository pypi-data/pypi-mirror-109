...
parsed_output = {"processor_pool":{"total":735981852,"used":272743032,"free":463238820},"pid":{"0":{"index":{"1":{"pid":0,"tty":0,"allocated":256940960,"freed":73576632,"holding":158001024,"getbufs":392,"retbufs":12905093,"process":"*Init*", "test_key": "^net_conf_aws"}}}}}
...
class test_dq(unittest.TestCase):
    ...
    def test_contains(self):
        ...
        output = parsed_output_dq.contains("^net_conf[\S\s]*", regex=True, escape_special_chars=['^'])
        self.assertEqual(output.reconstruct(),
        ...
        # is rightfully empty
        output = parsed_output_dq.contains("^net_conf[\S\s]*", regex=True)
        self.assertEqual(output.reconstruct(),
                {})
    ...
    def test_contains_key_wrong_regex_input(self):
        ...
        output = mod.contains_key_value('mac_address', '[a-z0-9\-\s]', value_regex=True)
        self.assertEqual(output.reconstruct(), {})
    ...
    def test_contains_key_value_leaf(self):
        ...
        output = mod.contains_key_value('slot/world_wide_name|mac.*|model', '[a-zA-Z0-9\-\s]+', key_regex=True, value_regex=True)
        self.assertEqual(output.reconstruct(),
    ...
    def test_contains_key_equal_contains(self):
        ...
        self.assertEqual(mod.contains('mac_address'),
                         mod.contains_key_value('mac_address', '[a-z0-9\-\s]+', value_regex=True))
    ...
    def test_query_validator_nested_functions(self):
        mod = Dq(module)
        query = "contains('%VARIABLES{abc}\.\d+', regex=True).contains_key_value('key1', 'key2').count()"
