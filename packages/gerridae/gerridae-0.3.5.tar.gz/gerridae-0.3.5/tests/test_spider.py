...
from gerridae import Item, Spider, TextField, RegexField, AttrField
...
class TestBaseField:
    ...
    def test_spider_css_selector_field(self):
        class TestItem(Item):
            last_chapter_url = AttrField(
                css_select='#wrapper > div.content_read > div > div.bookname > div.bottem1 > a:nth-child(2)',
                attr='href')
            next_chapter_url = AttrField(
                css_select='#wrapper > div.content_read > div > div.bookname > div.bottem1 > a:nth-child(4)',
                attr='href')

        class TestSpider(Spider):
            start_urls = 'https://www.230book.com/book/20237/6915563.html'

            async def parse(self, rep):
                text = await rep.text(encoding='gbk')
                result = TestItem.get_item(html=text)
                return result
