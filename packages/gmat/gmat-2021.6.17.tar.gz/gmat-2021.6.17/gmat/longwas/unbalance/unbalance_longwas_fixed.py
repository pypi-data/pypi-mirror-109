...
def unbalance_longwas_fixed(data_file, id, tpoint, trait, bed_file, agmat_file, var_com, condition_snp=None, snp_lst=None,
    ...
    :param condition_snp: SNP ID or a list of SNP ID for conditional snps added to the time fixed effect.
    ...
    if condition_snp is not None:
        ...
        for i in range(condition_snp_val.shape[1]):
            snp_condition = list(map(lambda x, y: x * y, leg_lst, list(condition_snp_val[:, i])))
    ...
    for i in tqdm(range(snp_mat.shape[1])):
        ...
        p_xsnp = np.dot(pmat, snp_fix[:, 0:1])
        xpx = np.dot(snp_fix[:, 0:1].T, p_xsnp)
