...
def fixed(pheno_file, bed_file, agmat_file, class_lst=None, fix=None, out_file='res.txt', condition_snp=None, npart=100,
    ...
    if speed:  # whether to speed up with estimated variances in the null model
        for i in range(npart):
            ...
            for k in tqdm(range(snp_mat.shape[1])):
                xmat_trans_k = np.concatenate([xmat_trans, snp_mat[:, k:(k + 1)]], axis=1)
