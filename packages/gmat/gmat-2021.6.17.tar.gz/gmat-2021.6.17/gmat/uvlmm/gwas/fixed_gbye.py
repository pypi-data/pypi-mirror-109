...
def fixed_gbye(pheno_file, bed_file, agmat_file, bye, class_lst=None, fix=None, out_file='res.txt', condition_snp=None, npart=100,
    ...
    :param bye: the environment effect that SNP interacts with
    ...
    if condition_snp is not None:
        ...
        xmat_con = np.dot(gmat_eigenvecs.T, xmat_con)
        xmat_trans = np.concatenate([xmat_trans, xmat_con], axis=1)
    logging.info('###Start the association')
    with open(out_file, 'w') as fout:
        fout.write('\t'.join(['chro', 'snp_ID', 'pos', 'allele1', 'allele2', 'cc', 'eff', 'se', 'p_snp', 'p_gbye', 'p_all']))
        fout.write('\n')
    maxiter = maxiter1
    if not convergence:
        maxiter = 0
    if speed:  # whether to speed up with estimated variances in the null model
        ...
        for i in range(npart):
            ...
            for k in tqdm(range(snp_mat.shape[1])):
                ...
                snpk = np.dot(gmat_eigenvecs.T, snpk)
                xmat_trans_k = np.concatenate([xmat_trans, snpk], axis=1)
                # main
                vx = np.multiply(vmat, xmat_trans_k[:, :-bye_df])
                xvx = np.dot(xmat_trans_k[:, :-bye_df].T, vx)
                try:
                    xvx = linalg.inv(xvx)
                ...
                # GbyE
                vx = np.multiply(vmat, xmat_trans_k)
                xvx = np.dot(xmat_trans_k.T, vx)
                try:
                    xvx = linalg.inv(xvx)
                except Exception as _:
                    xvx = np.diag(np.diag(xvx)) * 0.001 + xvx
                ...
                chi_all = np.sum(np.dot(b_k[-(bye_df+1):, :].T, np.dot(linalg.inv(xvx[-(bye_df+1):, -(bye_df+1):]), b_k[-(bye_df+1):, :])))
            ...
            eff_arr = np.array(eff_vec)
            se_arr = np.array(se_vec)
            cc_arr = np.array([1000.0]*len(p_snp_vec))
            p_snp_arr = np.array(p_snp_vec)
            p_gbye_arr = np.array(p_gbye_vec)
            p_all_arr = np.array(p_all_vec)
            ind_bool = ((p_snp_arr < pcut) + (p_gbye_arr < pcut) + (p_all_arr < pcut)) > 0
            cc_vec = []
            eff_vec = []
            se_vec = []
            p_snp_vec = []
            ...
            for k in tqdm(range(snp_mat.shape[1])):
                ...
                snpk = np.dot(gmat_eigenvecs.T, snpk)
                xmat_trans_k = np.concatenate([xmat_trans, snpk], axis=1)
                cc_val, eff, se, p_val, p_bye, p_all = _weight_emai_eigen_snp(y_trans, xmat_trans_k, bye_df, gmat_eigenvals, var,
            ...
            p_snp_arr[ind_bool] = p_snp_vec
            p_gbye_arr[ind_bool] = p_gbye_vec
            p_all_arr[ind_bool] = p_all_vec
            res_df.loc[:, 'cc_val'] = cc_arr
            res_df.loc[:, 'eff_val'] = eff_arr
            res_df.loc[:, 'se_val'] = se_arr
            res_df.loc[:, 'p_snp_val'] = p_snp_arr
            res_df.loc[:, 'p_gbye_val'] = p_gbye_arr
            res_df.loc[:, 'p_all_val'] = p_all_arr
...
def _weight_emai_eigen_snp(y_trans, xmat_trans, bye_df, gmat_eigenvals, var, maxiter=20, cc=1.0e-8):
    fd_mat = np.zeros(2)
    ai_mat = np.zeros((2, 2))
    em_mat = np.zeros((2, 2))
    num_id = gmat_eigenvals.shape[0]
    ...
    vmat = 1.0 / (gmat_eigenvals * var[0] + var[1])
    # main
    vx = np.multiply(vmat, xmat_trans[:, :-bye_df])
    xvx = np.dot(xmat_trans[:, :-bye_df].T, vx)
    try:
        xvx = linalg.inv(xvx)
    except Exception as _:
        xvx = np.diag(np.diag(xvx)) * 0.001 + xvx
    ...
    chi_all = np.sum(np.dot(b_k[-(bye_df + 1):, :].T,
                            np.dot(linalg.inv(xvx[-(bye_df + 1):, -(bye_df + 1):]), b_k[-(bye_df + 1):, :])))
    p_all = chi2.sf(chi_all, bye_df + 1)
    return cc_val, eff, se, p_val, p_bye, p_all
