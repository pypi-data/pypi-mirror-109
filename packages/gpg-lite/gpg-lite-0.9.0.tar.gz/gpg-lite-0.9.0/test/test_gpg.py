...
class TestUtils(unittest.TestCase):
    def test_split_url(self):
        self.assertEqual(
            gpg.split_host("http://hagrid.hogwarts.org:11371"),
            ("http://", "hagrid.hogwarts.org", ":11371"),
        )
        self.assertEqual(
            gpg.split_host("hkp://hagrid.hogwarts.org:11371"),
            ("hkp://", "hagrid.hogwarts.org", ":11371"),
        )
        self.assertEqual(
            gpg.split_host("hagrid.hogwarts.org:11371"),
            (None, "hagrid.hogwarts.org", ":11371"),
        )
        self.assertEqual(
            gpg.split_host("http://hagrid.hogwarts.org"),
            ("http://", "hagrid.hogwarts.org", None),
        )
        self.assertEqual(
            gpg.split_host("hagrid.hogwarts.org"),
            (None, "hagrid.hogwarts.org", None),
        )
        with self.assertRaises(ValueError):
            gpg.split_host("hagrid.hogwar:ts.org")
...
"""
        ...
        keys = list(gpg.parse_search_keyserver(response))
        self.assertEqual(
            keys,
            [
                gpg.KeyInfo(
    ...
    def test_parse_keys(self):
        key_payload = io.BytesIO(
            b"""tru::1:1571400791:0:3:1:5
pub:u:2048:1:FDC66B7A5F6FFD4B:1550241679:::u:::scESC::::::23:1571230031:1 http\\x3a//hagrid.hogwarts.org\\x3a11371:
...
"""
    ...
    def test_parse_keys_revocation_sig(self):
        key_payload = io.BytesIO(
            b"""
pub:-:2048:1:0000000000000000:1587731664:::-:::scESC::::::23:1588834129:1 http\\x3a//hagrid.hogwarts.org\\x3a11371:
...
sig:::1:771685B5FF0F9AC7:1588064955::::Unit Test Authority (validation key) <authority@roundhouse.gov>:13x::B37CE2A101EBFA70941DF885771685B5FF0F9AC7:::8:
rev:::1:771685B5FF0F9AC7:1588066811::::Unit Test Authority (validation key) <authority@roundhouse.gov>:30x,00::B37CE2A101EBFA70941DF885771685B5FF0F9AC7:::8:::::revsig\\nInvalid email:
            ...
            [
                gpg.Key(
                    ...
                    signatures=(
                        ...
                        gpg.Signature(
                            issuer_uid=KEY_AUTHORITY_UID,
                            issuer_key_id="771685B5FF0F9AC7",
                            creation_date="1588064955",
                            signature_class="13x",
                        ),
...
"""
        self.assertEqual(
            gpg.capture_fpr(b), ["AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA"]
        )
        b = b"""
...
gpg:          There is no indication that the signature belongs to the owner.
"""
        self.assertEqual(gpg.capture_fpr(b), [])
        b = b"""
[GNUPG:] NEWSIG
gpg: Signature made Tue 10 Mar 2020 11:29:27 CET
gpg:                using RSA key AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA
...
class TestGPGIntegration(IntegrationTest):
    ...
    def test_delete_keys(self):
        ...
        self.assertEqual(key.fingerprint, SGT_HARTMANN_FPRT)
...
class TestGPGKeyserver(IntegrationTest):
    def test_send_search(self):
        ...
        keys = gpg.search_keyserver(key_a.uids[0].full_name, keyserver=KEY_SERVER)
        self.assertEqual(list(keys), [to_info(key_a)])

        keys = gpg.search_keyserver(key_b.uids[0].full_name, keyserver=KEY_SERVER)
        self.assertEqual(list(keys), [to_info(key_b)])

        # By fingerprint
        keys = gpg.search_keyserver(key_b.fingerprint, keyserver=KEY_SERVER)
        self.assertEqual(list(keys), [to_info(key_b)])

        # By ID
        keys = gpg.search_keyserver(key_b.key_id, keyserver=KEY_SERVER)
        self.assertEqual(list(keys), [to_info(key_b)])
