...
class IDGeneration(Enum):
    """
    Enumerate the node id generation schemes.
    """

    DETERMINATE = 0
    """
    Generate a determinate and consistent id.
     
    A determinate id generated from a hash of the following properties:
      - Node type
      - Name
      - Short name
      - Version
...
class GRENMLObject:
    ...
    def __init__(
        ...
        short_name=None, version=None, id_format: IDGeneration = None, *args, **kwargs
    ):
        self.name = name
        ...
        if not id:
            if not id_format or id_format == IDGeneration.DETERMINATE:
                hsh = sha256()
                hsh.update(type(self).__name__.encode('utf-8'))

                hsh.update(':'.encode('utf-8'))
                if name:
                    hsh.update(self.name.encode('utf-8'))

                hsh.update(':'.encode('utf-8'))
                if short_name:
                    hsh.update(self.short_name.encode('utf-8'))

                id = hsh.hexdigest()
                self._id_format = IDGeneration.DETERMINATE
            elif id_format == IDGeneration.RANDOM:
                id = uuid()
                self._id_format = IDGeneration.RANDOM
            else:
                raise ValueError('invalid value for id_format')
        else:
            self._id_format = IDGeneration.CUSTOM
