...
class LunarDate():
    def __init__(self, lunar_year, lunar_month, lunar_day, leap_month=False):
        """
        Keyword Arguments:
            leap_month {bool} --  Is/Not in a leap month (default: {False})
    ...
    def to_datetime(self):
        ...
        Returns:
            datetime -- solar dates
        """
        return self.newyear + timedelta(days=self.__days_passed())

    @staticmethod
    def from_datetime(dt):
        """Converts solar calendar dates to lunar dates

        Arguments:
            dt {datetime} -- solar dates

        Returns:
            LunarDate -- lunar dates
        """
        lunar_year = dt.year
    ...
    def __days_passed(self):
        """
        Calculate the number of days difference between the current lunar date
        and the current Lunar New Year

        Returns:
            int -- Difference of days
        """
    ...
    def chinese(self):
        ...
        year_tiandi = LunarDate._chinese_era(self.lunar_year - 1900 + 36) + '年'
    ...
    @staticmethod
    def _chinese_era(anum):
        heavenly_stems = '甲乙丙丁戊己庚辛壬癸'
        mundane_branches = '子丑寅卯辰巳午未申酉戌亥'
        return f'{heavenly_stems[anum % 10]}{mundane_branches[anum % 12]}'
    ...
    def validate(year, month, day, leap):
        ...
        Arguments:
            ...
            month {int} -- Lunar
            day {int} -- Lunar
            leap {bool} -- Determine if the lunar calendar is a leap month date.

        Returns:
            bool -- Check pass or not
        """
    ...
    def decode(year_code):
        """Decode the annual lunar code function
...
if __name__ == "__main__":
    print(chinaCalendar(datetime(2020, 7, 1)))
