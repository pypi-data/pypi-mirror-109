import fire
import os
from guang.Utils.toolsFunc import path, replace_files_content
from guang.cv.video import resample, embedFrameInfo
from guang.media.ffmpeg import *


def replace(old, new, pathname='./**/*'):
    """ Replace files content from the given path.
    :param pathname: `glob` style path
    :param count: replace count. if count=0, relplace all
    """
    replace_files_content(old, new, pathname=pathname, count=0)
...
def fy(words):
    from guang.translate.run import tencent
    tencent(words)


def mie():
    origin_wd = os.getcwd()
    os.chdir(path(r"C:\BaiduNetdiskDownload\CODE\a_github\ScatteringLight"))
...
def upload(PAHT1="upload/", PATH2="/var/www/html/"):
    ...
    def download(
        remote_path: str,
        local_path: str,
        config_path=r"C:\BaiduNetdiskDownload\CODE\a_github\myWebsite\config.yaml"
    ):
