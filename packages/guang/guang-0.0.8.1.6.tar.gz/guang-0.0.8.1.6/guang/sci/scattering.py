...
from numpy import sin, pi, cos, real, arcsin, arccos, diff
import numpy as np
...
def theta_def(theta_i, p, m):
    ...
    theta_r = arcsin(sin(theta_i) / m + 0j)
...
def phase(theta_i, alpha, p, m):
    ...
    theta_i = theta_i * pi / 180
    if p != 0:
        theta_i[theta_i > real(arcsin(m + 0j))] = None

    #     costheta_r = np.sqrt(1 - np.sin(theta_i/m)**2)
    theta_r = real(arcsin(sin(theta_i) / m + 0j))  # 这里默认给出复数形式，取实数部分，
...
def get_k1k2(theta_1_begin: int,
             theta_2_begin: int,
             dthe1: int,
             dthe2: int,
             p1=1,
...
def predict(v1, v2, k1, k2, m_min=1.301, m_max=1.399):
    ...
    return {'alpha1': alpha1, 'alpha2': alpha2, 'm': m}
