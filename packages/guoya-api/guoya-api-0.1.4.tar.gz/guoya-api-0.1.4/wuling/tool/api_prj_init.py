...
def get_files(path):
    ...
    j = []
    for f in files:
        if (f in ['venv','.idea',"__pycache__",'.git','.pytest_cache']):
            continue
        if f.endswith(".pyc"):
            continue
        j.append(f)
...
def init_prj(git_addr):
    print('---------开始初始化-------------')
    root_path = _get_root_path()
    print('获取工程根目录：%s' % root_path)

    prj_name=git_addr.split(".git")[0].split('/')[-1]
    ...
    cmd = 'git clone ' + git_addr +' ' + prj_dir
    print(cmd)
    print('框架下载中，请耐心等候......... ')
    _invoke(cmd)

    ## 读取ui初始化配置文件
    files = get_files(prj_dir)
    for f in files:
        s = prj_dir+'/'+f
        if os.path.isdir(s):
            _copy_dir(s, root_path + f)
        else:
            _copy_file(s, root_path)
