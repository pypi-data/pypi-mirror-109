    ...
    def wait_for_gripper_stop(self, timeout: Optional[float] = None):
        ...
        while time.time() - start_time < timeout:
            if abs(self.gripper_velocity) < self.GRIPPER_VELOCITY_STOP_THRESHOLD:
                time.sleep(self.GRIPPER_STOP_WAIT_TIME)
                return

        raise robot_arms.RobotArmGripperTimeoutError(f'Timeout while waiting for gripper to stop')

    def open_gripper(self, position: Optional[Union[float, bool]] = None, duration: Optional[float] = None, wait: bool = True, timeout: Optional[float] = None):
        if position is None:
            position = 0.0
        ...
        if duration is not None:
            command.duration = round(duration)
        ...
        if wait:
            time.sleep(self.GRIPPER_WAIT_DELAY)
