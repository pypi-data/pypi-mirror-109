...
def value_difference(value: float, expected_value: float):
    ...
    """
    return 0 if expected_value == 0 or (isinstance(expected_value, list) and len(expected_value) == 0) else (
...
def _model_value_from_list(results: list): return results[0].get('value', []) if len(results) > 0 else []


def _model_value(result):
    return 0 if result is None else (
        _model_value_from_list(result) if isinstance(result, list) else (
            result.get('value', []) if isinstance(result, dict) else 0
        )
    )
