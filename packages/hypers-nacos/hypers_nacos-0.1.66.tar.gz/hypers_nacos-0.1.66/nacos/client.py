...
class NacosClient:
    ...
    def get_yaml_config(self, data_id, group, timeout=None, no_snapshot=None):
        """对get_config返回的内容解析"""
        content = self.get_config(data_id, group, timeout, no_snapshot) 
        configs = dict()
        try:
            configs = yaml.load(content, Loader=yaml.FullLoader)
        except yaml.scanner.ScannerError:
            raise Exception("[get-yaml_config] data_id:%s, group:%s, namespace:%s, 配置文件格式不对，请修改为yaml格式") % (data_id, group, self.namespace)
        except Exception as e:
            logger.error("[get-yaml_config] data_id:%s, group:%s, namespace:%s\n %s") % (data_id, group, self.namespace, e)
            raise
        data = namedtuple('configs', configs.keys())(*configs.values())  # todo不存在的key兼容
        return data
