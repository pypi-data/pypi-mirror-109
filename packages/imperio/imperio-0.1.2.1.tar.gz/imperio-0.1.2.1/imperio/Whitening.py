...
class WhiteningTransformer(BaseEstimator, TransformerMixin):
    ...
    def __init__(self, index : list = None) -> None:
        '''
        Constructor of the Whitening class
    ...
    def fit(self, X : 'np.array', y : 'np.array', **fit_params : dict):
        '''
            The fit function of the Whitening, fits up the model.
        ...
        :return: BocCoxTransformer
            The fitter Whitening object.
        '''
        self.shape = X.shape
        return self
        
    def transform(self, X : 'np.array', y : 'np.array', **fit_params : dict):
        '''
            The transform function of the Whitening, transforms the passed data..
        :param X: 2-d numpy array or pd.DataFrame
            The 2-d numpy array or pd.DataFrame that represents the feature matrix.
