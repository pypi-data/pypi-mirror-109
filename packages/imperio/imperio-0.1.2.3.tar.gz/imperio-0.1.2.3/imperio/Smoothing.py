...
class SmoothingTransformer(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X : 'np.array', y : 'np.array' = None, **fit_params : dict) -> 'SmoothingTransformer':
        ...
        if self.index is None:
            self.index = [i for i in range(self.X_copy.shape[1]) if isinstance(self.X_copy[0 ,i],str) and len(set(self.X_copy[:, i])) < self.min_int_freq and len(set(self.X_copy[:, i])) != 2]
        ...
        for col in self.index:
            ...
            target_average = np.zeros(self.X_copy.shape[0])
            ...
            for unique in unique_values:
                ...
                target_average[self.X_copy[:,col] == unique] = average
    ...
    def transform(self, X : 'np.array', y : 'np.array' = None, **fit_params : dict) -> 'np.array':
        ...
        if self.shape[1] == X.shape[1]:
            for i in self.index:
                self.X_copy[:,i] = np.array([self.__mapper[i][val] for val in self.X_copy[:,i]])
        else:
