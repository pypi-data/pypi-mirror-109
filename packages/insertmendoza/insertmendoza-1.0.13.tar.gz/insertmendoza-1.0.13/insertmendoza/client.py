...
class InsertMendoza:
    ...
    def EmployeeDelete(self, uuid, data):
        return self.Request(url=f'employee/{uuid}', method=DELETE, data=data)

    def CameraAdd(self, username, password, ip, port, endpoint='/h264/ch1/main/av_stream'):
        return self.Request(url=f'streaming/camera', method=POST, data={
            'username': username, 'password': password, 'ip': ip, 'port': port, 'endpoint': endpoint
        })

    def DeviceAdd(self, user):
        return self.Request(url=f'streaming/device', method=POST, data={'user': user})

    def ProfileAdd(self, mic, desktop, camera, device, volumen_mic=1, volumen_monitor=1, volumen_speakers=1, delay_mic=1000, delay_monitor=1000, thread_queue_size_monitor=8, thread_queue_size_camera=8, thread_queue_size_mic=8, description=None):
        return self.Request(url=f'streaming/profile', method=POST, data={
            'mic': mic, 'desktop': desktop, 'camera': camera, 'device': device,
            'volumen_mic': volumen_mic, 'volumen_monitor': volumen_monitor, 'volumen_speakers': volumen_speakers,
            'delay_mic': delay_mic, 'delay_monitor': delay_monitor,
            'thread_queue_size_monitor': thread_queue_size_monitor, 
            'thread_queue_size_camera': thread_queue_size_camera,
            'thread_queue_size_mic': thread_queue_size_mic, 
