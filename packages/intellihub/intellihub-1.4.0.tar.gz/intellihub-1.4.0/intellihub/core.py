...
from intellihub.assertions import validate_parameters, is_url_valid, allowed_file_extension
from intellihub.dataset_types import Dataset

def register(email, mobile_number, first_name, last_name, password):
        payload = {
            "email" : email,
        ...
        url = "https://dev-kong.intellihub.ai/register/otp"
...
def registration_confirm(token_value):
    url = "https://dev-kong.intellihub.ai/registrationConfirm?token=" + str(token_value)
    response = requests.get(url=url)
    return response

def forgot_password(email):
    url = "https://dev-kong.intellihub.ai/user/forgotPassword/otp?email=" + str(email)
    response = requests.post(url=url)
    return response

def reset_password(email, token, new_password):
    url = f"https://dev-kong.intellihub.ai/user/resetPassword/otp?email={email}&token={token}"
...
def initialize(authentication_token):
    url = "https://dev-kong.intellihub.ai/user/initialize"

    headers = {'AuthenticationToken': authentication_token}
    response = requests.post(url=url, headers=headers)
    return response

def login(user_name, password):
    url = "https://dev-kong.intellihub.ai/oauth/login"

    payload= f'grant_type=password&client_id=intellihub&username={user_name}&password={password}'
    headers = {
...
def create_app(app_name, description, access_token):
    url = "https://dev-kong.intellihub.ai/app/create/"
    payload = json.dumps({
        "name": app_name,
        "description": description
    })
    headers = {'AuthenticationToken': access_token, 'Content-Type': 'application/json'}
    response = requests.post(url=url, data=payload, headers=headers)
    return response

def app_list(access_token):
    url = "https://dev-kong.intellihub.ai/app/list"
    headers = {'AuthenticationToken': access_token}
...
class IntellihubClient:
    ...
    def __init__(self, api_key=None, base_url="http://14.1.1.75:8000"):
        """
    ...
    def train(self, service, algorithm, dataset, label, features, model_name=None, lib="weka", train_percentage=80, save_model=True, folds=5, cross_validation=False, params=None, dataset_source=None):
        ...
        :param save_model: If True, the model will be saved in the INTELLIHUB Storage.
    ...
    def feedback(self, service, algorithm, train_data, feedback_data, job_id, model_url, label, features, lib='weka',
        ...
        :param save_model: If True, the model will be saved in INTELLIHUB Storage.
