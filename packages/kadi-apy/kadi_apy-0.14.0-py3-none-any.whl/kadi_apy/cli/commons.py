...
def _update_attribute(item, attribute, value, pipe=False):
    ...
    if attribute not in meta:
        if not pipe:
            item.info(f"Attribute {attribute} does not exist.")
        return
    ...
    if value_old == value:
        if not pipe:
            item.info(f"The {attribute} is already '{value_old}'. Nothing to do.")
    else:
...
def print_item_created(
    ...
    if pipe:
        ...
        if item.created:
            item.info(f"Successfully created {item}.")
        else:
            item.info(f"The {item} already exists.")
...
def item_add_user(item, user, permission_new):
    ...
    if response.status_code == 201:
        item.info(f"Successfully added {user} as '{permission_new}' to {item}.")
    elif response.status_code == 409:
        response_change = super(item.__class__, item).change_user_role(
            user_id=user.id, role_name=permission_new
        )
        if response_change.ok:
            item.info(f"The {user} is '{permission_new}' of {item}.")
        else:
...
def item_add_group_role(item, group, permission_new):
    ...
    if response.status_code == 201:
        item.info(f"Successfully added {group} as '{permission_new}' to {item}.")
    elif response.status_code == 409:
        response_change = item.change_group_role(
            group_id=group.id, role_name=permission_new
        )
        if response_change.ok:
            item.info(f"The {group} is '{permission_new}' of {item}.")
        else:
            raise_request_error(response=response_change)
    else:
        raise_request_error(response=response)
...
def item_remove_group_role(item, group):
    ...
    if response.status_code == 204:
        item.info(f"The {group} was removed from {item}.")
    else:
        raise_request_error(response=response)
...
def item_remove_user(item, user):
    ...
    if response.status_code == 204:
        item.info(f"The {user} was removed from {item}.")
    else:
        raise_request_error(response=response)
...
def item_delete(item, i_am_sure):
    ...
    if response.status_code == 204:
        item.info("Deleting was successful.")
    else:
...
def item_add_record_link(item, record_to):
    ...
    if response.status_code == 201:
        item.info(f"Successfully linked {record_to} to {item}.")
    elif response.status_code == 409:
        item.info(f"Link from {item} to {record_to} already exists. Nothing to do.")
    else:
...
def item_remove_record_link(item, record):
    ...
    if response.status_code == 204:
        item.info(f"Successfully removed {record} from {item}.")
    else:
...
def item_add_collection_link(item, collection):
    ...
    if response.status_code == 201:
        item.info(f"Successfully linked {collection} to {item}.")
    elif response.status_code == 409:
        item.info(f"Link from {item} to {collection} already exists. Nothing to do.")
    else:
        item.info(f"Linking {collection} to {item} was not successful.")
        raise_request_error(response=response)
...
def item_remove_collection_link(item, collection):
    ...
    if response.status_code == 204:
        item.info(f"Successfully removed {collection} from {item}.")
    else:
...
def item_add_tag(item, tag):
    ...
    if response is None:
        item.info(f"Tag '{tag}' already present in {item}. Nothing to do.")
    elif response.status_code == 200:
        item.info(f"Successfully added tag '{tag}' to {item}.")
    else:
...
def item_remove_tag(item, tag):
    ...
    if not super(item.__class__, item).check_tag(tag):
        item.info(f"Tag '{tag}' not present in {item}. Nothing to do.")
        return

    response = super(item.__class__, item).remove_tag(tag)

    if response.status_code == 200:
        item.info(f"Successfully removed tag '{tag}' from {item}.")
    else:
