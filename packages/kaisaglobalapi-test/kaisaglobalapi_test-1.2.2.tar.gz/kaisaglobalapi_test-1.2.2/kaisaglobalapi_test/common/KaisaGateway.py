...
acc_id = 'sz1060'
acc_id_path = os.getcwd().replace('\\', '/') + '/logger'
_thisdate_ = datetime.datetime.now().strftime('%Y%m%d')
##  ## 每十分钟运行一遍【5分钟后退订行情】//每个小时运行一遍【50分钟后退订行情】
logger_path = r'/{}.csv'.format(datetime.datetime.now().strftime('%Y%m%d'))
logger_filename = r'logger/_sheet_{}.csv'.format(_thisdate_)
...
class KaisaGateway(object):
    ...
    ## 行情推送的时候记录行情
    ## 退订行情的时候记录退订
    ## 在退订行情后直接将直接所有记录的数据写入到报表csv中
    wiki_dict_ = {}

    def __init__(self):
        pass

    def jq_user_config(self, user_id, user_pwd, account_code, account_pwd, openapi_token, ip_address=None, environment='paper', callback=None):
    ...
    def _mkfile(self, filename):
        # 判断文件是否存在，不存在就创建
        headers = ['update_time', 'id', 'is_sub', 'is_gateway_auth', 'is_init_api', 'is_un_sub', 'is_quote_arrive',
                   'is_quote_right', 'tick_res_time', 'quote_res_time', 'broker_res_time', 'level_order_res_time',
                   'is_api_error']
        with open(filename, mode='w', encoding='UTF-8') as ff:
            employee_writer = csv.writer(ff, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
    ...
    def _make_file(self, filename):
        # 建文件[如果文件不存在，建文件]
        ff = open(filename, mode='w', encoding='UTF-8')
        ff.close()
    ...
    def _write_file(self, filename, data):
        ## 追加写入文件
        fs = open(filename, mode='a', encoding='utf-8')
        fs.write(data)
        fs.close()
    ...
    def init_md_api(self):
        ...
        try:
            self.wiki_dict_['id'] = 'sz1057'
            ## 判断文件夹是否存在，不存在就创建
            #self._mkdir(acc_id_path)
            ## 判断文件是否存在，不存在就创建
            self._make_file(logger_filename)
            ret_ok, ret_msg = self.gateway_auth()
            if ret_ok:
                print('行情账号登录成功!')
                self.init_api_bool = ret_ok
                self.wiki_dict_['is_gateway_auth'] = 'True'
                self.wiki_dict_['is_api_error'] = 'False'
            else:
                self.wiki_dict_['is_gateway_auth'] = 'False'
                print('行情账号登录失败,error:{}!'.format(ret_msg))
                ## -- 把行情推送中触发的异常写入log文档中[正常/失败]
    ...
    def query_detailstate(self, symbol='01638', market=2):
        # detailstate
        post_url = MARKET_POST_URL + "/quoteInfo/todayDetailState"
        rqdata = {
            'code': symbol,
            'market': Quote_MARKET_DICT[market],
            'hasQuoteInfo': True,
            'hasQuoteExt': True,
            'hasQuoteStat': True,
    ...
    def _on_message(self, ws, message) -> None:
        ...
        if message is None or message == '':
            return
        try:
            self.wiki_dict_['is_quote_arrive'] = 'True'
            data = kprotoc.rec_bstring_to_json(message)   # 把字符串转换为json数据类型
            if data is None:
                pass
                ...
                ##################################
                self.wiki_dict_['is_quote_right'] = 'True'
                if data['type'] == 'TICK':
                    if 'tick_res_time' not in self.wiki_dict_:
                        self.wiki_dict_['tick_res_time'] = [datetime.datetime.now()]
                    else:
                        tick_time_list = list(self.wiki_dict_['tick_res_time'])
                        tick_time_list.append(datetime.datetime.now())     ## time.time()
                        self.wiki_dict_['tick_res_time'] = tick_time_list

                if data['type'] == 'QUOTE':
                    if 'quote_res_time' not in self.wiki_dict_:
                        self.wiki_dict_['quote_res_time'] = [datetime.datetime.now()]
                    else:
                        # print('quote_res_time:{}'.format(self.wiki_dict_['quote_res_time']))
                        # print('quote_res_time--now:{}'.format(time.time()))
                        quote_time_list = list(self.wiki_dict_['quote_res_time'])
                        quote_time_list.append(datetime.datetime.now())
                        self.wiki_dict_['quote_res_time'] = quote_time_list

                if data['type'] == 'BROKER':
                    if 'broker_res_time' not in self.wiki_dict_:
                        self.wiki_dict_['broker_res_time'] = [datetime.datetime.now()]
                    else:
                        broker_time_list = list(self.wiki_dict_['broker_res_time'])
                        broker_time_list.append(datetime.datetime.now())
                        self.wiki_dict_['broker_res_time'] = broker_time_list
                if data['type'] == 'ORDER':
                    if 'level_order_res_time' not in self.wiki_dict_:
                        self.wiki_dict_['level_order_res_time'] = [datetime.datetime.now()]
                    else:
                        order_time_list = list(self.wiki_dict_['level_order_res_time'])
                        order_time_list.append(datetime.datetime.now())
                        self.wiki_dict_['level_order_res_time'] = order_time_list
                ##################################

        except Exception as exp:
            self.wiki_dict_['is_quote_right'] = 'False'
            print('on-message-cls error:{}'.format(exp))
            ## -- 把行情推送中触发的异常写入log文档中[正常/失败]
            pass

    def _on_error(self, ws, error) -> None:
        # ws异常回报--把接口触发的异常写入log文档中[正常/失败]
        self.wiki_dict_['is_api_error'] = 'True'
        print(f'ws异常回报，error:{error}')

    def _on_close(self, ws, close_status_code, close_msg) -> None:
        # ws关闭连接(closed)回报
        self.wiki_dict_['is_api_abnormal_close'] = 'True'
        print(f"ws已关闭>> close_msg:{close_msg}!")
    ...
    def _on_open(self, ws) -> None:
        ...
        def run(*args):
            time.sleep(1)
            self.req_id += 1
            sub_bstring = kprotoc.get_subscribe_bstring(self.subscribe_code_list, self.req_id)
            self._market_ws.sock.send_binary(sub_bstring)
            time.sleep(1)
        try:
            run()
            th = threading.Thread(target=self._market_ping, args=())
            th.start()
            self.wiki_dict_['is_sub'] = 'True'
        except Exception as exp:
            ## -- 把每次订阅的行为结果写入log文档中[正常/失败]
            print('on-open--error:{}'.format(exp))
            self.wiki_dict_['is_sub'] = 'False'
            pass
    ...
    def __market_connect_ws(self):
        ...
        try:
            ...
            #self._market_ws.on_open = self._on_open   # # ws open-ping
            self._market_connect_msg = ""

            try:
                self.wiki_dict_['is_init_api'] = 'True'
                self._market_ws.run_forever()
            except Exception as exp:
                ## --把每次连接行情接口的操作结果写入log文档中[正常/失败]
                print('__market_connect_ws:exp:{}'.format(exp))
                self.wiki_dict_['is_init_api'] = 'False'
                pass
            ######################new++++++++++++++++++
            ...
            # print('ws连接状态:{}'.format(self._market_ws.status))
            # print('ws连接connected:{}'.format(self._market_ws.connected))
            ...
            # self._market_connect_status = self._market_ws.connected
            # self._market_connect_msg = ""
            # time.sleep(5)
            # ret_status, ret_data = self.get_market_connect_ws_status()
            # if ret_status:
            #     print("market-ws connected!")
            # else:
            #     print('market-ws disconnected, error:{}'.format(ret_data))
            # while True:
            #     print('_market_ws--> recv >> ')
            #     text = self._market_ws.recv()              # 接口实时返回的字符串
            #     print('__market_connect_ws >> text:{}'.format(text))
            ...
            ## --把每次连接行情接口的操作结果写入log文档中[正常/失败]
            print('WebSocket Connection Status:{}'.format(self._market_ws.sock))
            print('market-ws disconnected, error:{}'.format(exp))
            self.wiki_dict_['is_init_api'] = 'False'
    ...
    def subscribe_marketdata(self, code_list):
        ...
        sub_types = [data['sub_type'] for data in code_list]
        sub_types = list(set(sub_types))
        if QUOTE in sub_types:
            self.wiki_dict_['quote_res_time'] = [datetime.datetime.now()]  # time.time()

        if TICK in sub_types:
            self.wiki_dict_['tick_res_time'] = [datetime.datetime.now()]

        if LEVEL_ORDER in sub_types:
            self.wiki_dict_['level_order_res_time'] = [datetime.datetime.now()]

        if BROKERS in sub_types:
            self.wiki_dict_['broker_res_time'] = [datetime.datetime.now()]

        self.subscribe_code_list = code_list
        self.req_id += 1
        self.market_connect_ws0()
    ...
    def unsubscribe_marketdata(self, code_list):
        ...
        try:
            ...
            self._market_ws.sock.send_binary(bstring)
            self.wiki_dict_['is_un_sub'] = 'True'
        except Exception as exp:
            ## --把每次退订行情的操作结果写入log文档中[正常/失败]
            self.wiki_dict_['is_un_sub'] = 'False'
            pass
        self.wiki_dict_['update_time'] = datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    ...
    def market_heartbeat(self):
        ...
        self._market_ws.sock.send_binary(ping_bstring)
