...
class AuthenticationTestCase(KaldelaTestCase):
...
class ManagementCommandsTestCase(TestCase):
    ...
    def test_change_default_admin(self):
        ...
        management.call_command(
            "load_initial_data", "--admin-username", "kaldelaadmin")
        self.assertTrue(
            self.client.login(username="kaldelaadmin", password="password"))
...
class ProfileTestCase(KaldelaTestCase):
...
class APIAccessFormTestCase(KaldelaTestCase):
...
class APICommunicationTestCase(KaldelaTestCase):
    """Check communication with the API."""

    def test_management_command(self):
        """Check command."""
        with httmock.HTTMock(
                mocks.kaldela_api_instance_search,
                mocks.kaldela_api_instance_create,
                mocks.kaldela_api_instance_update,
                mocks.kaldela_api_versions):
        ...
        with httmock.HTTMock(
                ...
                mocks.kaldela_api_instance_create,
                mocks.kaldela_api_instance_update,
                mocks.kaldela_api_versions_no_update):
            management.call_command("communicate_with_public_api")
        self.assertEqual(len(mail.outbox), 1)
