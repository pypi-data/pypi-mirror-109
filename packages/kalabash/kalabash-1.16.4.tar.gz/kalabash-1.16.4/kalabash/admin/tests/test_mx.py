...
from kalabash.lib.tests import KaldelaTestCase
from . import utils
from .. import factories, models
from ..lib import get_domain_mx_list


class MXTestCase(KaldelaTestCase):
    ...
    def test_management_command(
        ...
        management.call_command("kaldela", "check_mx", "--no-dnsbl", "--ttl=0")
        self.assertTrue(
        ...
        id_ = qs[0].id
        management.call_command("kaldela", "check_mx", "--no-dnsbl", "--ttl=7200")
        qs = models.MXRecord.objects.filter(domain=self.domain)
        ...
        # ttl
        management.call_command("kaldela", "check_mx", "--no-dnsbl")
        qs = models.MXRecord.objects.filter(domain=self.domain)
    ...
    def test_single_domain_update(
        ...
        management.call_command(
            "kaldela", "check_mx", "--domain", self.domain.name)
        self.assertTrue(
            models.MXRecord.objects.filter(domain=self.domain).exists())
        self.assertFalse(
        ...
        management.call_command(
            "kaldela", "check_mx", "--domain", str(self.bad_domain.pk))
        self.assertFalse(
            models.MXRecord.objects.filter(domain=self.bad_domain).exists())
        self.assertEqual(len(mail.outbox), 1)

        management.call_command(
            "kaldela", "check_mx", "--domain", "toto.com")
    ...
    def test_invalid_mx(
        ...
        management.call_command(
            "kaldela", "check_mx", "--domain", domain.name, "--no-dnsbl")
...
class DNSBLTestCase(KaldelaTestCase):
    """TestCase for DNSBL related features."""
    ...
    def test_management_command(
        ...
        management.call_command("kaldela", "check_mx")
        self.assertTrue(
    ...
    def test_notifications(
        ...
        mock_g_gethostbyname.return_value = "1.2.3.4"
        management.call_command(
            "kaldela", "check_mx", "--email", "user@example.test")
...
class DNSChecksTestCase(KaldelaTestCase):
