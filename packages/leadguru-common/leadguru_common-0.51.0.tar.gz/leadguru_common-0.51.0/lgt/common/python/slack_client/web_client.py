...
@dataclass()
class SlackCredentialsResponse:
    token: Optional[str]
...
def get_slack_credentials(user: UserModel, route: LeadModel) -> Optional[SlackCredentialsResponse]:
    """

    :rtype: UserBotCredentialsModel
    """

    if hasattr(route.message, "dedicated_options") and route.message.dedicated_options:
        return SlackCredentialsResponse(
            token=route.message.dedicated_options["token"],
            cookies=route.message.dedicated_options["cookies"],
            invalid_creds=False,
            slack_url=route.message.dedicated_options["slack_url"],
            user_name=None,
            password=None,
