...
from lgt_data.mongo_repository import UserBotCredentialsMongoRepository, DedicatedBotRepository
...
def get_dedicated_bot_credentials(user: UserModel, lead: LeadModel, update_expired=True) -> Optional[SlackCredentialsResponse]:
    bot = next([bot for bot in DedicatedBotRepository().get_user_bots(user.id) if bot.name == lead.message.name], None)
