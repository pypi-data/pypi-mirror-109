...
class LoadChatHistoryJob(BaseBackgroundJob, ABC):
    ...
    def _update_history(bots_map: dict, user: UserModel, lead: UserLeadModel) -> Optional[SlackHistoryMessageModel]:
        ...
        creds = lead.get_dedicated_credentials() if lead.is_dedicated_lead() else bots_map.get(lead.message.name)
        if not lead.is_dedicated_lead() and (not creds or creds.invalid_creds):
            return None
