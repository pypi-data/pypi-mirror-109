...
def retrieve_and_refresh_certification_authority_key(
    ...
    try:
        ...
        raise RuntimeError(
            f"an error occurred while trying to load the key "
...
def retrieve_refresh_and_validate_keys(
    ...
    for key_search_term in key_search_terms:
        ...
        # Refresh key if it was present in the local keyring.
        if len(keys) == 1:
            (key,) = refresh_keys(
                keys=keys,
                gpg_store=gpg_store,
        ...
        # If no local key is found, try to download it from a keyserver.
            ...
            if not keyserver_url:
                raise RuntimeError(
                    f"Key [{key_search_term}] is not available in the local "
                    f"keyring and no keyserver URL is provided."
                )
            key = download_key(
                key_search_term,
                gpg_store,
...
def validate_pub_key(
    ...
    if signee_key:
        assert_key_is_signed(key, signee_key, sig_class="13x")
    if keyserver_url:
        assert_keyserver_origin_valid(key=key, keyserver=keyserver_url)


def assert_key_is_signed(
    key: gpg.Key, signee_key: gpg.Key, sig_class: str = "13x"
) -> None:
    ...
    signee_key_id = signee_key.key_id
    ...
    # verify, an issue is raised.
    if not any(
        (
            sig.issuer_key_id == signee_key_id and sig.signature_class == sig_class
