...
def status(run: Optional[str] = None, details: Optional[bool] = False, export: Optional[str] = None) -> None:
    ...
    client.status(kind=kind, identifiers=observable, export=export)

    # If we have a Run, then don't print the global
    # interactive nodes table.
    if not run:
        client.status(kind=ObservableType.INTERACTIVE, identifiers=observable, export=export)
