...
def register_localstack_plugins():
 ...
 if HUlNn([is_api_enabled(api)for api in EXTERNAL_PORT_APIS]):
...
def load_plugin_files():
 try:
  ...
  if 'No module named' not in HUlNa(e):
   print('ERROR: %s'%e)
...
def api_key_configured():
 return HUlNc(os.environ.get('LOCALSTACK_API_KEY'))
