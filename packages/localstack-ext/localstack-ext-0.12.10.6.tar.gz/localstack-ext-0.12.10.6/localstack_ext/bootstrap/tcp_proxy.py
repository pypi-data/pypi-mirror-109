...
class AttrDict(pNHhk):
 def __init__(self,*args,**kwargs):
  pNHhI(AttrDict,self).__init__(*args,**kwargs)
  self.__dict__=self
def tcp_listen(six,addr,port,blk):
 s=socket.socket(AF_INET6 if six else AF_INET,SOCK_STREAM)
 if six and pNHhO(socket,'IPV6_V6ONLY'):
...
class Half(pNHhF):
 def __init__(self,opt,sock,addr,pNHhY):
  self.opt=opt
  self.sock=sock
  self.addr=addr
  self.pNHhY=pNHhY
  self.name='peer' if self.pNHhY else 'client'
  self.queue=[]
  self.dest=pNHha
  self.err=pNHha
  self.ready=pNHhP
 ...
 while qs:
  ...
  for q in qs:
   if q.post_wait(r,w,e):
    qs.remove(q)
def main():
 opt={'ip6':pNHhP,'bind_port':2223,'addr':'192.168.43.191','port':22}
 opt={'ip6':pNHhP,'bind_port':48429,'addr':'128.0.100.3','port':22}
 opt={'ip6':pNHhP,'port':54237,'bind_addr':'128.0.100.2','bind_port':22}
 opt={'ip6':pNHhP,'port':54237,'bind_addr':'192.168.100.3','bind_port':22}
 server_loop(opt)
