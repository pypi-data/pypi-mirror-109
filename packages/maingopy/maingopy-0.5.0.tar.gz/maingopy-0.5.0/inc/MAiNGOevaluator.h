...
class MaingoEvaluator {
  ...
  public:
	...
	*/
    ...
    {
        return dispatch(node->get_child<0>())[dispatch(node->get_child<1>()) - 1];
    }
    ...
    Var operator()(xlog_sum_node* node)
    {
        ...
        if (node->children.size() < 2) {
            throw MAiNGOException("  Error: MaingoEvaluator -- Called xlog_sum with less than arguments");
        }
        std::vector<Var> vars;
        std::vector<double> coeff;
        for (auto it = node->children.begin(); it != node->children.end(); ++it) {
    ...
    Var operator()(min_node* node)
    {
        ...
        for (; it != node->children.end(); ++it) {
            result = mc::min(dispatch(it->get()), result);
        }
        return result;
    }
    ...
    Var operator()(max_node* node)
    {
        ...
        it++;
        for (; it != node->children.end(); ++it) {
            result = mc::max(dispatch(it->get()), result);
        }
        return result;
    }
    ...
    Var operator()(set_min_node<TType>* node)
    {
        ...
        for (; it != elements.end(); ++it) {
            _symbols.define(node->name, new parameter_symbol<TType>(node->name, *it));
            result = mc::min(dispatch(node->template get_child<1>()), result);
        }
        _symbols.pop_scope();
        return result;
    }
    ...
    Var operator()(set_max_node<TType>* node)
    {
        ...
        for (; it != elements.end(); ++it) {
            _symbols.define(node->name, new parameter_symbol<TType>(node->name, *it));
            result = mc::max(dispatch(node->template get_child<1>()), result);
        }
        _symbols.pop_scope();
        return result;
    }
    ...
    {
        return mc::lmtd(dispatch(node->get_child<0>()), dispatch(node->get_child<1>()));
    }
    ...
    Var operator()(xexpax_node* node)
    {
        ...
        return mc::xexpax(dispatch(node->get_child<0>()), dispatch(node->get_child<1>()).num().val());
    }
    ...
    Var operator()(arh_node* node)
    {
        ...
        return mc::Op<mc::FFVar>::arh(dispatch(node->get_child<0>()), dispatch(node->get_child<1>()).num().val());
    }
    ...
    Var operator()(lb_func_node* node)
    {
        ...
        return mc::lb_func(dispatch(node->get_child<0>()), dispatch(node->get_child<1>()).num().val());
    }
    ...
    Var operator()(ub_func_node* node)
    {
        ...
        return mc::ub_func(dispatch(node->get_child<0>()), dispatch(node->get_child<1>()).num().val());
    }
    ...
    Var operator()(bounding_func_node* node)
    {
        ...
        return mc::bounding_func(dispatch(node->get_child<0>()), dispatch(node->get_child<1>()).num().val(), dispatch(node->get_child<2>()).num().val());
    }
    ...
    Var operator()(nrtl_dtau_node* node)
    {
        ...
        return mc::nrtl_dtau(dispatch(node->get_child<0>()), dispatch(node->get_child<1>()).num().val(), dispatch(node->get_child<2>()).num().val(),
    ...
    Var operator()(ext_antoine_psat_node* node)
    {
        ...
        return mc::vapor_pressure(dispatch(node->get_child<0>()), 1, dispatch(node->get_child<1>()).num().val(), dispatch(node->get_child<2>()).num().val(),
                                  dispatch(node->get_child<3>()).num().val(), dispatch(node->get_child<4>()).num().val(), dispatch(node->get_child<5>()).num().val(),
                                  dispatch(node->get_child<6>()).num().val(), dispatch(node->get_child<7>()).num().val());
    }
    ...
    Var operator()(antoine_psat_node* node)
    {
        ...
        // antoine_psat = type 2
        return mc::vapor_pressure(dispatch(node->get_child<0>()), 2, dispatch(node->get_child<1>()).num().val(), dispatch(node->get_child<2>()).num().val(),
                                  dispatch(node->get_child<3>()).num().val());
    }
    ...
    Var operator()(wagner_psat_node* node)
    {
        ...
        // wagner_psat = type 3
        return mc::vapor_pressure(dispatch(node->get_child<0>()), 3, dispatch(node->get_child<1>()).num().val(), dispatch(node->get_child<2>()).num().val(),
                                  dispatch(node->get_child<3>()).num().val(), dispatch(node->get_child<4>()).num().val(), dispatch(node->get_child<5>()).num().val(),
    ...
    Var operator()(ik_cape_psat_node* node)
    {
        ...
        // ik_cape_psat = type 4
        return mc::vapor_pressure(dispatch(node->get_child<0>()), 4, dispatch(node->get_child<1>()).num().val(), dispatch(node->get_child<2>()).num().val(),
                                  dispatch(node->get_child<3>()).num().val(), dispatch(node->get_child<4>()).num().val(), dispatch(node->get_child<5>()).num().val(),
                                  dispatch(node->get_child<6>()).num().val(), dispatch(node->get_child<7>()).num().val(), dispatch(node->get_child<8>()).num().val(),
                                  dispatch(node->get_child<9>()).num().val(), dispatch(node->get_child<10>()).num().val());
    }
    ...
    Var operator()(aspen_hig_node* node)
    {
        ...
        // aspen_hig = type 1
        return mc::ideal_gas_enthalpy(dispatch(node->get_child<0>()), dispatch(node->get_child<1>()).num().val(), 1, dispatch(node->get_child<2>()).num().val(),
                                      dispatch(node->get_child<3>()).num().val(), dispatch(node->get_child<4>()).num().val(), dispatch(node->get_child<5>()).num().val(),
                                      dispatch(node->get_child<6>()).num().val(), dispatch(node->get_child<7>()).num().val());
    }
    ...
    Var operator()(nasa9_hig_node* node)
    {
        ...
        // nasa9_hig = type 2
        return mc::ideal_gas_enthalpy(dispatch(node->get_child<0>()), dispatch(node->get_child<1>()).num().val(), 2, dispatch(node->get_child<2>()).num().val(),
                                      dispatch(node->get_child<3>()).num().val(), dispatch(node->get_child<4>()).num().val(), dispatch(node->get_child<5>()).num().val(),
                                      dispatch(node->get_child<6>()).num().val(), dispatch(node->get_child<7>()).num().val(), dispatch(node->get_child<8>()).num().val());
    }
    ...
    Var operator()(dippr107_hig_node* node)
    {
        ...
        return mc::ideal_gas_enthalpy(dispatch(node->get_child<0>()), dispatch(node->get_child<1>()).num().val(), 3, dispatch(node->get_child<2>()).num().val(),
                                      dispatch(node->get_child<3>()).num().val(), dispatch(node->get_child<4>()).num().val(), dispatch(node->get_child<5>()).num().val(),
                                      dispatch(node->get_child<6>()).num().val());
    }
    ...
    Var operator()(dippr127_hig_node* node)
    {
        ...
        // dippr127_hig = type 4
        return mc::ideal_gas_enthalpy(dispatch(node->get_child<0>()), dispatch(node->get_child<1>()).num().val(), 4, dispatch(node->get_child<2>()).num().val(),
                                      dispatch(node->get_child<3>()).num().val(), dispatch(node->get_child<4>()).num().val(), dispatch(node->get_child<5>()).num().val(),
                                      dispatch(node->get_child<6>()).num().val(), dispatch(node->get_child<7>()).num().val(), dispatch(node->get_child<8>()).num().val());
    }
    ...
    Var operator()(antoine_tsat_node* node)
    {
        ...
        // antoine_tsat = type 2
        return mc::saturation_temperature(dispatch(node->get_child<0>()), 2, dispatch(node->get_child<1>()).num().val(), dispatch(node->get_child<2>()).num().val(),
                                          dispatch(node->get_child<3>()).num().val());
    }
    ...
    Var operator()(watson_dhvap_node* node)
    {
        ...
        // watson_dhvap = type 1
        return mc::enthalpy_of_vaporization(dispatch(node->get_child<0>()), 1, dispatch(node->get_child<1>()).num().val(), dispatch(node->get_child<2>()).num().val(),
                                            dispatch(node->get_child<3>()).num().val(), dispatch(node->get_child<4>()).num().val(), dispatch(node->get_child<5>()).num().val());
    }
    ...
    Var operator()(dippr106_dhvap_node* node)
    {
        ...
        return mc::enthalpy_of_vaporization(dispatch(node->get_child<0>()), 2, dispatch(node->get_child<1>()).num().val(), dispatch(node->get_child<2>()).num().val(),
                                            dispatch(node->get_child<3>()).num().val(), dispatch(node->get_child<4>()).num().val(), dispatch(node->get_child<5>()).num().val(),
                                            dispatch(node->get_child<6>()).num().val());
    }
    ...
    Var operator()(cost_turton_node* node)
    {
        ...
        // cost_turton = type 1
        return mc::cost_function(dispatch(node->get_child<0>()), 1, dispatch(node->get_child<1>()).num().val(), dispatch(node->get_child<2>()).num().val(),
                                 dispatch(node->get_child<3>()).num().val());
    }
    ...
    Var operator()(covar_matern_1_node* node)
    {
        ...
        return mc::covariance_function(dispatch(node->get_child<0>()), 1);
    }

    Var operator()(covar_matern_3_node* node)
    {
        // covar_matern_3 = type 1
        return mc::covariance_function(dispatch(node->get_child<0>()), 2);
    }

    Var operator()(covar_matern_5_node* node)
    {
        // covar_matern_5 = type 1
        return mc::covariance_function(dispatch(node->get_child<0>()), 3);
    }

    Var operator()(covar_sqrexp_node* node)
    {
        // covar_sqrexp = type 1
        return mc::covariance_function(dispatch(node->get_child<0>()), 4);
    }
    ...
    Var operator()(nrtl_tau_node* node)
    {
        ...
        return mc::nrtl_tau(dispatch(node->get_child<0>()), dispatch(node->get_child<1>()).num().val(), dispatch(node->get_child<2>()).num().val(),
                            dispatch(node->get_child<3>()).num().val(), dispatch(node->get_child<4>()).num().val());
    }
    ...
    Var operator()(nrtl_g_node* node)
    {
        ...
        return mc::nrtl_G(dispatch(node->get_child<0>()), dispatch(node->get_child<1>()).num().val(), dispatch(node->get_child<2>()).num().val(),
                          dispatch(node->get_child<3>()).num().val(), dispatch(node->get_child<4>()).num().val(), dispatch(node->get_child<5>()).num().val());
    }
    ...
    Var operator()(nrtl_gtau_node* node)
    {
        ...
        return mc::nrtl_Gtau(dispatch(node->get_child<0>()), dispatch(node->get_child<1>()).num().val(), dispatch(node->get_child<2>()).num().val(),
                             dispatch(node->get_child<3>()).num().val(), dispatch(node->get_child<4>()).num().val(), dispatch(node->get_child<5>()).num().val());
    }
    ...
    Var operator()(nrtl_gdtau_node* node)
    {
        ...
        return mc::nrtl_Gdtau(dispatch(node->get_child<0>()), dispatch(node->get_child<1>()).num().val(), dispatch(node->get_child<2>()).num().val(),
                              dispatch(node->get_child<3>()).num().val(), dispatch(node->get_child<4>()).num().val(), dispatch(node->get_child<5>()).num().val());
    }
    ...
    Var operator()(nrtl_dgtau_node* node)
    {
        ...
        return mc::nrtl_dGtau(dispatch(node->get_child<0>()), dispatch(node->get_child<1>()).num().val(), dispatch(node->get_child<2>()).num().val(),
                              dispatch(node->get_child<3>()).num().val(), dispatch(node->get_child<4>()).num().val(), dispatch(node->get_child<5>()).num().val());
    }
    ...
    {
        return mc::euclidean_norm_2d(dispatch(node->get_child<0>()), dispatch(node->get_child<1>()));
    }
    ...
    {
        return mc::rlmtd(dispatch(node->get_child<0>()), dispatch(node->get_child<1>()));
    }


    Var operator()(xexpy_node* node)
    {
        return mc::expx_times_y(dispatch(node->get_child<1>()), dispatch(node->get_child<0>()));
    }
