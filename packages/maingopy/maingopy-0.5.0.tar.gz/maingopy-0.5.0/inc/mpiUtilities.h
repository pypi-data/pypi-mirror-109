...
send_babnode(const babBase::BabNode &node, const int dest)
{
    ...
    int hI                 = node.holds_incumbent() ? 1 : 0;
    ...
    MPI_Ssend(&hI, 1, MPI_INT, dest, TAG_NODE_HAS_INCUMBENT, MPI_COMM_WORLD);
...
recv_babnode(babBase::BabNode &node, const int source, const unsigned nvar)
{
    ...
    MPI_Recv(&hI, 1, MPI_INT, source, TAG_NODE_HAS_INCUMBENT, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
    MPI_Recv(&depth, 1, MPI_INT, source, TAG_NODE_DEPTH, MPI_COMM_WORLD, MPI_STATUS_IGNORE);

    node = babBase::BabNode(pruningScore, lb, ub, id, depth, hI == 1);
}
