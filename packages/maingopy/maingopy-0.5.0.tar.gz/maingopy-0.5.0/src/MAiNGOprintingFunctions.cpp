...
MAiNGO::_print_info_about_initial_point()
{
	if (_initialPointOriginal.empty()) {
		...
		outstream << std::endl << "  No initial point given." << std::endl << std::endl;
		_logger->print_message(outstream.str(), _maingoSettings->BAB_verbosity, VERB_ALL, _maingoSettings->loggingDestination);
		return;
	}
	if (_initialPointOriginal.size() != _originalVariables.size()) {
		std::ostringstream outstream;
		outstream << std::endl << "  WARNING: Initial point discarded because its dimension (" << _initialPointOriginal.size() << ")";
		outstream << "does not match the number of variables (" << _originalVariables.size() << ")." << std::endl << std::endl;
		_logger->print_message(outstream.str(), _maingoSettings->BAB_verbosity, VERB_ALL, _maingoSettings->loggingDestination);
		return;
	}
    ...
    // Variables:
    outstream << std::endl << "  Initial point:" << std::endl;
...
MAiNGO::_print_third_party_software_minlp()
{
    ...
    if (((_maingoSettings->UBP_solverPreprocessing != ubp::UBP_SOLVER_EVAL) && (_maingoSettings->UBP_solverPreprocessing != ubp::UBP_SOLVER_COBYLA) && (_maingoSettings->UBP_solverPreprocessing != ubp::UBP_SOLVER_BOBYQA))
        || ((_maingoSettings->UBP_solverBab != ubp::UBP_SOLVER_EVAL) && (_maingoSettings->UBP_solverBab != ubp::UBP_SOLVER_COBYLA) && (_maingoSettings->UBP_solverBab != ubp::UBP_SOLVER_BOBYQA))) {
        _logger->print_message("    - FADBAD++ by O. Stauning and C. Bendtsen (automatic differentiation)\n", _maingoSettings->BAB_verbosity, VERB_NORMAL, _maingoSettings->loggingDestination);
    }
    if ( (_maingoSettings->UBP_solverPreprocessing == ubp::UBP_SOLVER_IPOPT) || (_maingoSettings->UBP_solverBab == ubp::UBP_SOLVER_IPOPT) ||
    ...
    // UBP solvers:
    if ( (_maingoSettings->UBP_solverBab == ubp::UBP_SOLVER_COBYLA) || (_maingoSettings->UBP_solverPreprocessing == ubp::UBP_SOLVER_COBYLA) ) {
        _logger->print_message("    - COBYLA by M.J.D. Powell implemented in NLopt by S.G. Johnson (local NLP solver)\n", _maingoSettings->BAB_verbosity, VERB_NORMAL, _maingoSettings->loggingDestination);
    }
    if ( (_maingoSettings->UBP_solverBab == ubp::UBP_SOLVER_BOBYQA) || (_maingoSettings->UBP_solverPreprocessing == ubp::UBP_SOLVER_BOBYQA) ) {
        _logger->print_message("    - BOBYQA by M.J.D. Powell implemented in NLopt by S.G. Johnson (local NLP solver)\n", _maingoSettings->BAB_verbosity, VERB_NORMAL, _maingoSettings->loggingDestination);
    }
    if ( (_maingoSettings->UBP_solverBab == ubp::UBP_SOLVER_LBFGS) || (_maingoSettings->UBP_solverPreprocessing == ubp::UBP_SOLVER_LBFGS) ) {
        _logger->print_message("    - L-BFGS by L. Luksan implemented in NLopt by S.G. Johnson (local NLP solver)\n", _maingoSettings->BAB_verbosity, VERB_NORMAL, _maingoSettings->loggingDestination);
    }
    if ( (_maingoSettings->UBP_solverBab == ubp::UBP_SOLVER_SLSQP) || (_maingoSettings->UBP_solverPreprocessing == ubp::UBP_SOLVER_SLSQP) ) {
        _logger->print_message("    - SLSQP by D. Kraft implemented in NLopt by S.G. Johnson (local NLP solver)\n", _maingoSettings->BAB_verbosity, VERB_NORMAL, _maingoSettings->loggingDestination);
    }
    if ( (_maingoSettings->UBP_solverBab == ubp::UBP_SOLVER_IPOPT) || (_maingoSettings->UBP_solverPreprocessing == ubp::UBP_SOLVER_IPOPT) ) {
        _logger->print_message("    - IPOPT by A. Waechter and L.T. Biegler (local NLP solver)\n", _maingoSettings->BAB_verbosity, VERB_NORMAL, _maingoSettings->loggingDestination);
    }
    if ( (_maingoSettings->UBP_solverBab == ubp::UBP_SOLVER_KNITRO) || (_maingoSettings->UBP_solverPreprocessing == ubp::UBP_SOLVER_KNITRO) ) {
        _logger->print_message("    - Artelys KNITRO by R.H. Byrd, J. Nocedal, and R.A. Waltz (local NLP solver)\n", _maingoSettings->BAB_verbosity, VERB_NORMAL, _maingoSettings->loggingDestination);
    }

    // LBP solvers:
    if (_maingoSettings->LBP_solver == lbp::LBP_SOLVER_CPLEX) {
        _logger->print_message("    - IBM CPLEX (LP solver)\n", _maingoSettings->BAB_verbosity, VERB_NORMAL, _maingoSettings->loggingDestination);
    }
    if (_maingoSettings->LBP_solver == lbp::LBP_SOLVER_CLP) {
