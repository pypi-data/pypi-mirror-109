...
LbpCplex::_check_infeasibility(const babBase::BabNode &currentNode)
{
    ...
    if (reallyInfeasible) {
        ...
        if (res1 - res2 <= 0. && !mc::isequal(res1, res2, _computationTol * 1e1, _computationTol * 1e1)) {
            ...
            std::string str = "lpex_infeas_check";
    ...
    for (unsigned int i = 0; i < _nineq; i++) {
        for (unsigned int k = 0; k < _nLinIneq[i]; k++) {
            ...
            if (value - _rhsIneq[i][k] > _maingoSettings->deltaIneq) {
                if (_maingoSettings->LBP_verbosity > VERB_NONE) {
                    std::ostringstream outstr;
                    ...
                    if (_maingoSettings->LBP_verbosity > VERB_NORMAL) {
                        outstr << std::setprecision(16) << "           value: " << value << " _rhsIneq[" << i << "][" << k << "]: " << _rhsIneq[i][k] << std::endl;
    ...
    for (unsigned int i = 0; i < _neq; i++) {
        for (unsigned int k = 0; k < _nLinEq[i]; k++) {
            ...
            if (value - _rhsEq1[i][k] > _maingoSettings->deltaEq) {
                if (_maingoSettings->LBP_verbosity > VERB_NONE) {
                    std::ostringstream outstr;
                    outstr << "  Warning: CPLEX returned FEASIBLE although the point is an infeasible one w.r.t. equality " << i << "!" << std::endl;
                    if (_maingoSettings->LBP_verbosity > VERB_NORMAL) {
                        outstr << std::setprecision(16) << "           value: " << value << " _rhsEq1[" << i << "][" << k << "]: " << _rhsEq1[i][k] << std::endl;
                        outstr << "           LBP solution point: " << std::endl;
                        for (unsigned int i = 0; i < _nvar; i++) {
                            outstr << "            x(" << i << "): " << solution[i] << std::endl;
                        }
            ...
            if (value - _rhsEq2[i][k] > _maingoSettings->deltaEq) {
                if (_maingoSettings->LBP_verbosity > VERB_NONE) {
                    ...
                    outstr << "  Warning: CPLEX returned FEASIBLE although the point is an infeasible one w.r.t. equality " << i << "!" << std::endl;
                    if (_maingoSettings->LBP_verbosity > VERB_NORMAL) {
                        outstr << std::setprecision(16) << "           value: " << value << " _rhsEq2[" << i << "][" << k << "]: " << _rhsEq2[i][k] << std::endl;
                        outstr << "           LBP solution point: " << std::endl;
                        for (unsigned int i = 0; i < _nvar; i++) {
                            outstr << "            x(" << i << "): " << solution[i] << std::endl;
                        }
    ...
    for (unsigned int i = 0; i < _nineqRelaxationOnly; i++) {
        for (unsigned int k = 0; k < _nLinIneqRelaxationOnly[i]; k++) {
            ...
            if (value - _rhsIneqRelaxationOnly[i][k] > _maingoSettings->deltaIneq) {
                if (_maingoSettings->LBP_verbosity > VERB_NONE) {
                    std::ostringstream outstr;
                    outstr << "  Warning: CPLEX returned FEASIBLE although the point is an infeasible one w.r.t. inequality (rel only) " << i << "!" << std::endl;
                    if (_maingoSettings->LBP_verbosity > VERB_NORMAL) {
                        outstr << "           value: " << value << " _rhsIneqRelaxationOnly[" << i << "][" << k << "]: " << _rhsIneqRelaxationOnly[i][k] << std::endl;
                        outstr << "           LBP solution point: " << std::endl;
                        for (unsigned int i = 0; i < _nvar; i++) {
                            outstr << "            x(" << i << "): " << solution[i] << std::endl;
                        }
    ...
    for (unsigned int i = 0; i < _neqRelaxationOnly; i++) {
        for (unsigned int k = 0; k < _nLinEqRelaxationOnly[i]; k++) {
            ...
            if (value - _rhsEqRelaxationOnly1[i][k] > _maingoSettings->deltaEq) {
                if (_maingoSettings->LBP_verbosity > VERB_NONE) {
                    std::ostringstream outstr;
                    outstr << "  Warning: CPLEX returned FEASIBLE although the point is an infeasible one w.r.t. equality (rel only) " << i << "!" << std::endl;
                    if (_maingoSettings->LBP_verbosity > VERB_NORMAL) {
                        outstr << "           value: " << value << " _rhsEqRelaxationOnly1[" << i << "][" << k << "]: " << _rhsEqRelaxationOnly1[i][k] << std::endl;
                        outstr << "           LBP solution point: " << std::endl;
                        for (unsigned int i = 0; i < _nvar; i++) {
                            outstr << "            x(" << i << "): " << solution[i] << std::endl;
                        }
            ...
            if (value - _rhsEqRelaxationOnly2[i][k] > _maingoSettings->deltaEq) {
                if (_maingoSettings->LBP_verbosity > VERB_NONE) {
                    ...
                    outstr << "  Warning: CPLEX returned FEASIBLE although the point is an infeasible one w.r.t. equality (rel only) " << i << "!" << std::endl;
                    if (_maingoSettings->LBP_verbosity > VERB_NORMAL) {
                        outstr << "           value: " << value << " _rhsEqRelaxationOnly2[" << i << "][" << k << "]: " << _rhsEqRelaxationOnly2[i][k] << std::endl;
                        outstr << "           LBP solution point: " << std::endl;
                        for (unsigned int i = 0; i < _nvar; i++) {
                            outstr << "           x(" << i << "): " << solution[i] << std::endl;
                        }
    ...
    for (unsigned int i = 0; i < _nineqSquash; i++) {
        for (unsigned int k = 0; k < _nLinIneqSquash[i]; k++) {
            ...
            if (value - _rhsIneqSquash[i][k] > 1e-9) {
                if (_maingoSettings->LBP_verbosity > VERB_NONE) {
                    std::ostringstream outstr;
                    outstr << "  Warning: CPLEX returned FEASIBLE although the point is an infeasible one w.r.t. inequality (squash) " << i << "!" << std::endl;
                    if (_maingoSettings->LBP_verbosity > VERB_NORMAL) {
                        outstr << std::setprecision(16) << "           value: " << value << " _rhsIneqSquash[" << i << "][" << k << "]: " << _rhsIneqSquash[i][k] << std::endl;
                        outstr << "           LBP solution point: " << std::endl;
                        for (unsigned int i = 0; i < _nvar; i++) {
                            outstr << "            x(" << i << "): " << solution[i] << std::endl;
                        }
...
void
LbpCplex::_write_LP_to_file(std::string &fileName)
{
