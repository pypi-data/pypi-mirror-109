...
LINEARIZATION_RETCODE
LowerBoundingSolver::_linearize_model_at_incumbent(const std::vector<double> &lowerVarBounds, const std::vector<double> &upperVarBounds, const bool holdsIncumbent)
{
...
LowerBoundingSolver::_linearization_points_Kelley(const babBase::BabNode &currentNode)
{
    ...
    for (unsigned iLin = 1; iLin < _maxnParticipatingVariables; iLin++) {
        ...
        if (_LPstatus == LP_INFEASIBLE) {
            ...
            std::string str = "Kelley_not_optimal_or_infeas";
...
#ifdef LP__WRITE_CHECK_FILES
            std::string str = "Kelley_Simplex_not_optimal_or_infeas";
            _write_LP_to_file(str);
#endif
            break;
        }
