    ...
    parser(input, symbols)
{
    lex.reserve_keywords(
        {"definitions", "objective", "constraints", "outputs",
...
{
    while (!check(token::END) && !check_any_keyword("definitions", "objective", "constraints", "relaxation", "squashing", "outputs")) {
...
{
    while (current().type != token::END && !(current().type == token::KEYWORD && (current().lexeme == "definitions" || current().lexeme == "objective" || current().lexeme == "constraints" || current().lexeme == "relaxations" || current().lexeme == "squashing" || current().lexeme == "outputs"))) {
        consume();
    }
    buf.clear();
}
