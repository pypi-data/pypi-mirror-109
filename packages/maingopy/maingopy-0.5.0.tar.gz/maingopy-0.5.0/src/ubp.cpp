...
UpperBoundingSolver::solve(babBase::BabNode const &currentNode, double &objectiveValue, std::vector<double> &solutionPoint)
{
    ...
    if ((startingPointFeasible == SUBSOLVER_FEASIBLE) || (subsolverFoundFeasiblePoint == SUBSOLVER_FEASIBLE)) {
...
maingo::ubp::evaluate_objective(const double *currentPoint, const unsigned nvar, const bool computeGradient, double *gradient, std::shared_ptr<DagObj> dagObj)
{
    ...
    if (!computeGradient) {    // Derivative-free solver
        ...
        try {
            ...
            if (dagObj->maingoSettings->UBP_verbosity >= VERB_NORMAL && !dagObj->warningFlag) {
...
/////////////////////////////////////////////////////////////////////////
// method for evaluating the Hessian of the Lagranian
void
...
{
    for (unsigned int i = 0; i < _nvar; i++) {
        if ((currentPoint[i] > _originalUpperBounds[i])) {
            if (_maingoSettings->UBP_verbosity >= VERB_ALL) {
                std::ostringstream outstr;
        ...
        if ((currentPoint[i] < _originalLowerBounds[i])) {
            if (_maingoSettings->UBP_verbosity >= VERB_ALL) {
                std::ostringstream outstr;
