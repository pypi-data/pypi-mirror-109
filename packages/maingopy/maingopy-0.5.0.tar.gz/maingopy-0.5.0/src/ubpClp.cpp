...
UbpClp::_solve_nlp(const std::vector<double> &lowerVarBounds, const std::vector<double> &upperVarBounds, double &objectiveValue, std::vector<double> &solutionPoint)
{
    ...
    try {
        ...
        for (size_t i = 0; i < _constraintProperties->size(); i++) {
            ...
            switch ((*_constraintProperties)[i].type) {
                case OBJ:
                    _objectiveConstant = resultCoefficients[index].constant;
                    for (size_t k = 0; k < _nvar; k++) {
                        _objectiveCoeffs[k] = resultCoefficients[index].coeffsLin[k];
                    }
                    break;
                case INEQ:
                    _upperRowBounds[irow] = -resultCoefficients[index].constant;
                    for (size_t k = 0; k < _nvar; k++) {
                        constraintCoeffs[k * _numrows + irow] = resultCoefficients[index].coeffsLin[k];
                    }
                    irow++;
                    break;
                case EQ:
                    _lowerRowBounds[irow] = -resultCoefficients[index].constant;
                    _upperRowBounds[irow] = -resultCoefficients[index].constant;
                    for (size_t k = 0; k < _nvar; k++) {
                        constraintCoeffs[k * _numrows + irow] = resultCoefficients[index].coeffsLin[k];
                    }
