    ...
    UpperBoundingSolver(DAG, DAGvars, DAGfunctions, variables, nineqIn, neqIn, nineqSquashIn, settingsIn, loggerIn, constraintPropertiesIn, useIn)
{
    try {
        ...
        for (size_t i = 0; i < _constraintProperties->size(); i++) {
            ...
            for (size_t k = 0; k < resultCoefficients[index].nvar; k++) {
                expr += resultCoefficients[index].coeffsLin[k] * cplxVars[k];
                for (size_t j = 0; j < resultCoefficients[index].nvar; j++) {
                    expr += resultCoefficients[index].coeffsQuad[k][j] * cplxVars[k] * cplxVars[j];
                }
            }
