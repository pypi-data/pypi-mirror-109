...
def info():
    ...
    variables.pop('client_secret')

    labels = Column()
    values = Column()

    labels.add("CONTEXT NAME", "AUTH TOKEN", "CLIENT_ID",
               "ENDPOINT URL", "WORKSPACE ID")
    values.add(name, variables['auth_token'], variables['client_id'],
               variables['endpoint_url'], variables['workspace_id'])
