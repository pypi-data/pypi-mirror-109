...
class LicenseMessUsedModel(BaseCollectionMongoDB):
    ...
    def data_mess_to_string(self, data_mess):
        ...
        data_str += data_mess.get(self.month, "")
        data_str += str(data_mess.get(self.mess_base, 0))
        data_str += str(data_mess.get(self.mess_base_used, 0))
        data_str += str(data_mess.get(self.mess_increase, 0))
        data_str += str(data_mess.get(self.mess_increase_used, 0))
        data_str += str(data_mess.get(self.gift, 0))
        data_str += str(data_mess.get(self.gift_used, 0))
        data_str += str(data_mess.get(self.mess_surplus, 0))
        return data_str
