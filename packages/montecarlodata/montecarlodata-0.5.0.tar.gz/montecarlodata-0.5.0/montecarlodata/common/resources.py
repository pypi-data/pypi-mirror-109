...
class CloudResourceService:
    ...
    @manage_errors
    def list_emr_clusters(self, only_log_locations: Optional[bool] = False):
        """
        ...
        if only_log_locations:
            self._list_emr_clusters_only_log_locations()
        else:
            self._list_emr_cluster_details()

    def _list_emr_cluster_details(self):
        found = False
        for cluster in self._aws_wrapper.get_emr_cluster_details():
            if not cluster:  # Should never happen.
                continue
            found = True
            # Prints line by line to show progress as the list may be long.
            info = [[cluster['Name'], cluster['Id'], cluster['Status']['State'], cluster.get('LogUri')]]
            click.echo(tabulate(info, tablefmt='plain'))
        if not found:
            click.echo(f'No clusters found', err=True)

    def _list_emr_clusters_only_log_locations(self):
        click.echo('Listing log locations, it can take several minutes depending on the number of clusters', err=True)
        log_locations = set()
        found = False
        for cluster in self._aws_wrapper.get_emr_cluster_details():
            if not cluster:  # Should never happen.
