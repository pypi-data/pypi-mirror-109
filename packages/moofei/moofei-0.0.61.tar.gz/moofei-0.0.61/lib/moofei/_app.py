...
@valid.func(['path:str', 'page:int', 'bsize:int', 'words:','not_word:', 'pre_word:', 'fpath:', 'fname:', 'file:', 'curpage:int' ])
def pageView(path, page, bsize, words='', not_word="", pre_word="", fpath="", fname="", file="", curpage=0):
    path = path or fpath or fname or file
    if curpage: page=curpage-1
    elif not page: page=0
    data = _cat(path, page=page, bsize=bsize, words=words, not_word=not_word, pre_word=pre_word) 
    if data:
        txts, inlines = data
        rs = inlines[2]
