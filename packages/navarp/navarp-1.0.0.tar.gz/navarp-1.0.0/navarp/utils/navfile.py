...
class NavEntry:
    ...
    def init_ebins(self):
        """Initialize the fermi level (efermi) and the binding enrgies (ebins).
        """
    ...
    def isok(
        ...
        if self.valid_kspace:
            ...
            raise AttributeError(
                'No valid kspace, do set_kspace() first')
...
def load_nxarpes(h5f, file_path):
    ...
    h5fdic = {
        ...
        't_frame': "entry1/instrument/analyser/time_for_frames",
...
def load_mbs_krx(file_path):
    ...
    # analyzer
    analyzer = NavAnalyzer(tht_ap=50, phi_ap=0, work_fun=4.6, deflector=False)
    ...
    file_open.seek((pas[0]+(len_en*len_ang))*4 + 4)
...
def load_igorpro_itx(file_path):
    """Method for loading ARPES data from itx file
...
def load_navarp_yaml(file_path):
    ...
    if 'tht_an' in entry_info:
        entry_out.tht_an = entry_info['tht_an']
    else:
        entry_out.tht_an = None
