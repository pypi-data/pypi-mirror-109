...
from ...app.sent_classify.data_set import SentClassifyDataSet, SentClassifySample


class SentClassifyInfer(AbstractInfer):
    ...
    def load_model_config(self, model_config_file: str) -> SentClassifyExportedModelSettings:
        """
        读取配置文件
        :return:
        """
        ...
        try:
            settings = SentClassifyExportedModelSettings().parse_file(model_config_file)
    ...
    def create_data_pipe(self) -> SentClassifyDataSet:
        """
        创建数据处理pipe
        :return:
        """
        return SentClassifyDataSet(
...
def main():
    ...
    infer = SentClassifyInfer(model_path=args.release_path, model_config_file=args.config_file, gpu_idx=args.gpu_idx)
    ...
    while True:
        ...
        label, scores = infer.inference(data.queries[0])
        pred_scores = []
