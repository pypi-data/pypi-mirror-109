...
from ...app.doc_classify.data_set import DocClassifyDataSet, DocClassifySample
from ...app.doc_classify.settings import DocClassifyExportedModelSettings
from ...base.metrics import Metric


class DocClassifyInfer(AbstractInfer):
    def __init__(self, model_path: str, model_config_file: str, gpu_idx: int = -1):
        super().__init__(model_path, model_config_file, gpu_idx)

    def load_model_config(self, model_config_file: str) -> DocClassifyExportedModelSettings:
        """
        读取配置文件
        :return:
        """
        try:
            settings = DocClassifyExportedModelSettings().parse_file(model_config_file)
    ...
    def create_data_pipe(self) -> DocClassifyDataSet:
        """
        创建数据处理pipe
        :return:
        """
        return DocClassifyDataSet(
            labels=self.taggers[0],
            tokenizer=self.tokenizer,
            max_sentence=self.settings.max_sentences
        )

    def inference(self, title: str, content: List[str]) -> (str, torch.FloatTensor):
        """
        短文本分类
        :param title:
        :param content:
        ...
        sample = self.data_pipe.parse_sample(
            DocClassifySample(title=title, content=content, labels=[self.taggers[0].id2tag(0)]).json()
        )
        scores = self.model(
            self.set_tensor_device(torch.LongTensor([sample["data"]])),
            self.set_tensor_device(torch.LongTensor([sample["mask"]]))
        )
...
def main():
    ...
    infer = DocClassifyInfer(model_path=args.release_path, model_config_file=args.config_file, gpu_idx=args.gpu_idx)
    ...
    while True:
        ...
        if len(line) == 0:
            break
        data = DocClassifySample.parse_raw(line)
        y_true.append(data.labels[0])
        label, scores = infer.inference(title=data.title, content=data.content)
        pred_scores = []
