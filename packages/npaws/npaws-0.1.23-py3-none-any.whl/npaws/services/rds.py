...
class RDSClient():
    ...
    def check_preprocessed_pages(self,
        ...
        for page_id in page_ids:
            ...
            img_status.append(page['imgStatus'])
            ocr_status.append(page['ocrStatus'])
        all_preprocessed = ( all([s in ("COMPLETED", "FAILED") for s in img_status]) and
                             all([s in ("COMPLETED", "FAILED") for s in ocr_status]) )
        return all_preprocessed, img_status, ocr_status
    ...
    def get_preprocessed_pages(self,
        ...
        for page_id in page_ids:
            ...
            if page['tagged'] and page['imgStatus'] == "COMPLETED" and page['ocrStatus'] == "COMPLETED":
                query = f"SELECT classDefinition_id FROM neuralplatform_classification WHERE page_id = {page_id};"
