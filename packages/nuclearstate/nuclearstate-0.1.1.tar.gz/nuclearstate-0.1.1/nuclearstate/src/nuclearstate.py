...
def broadcast(event):
    ...
    elif mode == 'explicit' or mode == 'described':
        global eventReceipts
...
def startswith(text, sub):
    return text[:len(sub)] == sub

def removeend(text, toremove):
...
def scan():
    ...
    for obj in link:
        if isinstance(link[obj], Receiver):
            for func in dir(link[obj]):
                if startswith(func, eventFormat):
                    event_r(removeend(func, eventFormat), link[obj])
                elif startswith(func, messageFormat):
                    message_r(removeend(func, messageFormat), link[obj])
