...
class JaccountClient(WebApplicationClient):
    ...
    def get_authorize_url(self, redirect_url, **kwargs):
        ...
        url = self.prepare_authorization_request(
            authorization_url=authorization_url, redirect_url=redirect_url, **kwargs
        )
        state = self.state
        return url[0], state
    ...
    def get_logout_url(self, redirect_url):
        ...
        return "%s%s?%s" % (self.host, self.logout_path, params)
    ...
    def get_rest_api_url(self, access_token, path, **kwargs):
        params = urlencode(
            {"access_token": access_token, "client_id": self.client_id, **kwargs}
        )
        url = "%s%s?%s" % (self.api_host, path, params)
        return url
