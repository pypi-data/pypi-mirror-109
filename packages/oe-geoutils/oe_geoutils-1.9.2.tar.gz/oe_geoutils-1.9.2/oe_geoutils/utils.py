...
def convert_wktelement_to_geojson(wktelement):
    ...
    try:
        ...
        geojson_dict = geometry.mapping(geom)
        geojson_dict['precision'] = oe_geoutils.GEOJSON_PRECISION
        json_element = json.dumps(geojson_dict)
...
def convert_geojson_to_wktelement(value):
    ...
    if value is None or not value:
        return None
    try:
        value['precision'] = oe_geoutils.GEOJSON_PRECISION
        s = json.dumps(value)
        g1 = geojson.loads(s)
...
def transform_projection(shape, old_srid, new_srid):
    ...
    try:
        ...
        project = pyproj.Transformer.from_crs(
            old_srid, new_srid, always_xy=True
        ).transform
        return transform(project, shape)
    except Exception as e:
        raise ValueError(
            "Transformatie van projectiesysteem kan niet worden uitgevoerd "
            "tussen %s en %s" % (old_srid, new_srid),
            e
        )
...
def convert_geometry_to_geojson(value, srid):
    ...
    try:
        geojson_dict = geometry.mapping(value)
        geojson_dict['precision'] = oe_geoutils.GEOJSON_PRECISION
        json_value = json.dumps(geojson_dict)
        if srid is None or not value:
            return geojson.loads(json_value)
        else:
            return geojson.loads(
                json_value.rsplit('}', 1)[0]
                + ', "crs": {"type": "name", "properties": {"name": "' + epsg(srid, True)
                + '" }}}'
            )
    except Exception as e:
