...
from oe_geoutils.utils import remove_dupl_coords
...
class GeometryValidator:
    ...
    def __init__(self, node, gj, **kwargs):
        """
        ...
        self.node = node
        self.gj = gj
        self.check_flanders = kwargs.pop('check_flanders', True)
        if self.gj is None or not self.gj:
            raise colander.Invalid(self.node,
                                   "geometrie is verplicht en mag niet leeg zijn")
        try:
            self.geom = shape(self.gj)
        except ValueError:
            log.exception("Validatie geometry gefaald. Geojson: %s", self.gj)
...
class GeoJson(colander.SchemaType):
    ...
    def deserialize(self, node, cstruct):
        ...
        try:
            ...
            log.exception("Validatie geojson gefaald. Geojson: %s", cstruct)
...
class GeometrieSchemaNode(colander.SchemaNode):
    ...
    def validator(self, node, cstruct):
        return GeometryValidator(
            node, cstruct, check_flanders=self.check_flanders
        ).validate_multipolygon(self.max_area)


class PointGeometrieSchemaNode(colander.SchemaNode):
    def __init__(self, *args, **kwargs):
        self.check_flanders = kwargs.pop('check_flanders', True)
        super(PointGeometrieSchemaNode, self).__init__(*args, **kwargs)
...
class MultiPointGeometrieSchemaNode(colander.SchemaNode):
    def __init__(self, *args, **kwargs):
        self.check_flanders = kwargs.pop('check_flanders', True)
        super(MultiPointGeometrieSchemaNode, self).__init__(*args, **kwargs)
