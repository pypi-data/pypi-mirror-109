...
class ProvincieSchemaNode(colander.MappingSchema):
    naam = OEStrippedStringSchemaNode(
...
class DeelGemeenteSchemaNode(colander.MappingSchema):
    ...
    niscode = OEStrippedStringSchemaNode(
...
class LocationElementSchemaNode(colander.MappingSchema):
    ...
    type = OEStrippedStringSchemaNode(
    ...
    straat = OEStrippedStringSchemaNode(
    ...
    huisnummer = OEStrippedStringSchemaNode(
        validator=colander.Length(1, 255),
        missing=None
    )
    ...
    subadres = OEStrippedStringSchemaNode(
    ...
    postcode = OEStrippedStringSchemaNode(
        validator=colander.Length(1, 20),
        missing=None
    )

    land = OEStrippedStringSchemaNode(
        validator=colander.Length(1, 100),
        missing='BE'
    )
    ...
    omschrijving = OEStrippedStringSchemaNode(
        validator=colander.Length(1, 250),
        missing=None
    )
...
def _set_deelgemeente(locatie_element, gemeente, crab_gateway):
    input_deelgemeente = locatie_element.get('deelgemeente') or {}
    if input_deelgemeente.get('niscode') is not None:
        deelgemeente = crab_gateway.deelgemeenten.get(
            input_deelgemeente.get('niscode')
        )
        if deelgemeente:
            locatie_element['deelgemeente'] = {
                'niscode': deelgemeente['id'],
                'naam': deelgemeente['naam']
            }
    elif input_deelgemeente.get('naam') is not None and gemeente:
        for gem in crab_gateway.list_deelgemeenten_by_gemeente(gemeente):
            if gem.naam.lower() == (input_deelgemeente.get('naam') or '').lower():
                locatie_element['deelgemeente'] = {
                    'niscode': gem.id,
                    'naam': gem.naam
                }
