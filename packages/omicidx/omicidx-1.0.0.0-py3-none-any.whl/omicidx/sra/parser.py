...
def parse_study(xml):
    ...
    required_keys = [
        'abstract', 'BioProject', 'GEO', 'accession', 'alias', 'attributes',
        'center_name', 'broker_name', 'description', 'study_type',
...
def parse_experiment(xml):
    ...
    """
    required_keys = [
        'accession', 'attributes', 'alias', 'center_name', 'design',
        'description', 'experiment_accession', 'identifiers',
        'instrument_model', 'library_name', 'library_construction_protocol',
        'library_layout_orientation', 'library_layout_length',
        'library_layout_sdev', 'library_strategy', 'library_source',
        'library_selection', 'library_layout', 'xrefs', 'platform',
        'sample_accession', 'study_accession', 'title'
    ]
    ...
    path_map = {
        ...
        ('./DESIGN/LIBRARY_DESCRIPTOR/LIBRARY_LAYOUT/PAIRED', 'ORIENTATION'),
        'library_layout_length':
        ('./DESIGN/LIBRARY_DESCRIPTOR/LIBRARY_LAYOUT/PAIRED',
         'NOMINAL_LENGTH'),
        'library_layout_sdev':
        ('./DESIGN/LIBRARY_DESCRIPTOR/LIBRARY_LAYOUT/PAIRED', 'NOMINAL_SDEV'),
        'pooling_stategy': ('./DESIGN/LIBRARY_DESCRIPTOR/POOLING_STRATEGY',
                            'text'),
        'library_construction_protocol':
        ('./DESIGN/LIBRARY_DESCRIPTOR/LIBRARY_CONSTRUCTION_PROTOCOL', 'text'),
...
def _parse_taxon(node):
    ...
    try:
        if d['tax_analysis']['nspot_analyze'] is not None:
            d['tax_analysis']['nspot_analyze'] = int(
                d['tax_analysis']['nspot_analyze'])
    except:
    ...
    try:
        if d['tax_analysis']['total_spots'] is not None:
            d['tax_analysis']['total_spots'] = int(
                d['tax_analysis']['total_spots'])
    except:
    ...
    try:
        if d['tax_analysis']['mapped_spots'] is not None:
            d['tax_analysis']['mapped_spots'] = int(
                d['tax_analysis']['mapped_spots'])
    except:
...
class SRAExperimentPackage(object):
    def __init__(self, node):
        ...
        try:
            ...
            runs = list(
                [SRARunRecord(run).data for run in node.findall(".//RUN")])
            self.data = {
...
def srastatrep(**params):
    ...
    def _process_row(row):
        if ("meta" in row and row['meta'] != ''):
            ...
            if (row['insdc'] == 'True'):
                d['insdc'] = True
            pydantic_model = getattr(pydantic_models,
                                     'Sra' + model_type.capitalize())(**d)
...
def load_experiment_xml_by_accession(accession):
    with urllib.request.urlopen(
            'https://trace.ncbi.nlm.nih.gov/Traces/sra/sra.cgi?save=efetch&db=sra&rettype=FullXml&term={}'
            .format(accession)) as response:
...
def load_runbrowser_xml_by_accession(accession):
    ...
    with session().get(
            "https://trace.ncbi.nlm.nih.gov/Traces/sra/?run={}&retmode=xml".
            format(accession)) as response:
...
def run_iterator(from_date="2001-01-01",
                 to_date="2050-01-01",
                 count=100,
                 offset=0):
...
def get_accession_list(from_date="2001-01-01",
                       to_date="2050-01-01",
                       count=100,
                       offset=0,
                       type="EXPERIMENT"):
    column_names = [
        "Accession", "Submission", "Type", "Received", "Published",
        "LastUpdate", "Status", "Insdc"
    ]
...
def sra_object_generator(fh):
    ...
    for event, element in et.iterparse(fh):
        if ((element.tag.lower() in validClasses) and (event == "end")):
            yield getattr(parsers, "SRA" + element.tag.title() + "Record")(element)
...
class LiveList(Iterable):
    ...
    def _url(self) -> str:
        ...
        columns = ",".join([
            "Accession", "Submission", "Type", "Received", "Published",
            "LastUpdate", "Status", "Insdc", "ReplacedBy", "Meta"
        ])
        ...
        url = url.format(self.from_date, self.count, self.offset, columns,
            ...
            try:
                ...
                if (response.status_code == 200):
                    ...
                    for row in vals['rows']:
                        reader.append(dict(zip(colnames,
                                               [val for val in row])))
