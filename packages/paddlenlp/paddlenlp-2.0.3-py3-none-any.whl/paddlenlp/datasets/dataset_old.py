    ...
    def read_datasets(self, splits=None, data_files=None):
        ...
        if data_files:
            assert isinstance(data_files, str) or isinstance(
                ...
                data_files, list
            )) and (isinstance(data_files[0], str) or isinstance(data_files[0], dict))), "`data_files` should be a string or dictionary or tuple or list of strings or dictionaries whose key is split name and value is the path(s) of data file(s)."

            default_split = 'train'
            if isinstance(data_files, str) or isinstance(data_files, dict):
                data_files = [data_files]
            for data_file in data_files:
                if isinstance(data_file, str):
                    datasets.append(self.read(filename=data_file, split=default_split))
                elif isinstance(data_file, dict):
                    assert len(data_file) == 1, "Each dictionary should have only one key when using dictionary to specify the split of loacl data file."
                    split, filename = data_file.popitem()
                    if isinstance(filename, str) :
                        filename = [filename]
                    for f in filename:
                        datasets.append(self.read(filename=f, split=split))
