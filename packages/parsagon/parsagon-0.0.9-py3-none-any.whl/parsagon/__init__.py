...
class Client:
    ...
    def execute(self, scraper_name, urls, env, max_page_loads=1, scroll_speed='fast', format='json', output='data', file_path='', is_retry=False):
        ...
        data = {'scraper_name': scraper_name, 'urls': urls, 'max_page_loads': max_page_loads, 'scroll_speed': SCROLL_SPEEDS[scroll_speed], 'environment': ENVIRONMENTS[env], 'is_retry': is_retry}
        ...
        if not result.get('id2'):
            data = {'scraper_name': scraper_name, 'urls': urls, 'max_page_loads': max_page_loads, 'scroll_speed': SCROLL_SPEEDS[scroll_speed]}
