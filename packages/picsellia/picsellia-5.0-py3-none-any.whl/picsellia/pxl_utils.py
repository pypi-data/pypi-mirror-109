...
def check_status_code(request):
    ...
    if int(request.status_code) in [200, 201]:
        ...
        raise exceptions.UnauthorizedError("Not allowed to read/write on this element, please check your token or permissions")

    elif int(request.status_code) == 404:
        raise exceptions.ResourceNotFoundError("Ressource not found")
