...
SAMPLES_LINE_CMD_PATTERN = "zgrep -m1 '^#CHROM' {}"
SEARCH_CMD_PATTERN = 'bcftools view {} {}:{}'
GENOTYPE_STRING_RE = re.compile('\sGT(:[A-Z]{2,3})*\s(.+)')
...
def get_sample_names(gzipped_vcf_paths: List[str]) -> List[str]:
    logger.info('Getting sample names')
    sample_names_for_all_files = []
    for gzipped_vcf_path in gzipped_vcf_paths:
        p = subprocess.Popen(SAMPLES_LINE_CMD_PATTERN.format(gzipped_vcf_path), stdout=subprocess.PIPE,
                             shell=True)
        output, _ = p.communicate()
        samples_string = output.decode("utf-8").split('FORMAT')[1].strip()
