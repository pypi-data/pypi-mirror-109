...
class PortDataSplitter:
    ...
    def __init__(self, ip: str, port: int, port_names_list=['/dev/ttyUSB0',], debug=False, device_name='unknown',
                 test_mode=False, api_debug=False,  *args, **kwargs):
        """ Принимает:
        ...
        manual_value :str - данные, которые рассылает сервер рассылки. Его можно установить через метод set_test_value
        """
        ...
        self.data_list = [functions.get_format_send_message(weight_data=s.no_data_code,
    ...
    def start_api(self, ip, port, api_debug):
        """ Запустить API PDS"""
        self.api_server = QPI(ip, port, self, debug=api_debug, without_auth=True)
    ...
    def set_manual_value(self, manual_value, port_name=None, *args, **kwargs):
        ...
        port_name не указан, то это значение будет отправляться от всех портов из словаря self.ports_dict
        """
        return functions.set_manual_value(self.ports_dict, manual_value, port_name)

    def get_value(self, port_name=None, *args, **kwargs):
        """ Возвращает последний массив данных, у которого port_name=port_name, если же port_name=None, этот фильтр
        игнорируется и возвращается любой массив."""
        return functions.get_last_data_dict(self.data_list, port_name, *args, **kwargs)

    def unset_manual_value(self):
        """ Убрать отправку данных, заданных вручную операцией set_manual_value. Отныне данные будут отправлять из
        object_port, если PDS не запущен в режиме тестов. Если же он запущен в режиме тестов, он попытается отправить
        manual_set=None, но после форматирования функцией get_manual_set_value отправит 101010 (потому что None не
        отправляется
        """
    ...
    def _mainloop(self):
        ...
        for port_name in self.port_names_list:
            threading.Thread(target=self.serve_port_name, args=(port_name,)).start()

    def serve_port_name(self, port_name):
        """ Обслуживать порт port_name"""
        self.show_print('\nStart serving thread for port: {}'.format(port_name))
        ...
        while True:
            if not self.test_mode:
                ...
                data = functions.get_manual_set_value(self.ports_dict[port_name]['manual_value'])
                sleep(1)
    ...
    def create_port(self, port_name):
        self.ports_dict[port_name] = {'manual_value': None, 'manual_value_set': False, 'test_mode': self.test_mode}
        if not self.test_mode:
            port = Serial(port_name, bytesize=8, parity='N', stopbits=1, timeout=1, baudrate=9600)
    ...
    def prepare_data_to_send(self, port_name, data, *args, **kwargs):
        ...
        data_to_send = functions.get_format_send_message(weight_data=data, port_name=port_name,
                                                         manual_value_set=self.ports_dict[port_name]['manual_value_set'],
                                                         test_mode=self.test_mode, creation_date=datetime.datetime.now())
