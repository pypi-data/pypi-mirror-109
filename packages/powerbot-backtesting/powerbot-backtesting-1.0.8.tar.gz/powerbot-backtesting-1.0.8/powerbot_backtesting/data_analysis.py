...
class BacktestingAlgo:
	...
	This class is used by providing two parameters: the orderbooks and an input file based on these orderbooks.
	...
	the algorithm function. All signals/ columns that you have added to the input file will be reflected as class
	attributes. Please be aware, that the column names will be the attribute names after conversion.
	...
	Any trades you perform alter the orderbook and will also be gathered in a list of results.
	"""
	...
	def match_orders(self, side, orderbook, timestamp, price, position, contract_time, key, order_execution):
	...
	def run(self):
		for nr, (key, contract) in enumerate(self.orderbooks.items()):
			...
			self.params["contract_time"] = int((datetime.strptime(contract_time[3], TIME_HM) - datetime.strptime(contract_time[1],
			                                                                                                     TIME_HM)).seconds / 60)
			...
			if self.params["position"]:
				self.results[key]["Remaining Quantity"] = float(round(self.params["position"], 1))
	...
	def algorithm(self, timestamp: str, orderbook: pd.DataFrame, key: str) -> bool:
		"""
		...
		Args:
			...
			contract_time (int): Contract time in minutes (60, 30 or 15)
		...
		if self.params["position"] < 0:
			self.params["position"] = - self.match_orders(
				...
				orderbook=orderbook,
				timestamp=timestamp,
				price=self.params["price"] - 2 if "price" in self.params else 38,
		...
		# Sell
			...
			self.params["position"] = self.match_orders(
				...
				orderbook=orderbook,
				timestamp=timestamp,
				price=self.params["price"] + 2 if "price" in self.params else 42,
