	...
	for key, value in data.items():
		...
		if as_csv and not cache_path.joinpath(
			...
			value.to_csv(cache_path.joinpath(
				f"{environment}\\{exchange}_{delivery_area}\\{year_month}\\{day_month}\\"
				f"{folder}\\{file_name}.csv{file_ending}"),
...
def _get_file_cachepath(api_client: ApiClient, contract_key: str, delivery_area: str) -> str:
	"""
...
def _check_contracts(contract, delivery_areas: list[str], products: list[str]) -> bool:
	"""
...
def _order_matching(order_side: str,
	...
	IOC - Immediate and Cancel, order is executed to maximum extent by first matching order, next orderbook is loaded ->
	...
	for ind, row in orderbook.iterrows():
		if quantity > 0:
			...
			if price_match:
				...
				cash = traded_quant * Decimal(row.price) * operator[order_side] / cash_adjust[contract_time]  # Calculate Cost
...
def _delta_filter(orderbook: pd.DataFrame) -> pd.DataFrame:
	...
	Returns:
		pd.Dataframe
	"""
	ind = orderbook[~orderbook.delta].index
	if not ind.empty:
		last_delta = ind[-1]
		...
		return orderbook[orderbook.index >= last_delta].drop(columns=["delta"])
