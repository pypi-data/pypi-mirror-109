...
class MonitoringCenterService(RequestBase, ABC):
    ...
    def get_weibo_s(self, uid):
        ...
        return self.get(f"{self.url}/api/wb/get-s/", timeout=self.TIMEOUT, params=data, headers=self._headers).json()
    ...
    def get_weibo_registered_equipment(self):
        ...
        """
        return self.get(f"{self.url}/api/wb/registered-equipment", timeout=self.TIMEOUT, headers=self._headers).json()
