...
def validate_zip_file(zip_file_path, app_path):
    try:
        ...
        biolib_logger.error('App could not be zipped')
        raise e
    ...
    if f'.biolib' in files and f'biolib' in files:
        biolib_logger.error('You provided both a biolib and a .biolib folder. Please only provide the .biolib folder')
        sys.exit(1)
        ...
        biolib_logger.error('Your biolib folder appears to be called "biolib" - it must be called ".biolib".')
        sys.exit(1)

    else:
        biolib_logger.error(f'Could not find a .biolib folder in provided app folder {app_path}')
        sys.exit(1)
    ...
    if f'{biolib_folder_path}/config.yml' in files:
        ...
        biolib_logger.error('Your biolib config file has the .yaml file extension - it must be .yml')
        sys.exit(1)
