...
def main():
    ...
    utils.configure_logging(default_log_level=logging.ERROR)
    ...
    if len(sys.argv) > 2 and sys.argv[1] == 'run':
        ...
        utils.logger.info(f'Running BioLib application in local directory {app_path}...')
        ...
        try:
            ...
            if isinstance(exception, BiolibValidationError):
                utils.logger.error('Validation check failed for config file at .biolib/config.yml')
                sys.exit(1)
            raise exception
        ...
        if args.command == 'push':
            ...
            utils.configure_logging(default_log_level=logging.INFO)
