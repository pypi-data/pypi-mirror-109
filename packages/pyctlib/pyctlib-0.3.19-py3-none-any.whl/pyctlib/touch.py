...
def retry(func: Callable, max_num=10, interval=0.5, logger=None, logger_str=None):
    step = 0
    while max_num < 0 or step < max_num - 1:
        ...
        if logger is not None:
            if logger_str:
                logger.debug("encounter error, try again [%d/%d]".format(step, max_num), logger_str)
            else:
                logger.debug("encounter error, try again [%d/%d]".format(step, max_num))
        sleep(interval)
    return func()
