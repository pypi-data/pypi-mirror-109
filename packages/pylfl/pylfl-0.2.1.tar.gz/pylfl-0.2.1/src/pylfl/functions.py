...
def gmin():
    if os.path.isfile('data') and os.path.isfile('coords') and os.path.isfile('MLPdata'):
        ...
        if opsys != 'win':
            subprocess.call(f'bin/{opsys}/GMIN', stdout=f)
        else:
            subprocess.call('bin/win/GMIN.exe', stdout=f)
    else:
...
def optim(outfile='optim_output.txt'):
    if os.path.isfile('odata') and os.path.isfile('MLPdata'):
        ...
        if opsys != 'win':
            subprocess.call(f'bin/{opsys}/OPTIM', stdout=f)
        else:
            subprocess.call('bin/win/OPTIM.exe', stdout=f)
    else:
...
def pathsample():
    if os.path.isfile('pathdata') and os.path.isfile('MLPdata'):
        ...
        if opsys != 'win':
            subprocess.call(f'bin/{opsys}/PATHSAMPLE', stdout=f)
        else:
            subprocess.call('bin/win/PATHSAMPLE.exe', stdout=f)
    else:
