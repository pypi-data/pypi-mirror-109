...
class IveoDevice(Device):
    ...
    def manualIveoCommand(self, command):
        command = IveoCommandManual(self.mask, command)
        command.execute(self.gateway)
        return command

    def automaticIveoCommand(self, command):
    ...
    def stop(self):
        self.automaticIveoCommand(self.mask, CommandTypeIveo.STOP)

    def moveDown(self):
        self.automaticIveoCommand(self.mask, CommandTypeIveo.DRIVEDOWN)
    
    def moveUpIveo(self):
        self.automaticIveoCommand(self.mask, CommandTypeIveo.DRIVEUP)
    
    def moveIntermediatePosition1(self):
        self.manualIveoCommand(self.mask, CommandTypeIveo.POSITION_1)

    def moveIntermediatePosition2(self):
        self.manualIveoCommand(self.mask, CommandTypeIveo.POSITION_2)
