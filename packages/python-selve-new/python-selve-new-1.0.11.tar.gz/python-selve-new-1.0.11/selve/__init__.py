...
class Gateway():   
    ...
    async def __init__(self, port, discover = True):
        """                
        Arguments:
    ...
    async def executeCommand(self, command):
        a = CommeoServiceGetState()
        await self._executeCommand(a)
        _LOGGER.debug("Service State: " + str(a.status))
        if a.status == ServiceState.READY:
            return self._executeCommand(command)
        else:
            _LOGGER.error("Gateway not ready to execute command")
        return None

    async def _executeCommand(self, command):
        """[summary]
