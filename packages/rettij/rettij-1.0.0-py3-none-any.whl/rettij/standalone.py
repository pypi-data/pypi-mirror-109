...
def standalone() -> None:
    ...
    parser.add_argument(
        ...
        default="",
        type=str,
        help=f"Path to simulation sequence file (optional). Use '{SRC_DIR}/examples/sequences/basic_ping_sequence.py' to run an example sequence.",
    )
    ...
    kubeconfig_path: str = args.kubeconfig
    topology_path: ValidatedFilePath = ValidatedFilePath(args.topology)
    sequence_path: str = args.sequence
    components_dir_path: ValidatedDirPath = ValidatedDirPath(args.components)

    file_loglevel = Loglevel[args.loglevel]  # define file loglevel
    console_loglevel = Loglevel[args.console]  # define console loglevel

    if args.monitoring:
        monitoring_logging.get_logger(args.monitoring_config)

    rettij: Rettij = Rettij(
        file_loglevel=file_loglevel, console_loglevel=console_loglevel
    )  # create rettij simulator object

    cli_exit_event: threading.Event = threading.Event()

    try:
        rettij.init(
            topology_path, sequence_path, kubeconfig_path=kubeconfig_path, components_dir_path=components_dir_path
        )
        rettij.create()
