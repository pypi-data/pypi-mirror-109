...
class Node:
    ...
    __NAME_MAX_LENGTH: int = 20  # arbitrary value for now which should be long enough, though
    ...
    def __init__(
        ...
        :param config: Executor-specific configuration object.
        :param data: Initial co-simulation data.
        """
    ...
    def copy_file_from_node(self, src_file_path: str, dst_dir: str = "", exec_now: bool = True) -> Command:
        """
        ...
        """
        cmd = RetrieveFileCommand(self.executor, src_file_path, dst_dir)
        if exec_now:
            cmd.execute(0, {})
        return cmd
    ...
    def name(self, name: str) -> None:
        ...
        """
        if len(name) > Node.__NAME_MAX_LENGTH:
            raise ValueError(f"Length of node name {name} too long (max. {Node.__NAME_MAX_LENGTH})")
        self.__name = name
    ...
    def get_data(self, keys: Optional[List[str]] = None) -> Dict[str, Dict[str, Any]]:
        ...
        """
        self.data.update(self.executor.read_values(keys))
        return self.data
    ...
    def get_name_max_length(cls) -> int:
        """
        Retrieve the maximum Node name length.

        :return: Maximum Node name length.
        """
        return cls.__NAME_MAX_LENGTH
    ...
    @config.setter
    def config(self, config: KubernetesPodConfig) -> None:
        """
