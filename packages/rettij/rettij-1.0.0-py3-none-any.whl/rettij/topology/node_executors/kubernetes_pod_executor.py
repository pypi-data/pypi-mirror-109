...
class KubernetesPodExecutor(NodeExecutor):
    ...
    def generate_config(self, **kwargs: Any) -> Dict[str, Any]:
        ...
        if not isinstance(node.config, KubernetesPodConfig):
            raise RuntimeError("KubernetesPodExecutor can only be used with KubernetesPodConfig!")

        pod_config: KubernetesPodConfig = node.config
    ...
    def execute_command(
        ...
        **kwargs: Any,
        ...
        # Make sure to always log stderr as an error
        if result.std_err:
            self.logger.error(f"\n{result.std_err}")
