...
class TestRettij(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.topology_path: ValidatedFilePath = ValidatedFilePath(
            os.path.join(self.test_base_path, "resources", "topology.yml")
        )
        self.scheduled_sequence_path: ValidatedFilePath = ValidatedFilePath(
            os.path.join(self.test_base_path, "resources", "scheduled_sequence.py")
        )
        self.script_sequence_path: ValidatedFilePath = ValidatedFilePath(
            os.path.join(self.test_base_path, "resources", "script_sequence.py")
        )
    ...
    def reboot(self) -> None:
        ...
        # Make sure the execution time is as close to 10 seconds as possible (full accuracy isn't possible).
        self.assertLess(abs((datetime.now() - start_time).seconds - 10), 0.1)
