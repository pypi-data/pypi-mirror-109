...
class TopologyReaderTest(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.custom_components_dir_path = ValidatedDirPath.join_paths(
            cls.test_base_path, "resources", "custom_components"
        )
    ...
    def test_reader_with_valid_topo(self) -> None:
        ...
        try:
            ...
            TopologyExporterMermaid(nodes, channels).export(
                str(Path(tempfile.mkdtemp()) / "test_reader_with_valid_topo.mermaid")
            )
    ...
    def test_reader_with_single_node_and_no_interface(self) -> None:
        ...
        try:
            ...
            TopologyExporterMermaid(nodes, channels).export(
                str(Path(tempfile.mkdtemp()) / "test_reader_with_no_interfaces.mermaid")
            )
        except Exception as e:
            self.fail(
                msg=f"[{self.__class__.__name__}.test_reader_with_various_configs]: Testing reading of correct topology without interfaces failed. "
    ...
    def test_reader_without_mac(self) -> None:
        ...
        try:
            ...
            TopologyExporterMermaid(nodes, channels).export(
                str(Path(tempfile.mkdtemp()) / "test_reader_without_mac.mermaid")
            )
        except Exception as e:
            self.fail(msg=str(e))
    ...
    def test_valid_custom_components(self) -> None:
        ...
        try:
            ...
            if not isinstance(nodes["n0"].config, KubernetesPodConfig):
                raise RuntimeError("test_valid_custom_components can only be used on Kubernetes nodes!")
            n0_config: KubernetesPodConfig = nodes["n0"].config
            if not isinstance(nodes["n1"].config, KubernetesPodConfig):
                raise RuntimeError("test_valid_custom_components can only be used on Kubernetes nodes!")
            n1_config: KubernetesPodConfig = nodes["n1"].config
            self.assertIsInstance(n0_config.pod_spec, Dict)
            self.assertIsInstance(n1_config.pod_spec, Dict)

            TopologyExporterMermaid(nodes, channels).export(
                str(Path(tempfile.mkdtemp()) / "test_valid_custom_components.mermaid")
            )
        except Exception as e:
            self.fail(msg=str(e))
    ...
    def test_valid_gateways(self) -> None:
        ...
        try:
            ...
            TopologyExporterMermaid(nodes, channels).export(
                str(Path(tempfile.mkdtemp()) / "test_valid_gateways.mermaid")
            )
        except Exception as e:
            self.fail(msg=str(e))
    ...
    def test_component_hooks(self) -> None:
        ...
        try:
            ...
            n0_hooks = nodes["n0"].config.hooks
            ...
            TopologyExporterMermaid(nodes, channels).export(
                str(Path(tempfile.mkdtemp()) / "test_component_hooks.mermaid")
            )
    ...
    def test_inplace_components(self) -> None:
        ...
        try:
            ...
            TopologyExporterMermaid(nodes, channels).export(
                str(Path(tempfile.mkdtemp()) / "test_inplace_components.mermaid")
            )
        except Exception as e:
            self.fail(msg=str(e))
    ...
    def test_reader_with_unconnected_channels(self) -> None:
        ...
        try:
            ...
            TopologyExporterMermaid(nodes, channels).export(
                str(Path(tempfile.mkdtemp()) / "test_reader_with_unconnected_channels.mermaid")
            )
        except Exception as e:
            self.fail(msg=str(e))
