...
class Factory:
    ...
    def __API(self, api='Old', Url=None):
        ...
        if api == 'Data':
            request = Request('http://sama-device-data:8080/api/')
        ...
        if api == 'RuleExe':
            request = Request('https://rule-exe:5001/')

        if api == 'WorkflowsExe':
            request = Request('https://workflow-exe:5000/')
...
#
# Pipeline().GetLastData(aux.AuxOnID('489'), lastData)
# #
