...
class LongHandlerEN(Operation):
    def __init__(self, symbols: dict, white_list: str, min_sentence_length: int = 5, name=None):
        ...
        self.is_dialogue_gt = condition.DialogueIsGreaterThanEN(3)
        self.is_bracket_gt = condition.BracketIsGreaterThan(3, symbols)
        ...
        for i in range(state.sentence_start, org_pointer):
            ...
            if (self.is_sentence_dash(state) or self.symbols['semicolon'].match(
                ...
                return i, True
                    ...
                    state.current_value) and sub_len > self.min_sentence_length and self.is_bracket_gt(
                state) and self.is_next_with_space(state):
