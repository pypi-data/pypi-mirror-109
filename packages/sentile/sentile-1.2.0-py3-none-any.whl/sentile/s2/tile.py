...
class Tile:
    ...
    def get_MGRS_GDZ(self) -> str:
        return self.name[1:len(self.name)-2]

    def get_MGRS_square_ID(self) -> str:
        return self.name[-2:]

    def get_MGRS_UTM_zone(self) -> int:
        utm_zone = [int(s) for s in re.findall(r'-?\d+\.?\d*', self.name)]
        assert(len(utm_zone) == 1)
        return utm_zone[0]

    def get_MGRS_latitude_band(self) -> str:
        return self.name[-3:-2]

    def get_MGRS_square_column(self) -> str:
        return self.name[-2:-1]

    def get_MGRS_square_row(self) -> str:
        return self.name[-1:]
