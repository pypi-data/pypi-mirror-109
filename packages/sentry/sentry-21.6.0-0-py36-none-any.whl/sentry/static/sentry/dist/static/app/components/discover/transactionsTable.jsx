        ...
        var headers = tableTitles.map(function (title, index) {
            ...
            <GuideAnchor target="span_op_relative_breakdowns">
              <SortLink align={align} title={title === t('operation duration') ? (<React.Fragment>
                      {title}
                      <Tooltip title={t('Durations are calculated by summing span durations over the course of the transaction. Percentages are then calculated by dividing the individual op duration by the sum of total op durations. Overlapping/parallel spans are only counted once.')}>
