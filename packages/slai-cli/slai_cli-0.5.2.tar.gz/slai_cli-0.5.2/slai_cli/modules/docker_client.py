...
class DockerClient:
    def __init__(self, *, project_name, project_id):
        ...
        self.earliest_image_tag_time = dateutil.parser.isoparse(
            self.cli_version["earliest_docker_image_tag_time"]
        )
    ...
    def _connect_to_container(self):
        ...
        if len(containers) == 1:
            ...
            self.last_tag_time = dateutil.parser.isoparse(
                self.project_container.image.attrs["Metadata"]["LastTagTime"]
            )

            if self.last_tag_time < self.earliest_image_tag_time:
                log.warn(
