...
class Api(snappi.Api):
    ...
    def _loadPorts(self):
        if 'ports' in self._cfg:
            ...
            for p in self._cfg['ports']:
                self._portIndices.append(p['name'])
    ...
    def set_config(self, config):
        ...
        try:
            # prepare our ports
            self._c.reset(ports = list(range(len(self._portIndices))))
            ...
            for f in self._cfg["flows"]:
                ...
                self._c.add_streams([s1], ports=[self._portIndices.index(f['tx_rx']['port']['tx_name'])])
    ...
    def set_transmit_state(self, payload):
        ...
        try:
            ...
            self._c.start(ports = list(range(len(self._portIndices))))

            # Wait until traffic stops
            self._c.wait_on_traffic(ports = list(range(len(self._portIndices))))
