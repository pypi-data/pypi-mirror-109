    ...
    def transform(self, X, reduction = 'PCA'):
        ...
        if self.dispersion_method == 'UMAP':
            ...
            if Y.shape[0] == 1:
                ...
                self.disp_model = UMAP(learning_rate=self.um_lr, n_components=self.dim, n_epochs=self.um_epochs, init=Y_init, min_dist=self.um_min_dist, verbose=self.verbose)
                X_pc = self.pca.transform(X)
                if self.verbose:
