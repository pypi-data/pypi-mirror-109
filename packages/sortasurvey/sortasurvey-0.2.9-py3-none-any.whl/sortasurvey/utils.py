...
def assign_priorities(survey, obs={}, m=1):
    ...
    obliquity = [int(np.floor(each)) for each in survey.programs.loc['SC2Bii', 'high_priority']]
    for target in survey.programs.loc['SC2Bii', 'high_priority']:
        if int(np.floor(target)) not in observed.toi.values.tolist():
            idx=len(observed)
            observed.loc[idx,'toi'] = int(np.floor(target))
            observed.loc[idx,'tic'] = survey.df[survey.df['toi'].isin([target])]['tic'].values.tolist()[0]
            observed.loc['overall_priority'] = idx+1
            observed.loc['programs'] = ['SC2Bii']
        else:
            idx=observed.index[observed['toi'].isin([int(np.floor(target))])].tolist()[0]
            observed.loc[idx,'programs'].append('SC2Bii')
