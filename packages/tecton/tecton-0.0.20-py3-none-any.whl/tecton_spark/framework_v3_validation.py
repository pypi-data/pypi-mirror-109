...
def _run_duplicate_fco_validation(
    ...
    # 3. Validate that produced dataframes are identical (besides timestamp 1s difference)
...
def _compare_feature_dataframes(fp_df: DataFrame, fv_df: DataFrame, fp_name: str, ts_column: str):
    ...
    if ts_column in fp_df:
        ...
        # Parameter `batch_schedule` is always multiple of 1 hour, so we check for `xx:59:59`
        all_timestamps_adjusted = len(fv_df) == len(
            fv_df.loc[(fv_df[ts_column].dt.minute == 59) & (fv_df[ts_column].dt.second == 59)]
        )
        if all_timestamps_adjusted:
            fv_df.loc[
                (fv_df[ts_column].dt.minute == 59) & (fv_df[ts_column].dt.second == 59), ts_column
            ] += pandas.to_timedelta("1S")
