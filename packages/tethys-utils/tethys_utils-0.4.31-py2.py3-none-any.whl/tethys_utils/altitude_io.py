...
dem_remote = {'bucket': 'nasa-data', 'connection_config': 'https://b2.tethys-ts.xyz'}
...
def get_altitude(stn_df, dem_remote=dem_remote, dem_ds_id=dem_ds_id, source_ds_id=None, source_remote=None):
    ...
    ## Get existing stns
    if isinstance(source_remote, dict) and isinstance(source_ds_id):
        try:
    ...
    for i, row in combo1.iterrows():
        ...
        if np.isnan(row['altitude']):
            results = t1.get_results(dem_ds_id, lon=row['lon'], lat=row['lat'], squeeze_dims=True, output='DataArray', cache='memory')
            alt = int(results)
        else:
