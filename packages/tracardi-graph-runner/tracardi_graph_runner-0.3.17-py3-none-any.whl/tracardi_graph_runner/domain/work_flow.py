...
class WorkFlow:
    ...
    def __init__(self, flow_history: FlowHistory, session: Entity, profile: Entity, event: Entity):
    ...
    async def invoke(self, flow: Flow, debug=False) -> DebugInfo:
        ...
        if self.flow_history.is_acyclic(flow.id):
            ...
            exec_dag.init(self.event, self.session, self.profile, debug=debug)

            debug_info = await exec_dag.run(payload={}, flow_id=flow.id)
            debug_info.event.id = self.event.id
