...
from trident.backend.common import to_list, format_time, get_terminal_size, get_session, get_backend, \
    PrintException, OrderedDict, split_path, sanitize_path, Signature, adaptive_format, dtype, cyan_color
    ...
    def data_preprocess(self, img_data):
        ...
        if isinstance(img_data, np.ndarray):
            ...
            if self.input_spec is None:
                self.signature.inputs['input'] = TensorSpec(shape=tensor_to_shape(to_tensor(img_data), need_exclude_batch_axis=False), dtype=dtype.float32,
        ...
        if is_tensor(model):
            ...
            if isinstance(output, (list, tuple)):
                ...
                if is_training and self.use_output_as_loss:
                    ...
                    self.training_context['losses'].collect(signature.outputs.key_list[0], self.training_context['steps'], this_loss)
                    self.training_context['current_loss'] = self.training_context['current_loss'] + this_loss
