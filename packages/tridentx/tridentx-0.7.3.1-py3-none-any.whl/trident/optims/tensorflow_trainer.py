...
class Model(ModelBase):
    ...
    def do_on_optimization_step_end(self):
        ...
        self.training_context['losses'].collect('total_losses', self.training_context['steps'], to_numpy(self.training_context['current_loss']).mean())
...
class MuiltiNetwork(Model):
    ...
    def do_on_training_start(self):
        ...
        self.training_context['steps'] == 0
        for k in self._networks.keys():
            self._networks[k].do_on_training_start()

    def do_on_training_end(self):
        self.eval()
        for k in self._networks.keys():
            self._networks[k].do_on_training_end()
