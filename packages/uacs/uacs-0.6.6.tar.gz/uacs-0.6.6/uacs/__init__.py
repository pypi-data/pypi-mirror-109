...
# TODO : integration du choix des "services status"
...
def append(append_service, verbose):  # Ajoute un service a la liste et l'ajoute également au fichier (file_ini)
    ...
    print(f""""sudo systemctl list-unit-files | grep "^{append_service}" """)
    exist_service = os.system(f""""sudo systemctl list-unit-files | grep "^{append_service}" """)
...
def check_service(verbose):
    ...
    for i in range(len(service_check)):
        print(f"systemctl is-failed {service_check[i]}")
        active = os.system(f"sudo systemctl is-failed {service_check[i]}")
        if str(active).find("failed"):

            if service_check[i] == subject:
                if time.time() - last_mail >= 300:
                    print(f"Send mail")
                    send_mail(service_check[i], verbose)
...
def service_down(service, verbose):
    ...
    for i in range(number_restart):
        ...
        active = os.system(f"sudo systemctl status {service}")
...
def send_mail(subject, verbose):
    ...
    service_status = subprocess.Popen(f"sudo systemctl is-failed {subject}", shell=True, stderr=subprocess.PIPE)

    service_enable = subprocess.Popen(f"sudo systemctl is-enabled {subject}", shell=True, stderr=subprocess.PIPE)
...
Service Status : {service_status}, {service_enable}
...
"""
    ...
    try:
        ...
        print(f"Mail with id {mail_id} error")
...
def run_daemon():
    threading.Thread(target=daemon()).timedown_mail()
