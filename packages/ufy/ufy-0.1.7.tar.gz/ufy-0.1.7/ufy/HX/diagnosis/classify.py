...
def load_data_csvs(filenames, target_cols=1, drop_id=True, id_cols=1, split_rate=0.7, shuffle=True):
    ...
    if shuffle:
        ...
        np.random.shuffle(data)  # 功能与前行代码相同
...
def plot_roc_curve(y_trues, y_pres, model_names=None, pos_label=1, issave=False, savename='roc.jpg',
    ...
    for i in range(len(y_pres)):
        ...
        print(model_names[i] + ':')
        y_pres_i = (y_pres[i] > 0.5).astype(int)
...
def KNN(x_train, y_train, model_savepath='knn.pkl', istraining=False):
    if os.path.exists(model_savepath):
...
def model_test(x_test, y_test, models_path: List, models_name: List, savename: str, analysis_filename: str, summaryname: str):
    ...
    plot_roc_curve(y_trues, y_preds, models_name, pos_label=1, issave=True, savename=savename,
                   analysis_filename=analysis_filename)

    summary_data = DataFrame(columns=['model', 'Accuracy', 'Kappa', 'sensitivity', 'Specificity', 'F1-score', 'AUC'])
    for i in range(len(models_name)):
        evaluate = Evaluation(model_name=models_name[i], y_true=y_trues[i], y_pred=y_trues[i])
