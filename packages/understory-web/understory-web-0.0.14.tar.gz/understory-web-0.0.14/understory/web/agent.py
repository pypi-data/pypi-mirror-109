...
def apply_dns(url):
    ...
    if url.host == "alice.example":
        url = str(url).replace("http://alice.example", "http://127.0.0.1:8080")
    elif url.host == "bob.example":
        url = str(url).replace("http://bob.example", "http://127.0.0.1:8081")
    elif url.host == "hello.example":
        url = str(url).replace("http://hello.example", "http://127.0.0.1:8082")
    else:
        url = str(url)
    return url
...
def unapply_dns(url):
    ...
    if url.host == "127.0.0.1":
        if url.port == 8080:
            url = str(url).replace("http://127.0.0.1:8080", "http://alice.example")
        elif url.port == 8081:
            url = str(url).replace("http://127.0.0.1:8081", "http://bob.example")
        elif url.port == 8082:
            url = str(url).replace("http://127.0.0.1:8082", "http://hello.example")
    else:
        url = str(url)
    return url
