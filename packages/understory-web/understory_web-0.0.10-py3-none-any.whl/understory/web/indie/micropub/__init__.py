...
content = web.application("Content", mount_prefix=r"\d{{4}}", db=False,
                          year=r"\d{4}", month=r"\d{2}", day=r"\d{2}",
...
def define_table():
    ...
    tx.db.define(resources="""permalink TEXT UNIQUE, version TEXT UNIQUE,
                              resource JSON""",
                 # TODO updates="""""",
...
class LocalClient:
    ...
    def create(self, resource_type, **resource):
        ...
        # XXX pprint(resource_kwargs)
        permalink_base = f"/{web.timeslug(published[0]['datetime'])}"
        ...
        while True:
            ...
            try:
                tx.db.insert("resources", permalink=permalink,
                             version=web.nbrandom(10), resource=resource)
...
@content.route(r"{month}/{day}/{seconds}(/{slug})?")
class Permalink:
    """An individual entry."""
