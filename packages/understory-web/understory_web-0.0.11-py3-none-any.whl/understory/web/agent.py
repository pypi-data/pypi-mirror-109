...
__all__ = ["post", "get", "put", "delete",
           "parse", "browser", "cache", "discover_link", "download"]
...
class Transaction:
    ...
    @property
    def card(self):
        return Semantics(mf.representative_card(self.mf2json.data,
                                                source_url=self.url))
...
class Browser:
    ...
    def get(self, url, data=None):
        """GET resource at `url`."""
        return self.handle("GET", url, data)
    ...
    def handle(self, method, url, data=None):
        """
        Handle a request to the outside web or a local WSGI app.

        Use `requests` to reach the world wide web. Otherwise communicate
        directly with WSGI apps in-process.

        """
        # TODO remove extraneous WSGI items
        url = str(url)
        if url.startswith(("http://", "https://")):
            return getattr(requests, method.lower())(url)
        response = []
        hostname, _, path = url.partition("/")
        path, _, query = path.partition("?")
        app = self.apps[hostname]
        if data:
            wsgi_input = io.StringIO(json.dumps(data))
        else:
            wsgi_input = open("/dev/null")
        env = {'HTTP_HOST': hostname,
               ...
               # XXX 'REMOTE_PORT': '44612',
               'REQUEST_METHOD': method,
               # XXX 'SCRIPT_NAME': '',
               'SERVER_PROTOCOL': 'HTTP/1.1',
               'SERVER_SOFTWARE': 'gunicorn/0.17.4',
               # XXX 'SERVER_NAME': 'localhost',
               # XXX 'SERVER_PORT': '22739',
        ...
        response.append(app(env, start_response))

        transaction = Transaction(url, fetch=False)
        transaction.status = web.tx.response.status
        transaction.headers = web.tx.response.headers
        transaction.text = web.tx.response.body
        return transaction
