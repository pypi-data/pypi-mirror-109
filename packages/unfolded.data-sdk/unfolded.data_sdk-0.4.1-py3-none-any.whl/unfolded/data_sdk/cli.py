...
def list_maps() -> None:
    ...
    """
    data_sdk = DataSDK()
    output_data = [map.dict() for map in data_sdk.list_maps()]
    click.echo(json.dumps(output_data, indent=4, default=pydantic_encoder))
...
def get_map(map_id: str) -> None:
    """Get an Unfolded map record by its id
    """
    data_sdk = DataSDK()
    map = data_sdk.get_map_by_id(map_id)
    click.echo(map.json())
