...
class Downloader:
    ...
    def download(_to,_from,_what,_back):
        ...
        if(pkg["isfile"]):
            with open(Processing.correct_path(_to + pkg["package"] + "." + _back), "w") as f:
                f.write(text)
        else:
            ...
            for i in r:
                if i["type"] == "file":
                    with open(Processing.correct_path(folder + "/" + i["name"]), "w") as f:
                    ...
                    for j in r2:
                        if j["type"] == "file":
                            with open(Processing.correct_path(folder + "/" + i["name"] + "/" + j["name"]), "w") as f:
                                text = Downloader.get(_from + pkg["package"] + "/" + i["name"] + "/" + j["name"])
                                f.write(text)

        if(len(pkg["dep"]) > 0):
            for d in pkg["dep"].values():
                with open(Processing.correct_path(_to + d + "." + _back), "w") as f:
