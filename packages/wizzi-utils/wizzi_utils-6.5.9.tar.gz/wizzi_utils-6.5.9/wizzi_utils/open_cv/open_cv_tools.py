...
from wizzi_utils.open_cv.models_dnn_meta_data import MODELS_DNN_META_DATA
...
class DnnObjectDetectionModels:
    ...
    def _download_if_needed(local_path: str, url_dict: dict, file_key: str) -> None:
        if not os.path.exists(local_path):
            ...
            if download_style == DownloadStyle.Direct.value:
                ...
                mt.extract_file(src=comp_path, dst_folder=extracted_folder, file_type=download_style)  # extract
    ...
    def __init__(self,
        ...
        if model_name not in MODELS_DNN_META_DATA:
            mt.exception_error('model name must be one of {}'.format(list(MODELS_DNN_META_DATA.keys())), tabs=tabs)
            exit(-1)
        else:  # supported model
            self.model_conf = MODELS_DNN_META_DATA[model_name]
            ...
            if self.model_conf['job'] != Jobs.OBJECT_DETECTION.value:
                mt.exception_error('this class supports only {}'.format(Jobs.OBJECT_DETECTION.value), tabs=tabs)
                exit(-1)
        ...
        if net is None:
            mt.exception_error('Failed to create network')
            exit(-1)
