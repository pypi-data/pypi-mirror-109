...
SAVE_SCOPE = 'CVT'
BEST_MODEL = 'yolov3'
BEST_MODEL_SAVE_DIR = '{}/{}/{}'.format(mtt.CV2_MODELS, cvt.MODELS_DNN_META_DATA[BEST_MODEL]['job'], BEST_MODEL)
...
def __get_best_model():
    model = cvt.DnnObjectDetectionModels(
        save_load_dir=BEST_MODEL_SAVE_DIR,
        model_name=BEST_MODEL,
        # threshold=0.2,  # take default
        # nms_threshold=0.4, # take default
        allowed_class=None,
        tabs=1
    )
    print(model)
...
def __get_best_models():
    ...
    for m_name in models_names:
        save_dir = '{}/{}/{}'.format(mtt.CV2_MODELS, cvt.MODELS_DNN_META_DATA[m_name]['job'], m_name)
...
def __models_cap_test(
    ...
    fps_classify = mt.FPS(summary_title='detection')
