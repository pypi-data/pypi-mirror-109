...
from zapf.spec import is_function
...
class DevicesLayout(Struct):
    ...
    # 112..239
    fax64_params = Var(array(lreal, 1, 16), [0, 100, 10, 90, 5, 500, 0, 10, 2,
                                             0, 0.125, 0, 10, 1, 0, 0])
    ...
    # 660..723
    fax32_params = Var(array(real, 1, 16), [0, 100, 10, 90, 5, 500, 0, 10, 2,
                                            0, 0.125, 0, 10, 1, 0, 0])
...
class Global(Globals):
    ...
    # note: Name fileds will be overriden later in Init().
...
def handle_pctl(pctl, pvalue, valuestore):
    ...
    if cmd == 1:  # DO_READ
        if pnum in WRITEABLE_PARAMS:
            ...
            # index + 1 due to 1-based index in valuestore
            return 0x8000 | pnum, valuestore[WRITEABLE_PARAMS.index(pnum) + 1]
        # invent value and DONE
        # HACK: pnums <= 30 should be ints: fake this by returning 0
        return 0x8000 | pnum, 0 if pnum <= 30 else pnum**1.5
    if cmd == 2:  # DO_WRITE
        if pnum in WRITEABLE_PARAMS:
            valuestore[WRITEABLE_PARAMS.index(pnum) + 1] = pvalue
...
def Implementierung(v):
    ...
    d.ax64_value, d.ax64_target, d.ax64_estatus = \
        advance64(d.ax64_value, d.ax64_target, d.ax64_estatus)
    d.ax64_nerrid = d.ax64.estatus >> 15 if d.ax64_estatus[[31]] else 0

    d.ax32_value, d.ax32_target, d.ax32_estatus = \
        advance64(d.ax32_value, d.ax32_target, d.ax32_estatus)

    d.lax32_value, d.lax32_target, d.lax32_status = \
        advance32(d.lax32_value, d.lax32_target, d.lax32_status)

    d.dx16_target &= 0xff
    d.dx16_value, d.dx16_status = handle_dx16(d.dx16_value, d.dx16_target,
                                              d.dx16_status)

    d.dx32_target &= 0xffff
    d.dx32_value, d.dx32_estatus = handle_dx32(d.dx32_value, d.dx32_target,
                                               d.dx32_estatus)

    d.dx64_target &= 0xffffffff
    d.dx64_value, d.dx64_estatus = handle_dx32(d.dx64_value, d.dx64_target,
                                               d.dx64_estatus)
    ...
    d.fax64_value, d.fax64_target, d.fax64_estatus = \
        advance64(d.fax64_value, d.fax64_target, d.fax64_estatus,
    ...
    d.fax32_value, d.fax32_target, d.fax32_status = \
        advance32(d.fax32_value, d.fax32_target, d.fax32_status,
    ...
    d.vx32_valuetargets[31], d.vx32_valuetargets[32], d.vx32_status = \
        advance32(d.vx32_valuetargets[31], d.vx32_valuetargets[32],
                  d.vx32_status)
    if d.vx32_status[[14]]:  # while start/busy/stop
        for i in range(1, 31):
            d.vx32_valuetargets[i] = d.vx32_valuetargets[31]
    # handle pctl.
    d.vx32_pctl, d.vx32_pvalue = \
        handle_pctl(d.vx32_pctl, d.vx32_pvalue, d.fax32_params)

    d.vx64_valuetargets[31], d.vx64_valuetargets[32], d.vx64_estatus = \
        advance64(d.vx64_valuetargets[31], d.vx64_valuetargets[32],
                  d.vx64_estatus)
    ...
    # handle pctl.
    d.vx64_pctl, d.vx64_pvalue = \
        handle_pctl(d.vx64_pctl, d.vx64_pvalue, d.fax64_params)
