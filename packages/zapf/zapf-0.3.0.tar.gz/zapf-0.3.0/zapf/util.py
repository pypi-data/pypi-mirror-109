...
class UncasedMap(dict):
    ...
    def init_keys(self):
        """Return those possible keys that were keys in the initial mapping."""
        return self._keys

    def _gendefaults(self, prefix, idxlist):
        for idx in idxlist:
            if idx not in self:
                k = '%s%d' % (prefix, idx)
                self[k] = idx
...
class ParamMap(UncasedMap):
    def __init__(self, *args, **kwds):
        UncasedMap.__init__(self, *args, **kwds)
        self._gendefaults('Param', range(1, 128))
        self._gendefaults('Command', range(128, 192))
        self._gendefaults('Param', range(192, 224))
        self._gendefaults('Command', range(224, 240))
        self._gendefaults('Reserved', range(240, 256))
